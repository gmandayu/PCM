...
public partial class PCM {
    ...
    /// mtManningAgentEdit
    /// </summary>
    public static MtManningAgentEdit mtManningAgentEdit
    {
        get => HttpData.Get<MtManningAgentEdit>("mtManningAgentEdit")!;
        set => HttpData["mtManningAgentEdit"] = value;
    }

    /// <summary>
    /// Page class for MTManningAgent
    /// </summary>
    public class MtManningAgentEdit : MtManningAgentEditBase
    {
        // Constructor
        public MtManningAgentEdit(Controller controller) : base(controller)
        {
        }

        // Constructor
        public MtManningAgentEdit() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class MtManningAgentEditBase : MtManningAgent
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "mtManningAgentEdit";
        ...
        // Page name
        public string PageName => "MtManningAgentEdit";
        ...
        // Constructor
        public MtManningAgentEditBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        switch (CurrentAction) {
                case "show": // Get a record to display
                        if (!loaded) { // Load record based on key
                            ...
                            return Terminate("MtManningAgentList"); // No matching record, return to list
                        }
                    break;
        ...
        protected async Task<JsonBoolResult> EditRow() { // DN
            ...
            // Active
            Active.SetDbValue(rsnew, ConvertToBool(Active.CurrentValue), Active.ReadOnly);

            // Update current values
            SetCurrentValues(rsnew);

            // Check field with unique index (Code)
            if (!Empty(Code.CurrentValue)) {
                string filterChk = "([Code] = '" + AdjustSql(Code.CurrentValue, DbId) + "')";
                filterChk = filterChk + " AND NOT (" + filter + ")";
                try {
                    using var rschk = await LoadReader(filterChk);
                    if (rschk?.HasRows ?? false) {
                        var idxErrMsg = Language.Phrase("DupIndex").Replace("%f", Code.Caption);
                        idxErrMsg = idxErrMsg.Replace("%v", ConvertToString(Code.CurrentValue));
