...
public partial class PCM {
    ...
    /// mtClothesSizeEdit
    /// </summary>
    public static MtClothesSizeEdit mtClothesSizeEdit
    {
        get => HttpData.Get<MtClothesSizeEdit>("mtClothesSizeEdit")!;
        set => HttpData["mtClothesSizeEdit"] = value;
    }

    /// <summary>
    /// Page class for MTClothesSize
    /// </summary>
    public class MtClothesSizeEdit : MtClothesSizeEditBase
    {
        // Constructor
        public MtClothesSizeEdit(Controller controller) : base(controller)
        {
        }

        // Constructor
        public MtClothesSizeEdit() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class MtClothesSizeEditBase : MtClothesSize
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "mtClothesSizeEdit";
        ...
        // Page name
        public string PageName => "MtClothesSizeEdit";
        ...
        // Constructor
        public MtClothesSizeEditBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        switch (CurrentAction) {
                case "show": // Get a record to display
                        if (!loaded) { // Load record based on key
                            ...
                            return Terminate("MtClothesSizeList"); // No matching record, return to list
                        }
                    break;
                case "update": // Update // DN
                    CloseRecordset(); // DN
                    string returnUrl = ReturnUrl;
                    if (GetPageName(returnUrl) == "MtClothesSizeList")
                    ...
                    if (res) {
                        ...
                        if (IsModal && UseAjaxActions) {
                            ...
                            if (GetPageName(returnUrl) != "MtClothesSizeList") {
                                ...
                                returnUrl = "MtClothesSizeList"; // Return list page content
                            }
                        }
        ...
        protected async Task LoadFormValues() {
            ...
            // Check field name 'CoverallSizeDescription' before field var 'x_CoverallSizeDescription'
            val = CurrentForm.HasValue("CoverallSizeDescription") ? CurrentForm.GetValue("CoverallSizeDescription") : CurrentForm.GetValue("x_CoverallSizeDescription");
            if (!CoverallSizeDescription.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("CoverallSizeDescription") && !CurrentForm.HasValue("x_CoverallSizeDescription")) // DN
                    CoverallSizeDescription.Visible = false; // Disable update for API request
                else
                    CoverallSizeDescription.SetFormValue(val);
            }

            // Check field name 'ShirtSizeDescription' before field var 'x_ShirtSizeDescription'
            val = CurrentForm.HasValue("ShirtSizeDescription") ? CurrentForm.GetValue("ShirtSizeDescription") : CurrentForm.GetValue("x_ShirtSizeDescription");
            if (!ShirtSizeDescription.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("ShirtSizeDescription") && !CurrentForm.HasValue("x_ShirtSizeDescription")) // DN
                    ShirtSizeDescription.Visible = false; // Disable update for API request
                else
                    ShirtSizeDescription.SetFormValue(val);
            }

            // Check field name 'TrouserSizeDescription' before field var 'x_TrouserSizeDescription'
            val = CurrentForm.HasValue("TrouserSizeDescription") ? CurrentForm.GetValue("TrouserSizeDescription") : CurrentForm.GetValue("x_TrouserSizeDescription");
            if (!TrouserSizeDescription.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("TrouserSizeDescription") && !CurrentForm.HasValue("x_TrouserSizeDescription")) // DN
                    TrouserSizeDescription.Visible = false; // Disable update for API request
                else
                    TrouserSizeDescription.SetFormValue(val);
            }
        ...
        public void RestoreFormValues()
        {
            ...
            CoverallSizeDescription.CurrentValue = CoverallSizeDescription.FormValue;
            ShirtSizeDescription.CurrentValue = ShirtSizeDescription.FormValue;
            TrouserSizeDescription.CurrentValue = TrouserSizeDescription.FormValue;
        }
        ...
        public async Task RenderRow()
        {
            ...
            CoverallSizeDescription.RowCssClass = "row";

            // ShirtSizeDescription
            ShirtSizeDescription.RowCssClass = "row";
        ...
        protected async Task<bool> ValidateForm() {
            ...
            if (CoverallSizeDescription.Required) {
                if (!CoverallSizeDescription.IsDetailKey && Empty(CoverallSizeDescription.FormValue)) {
                    CoverallSizeDescription.AddErrorMessage(ConvertToString(CoverallSizeDescription.RequiredErrorMessage).Replace("%s", CoverallSizeDescription.Caption));
                }
            }
            if (ShirtSizeDescription.Required) {
                if (!ShirtSizeDescription.IsDetailKey && Empty(ShirtSizeDescription.FormValue)) {
                    ShirtSizeDescription.AddErrorMessage(ConvertToString(ShirtSizeDescription.RequiredErrorMessage).Replace("%s", ShirtSizeDescription.Caption));
                }
            }
            if (TrouserSizeDescription.Required) {
                if (!TrouserSizeDescription.IsDetailKey && Empty(TrouserSizeDescription.FormValue)) {
                    TrouserSizeDescription.AddErrorMessage(ConvertToString(TrouserSizeDescription.RequiredErrorMessage).Replace("%s", TrouserSizeDescription.Caption));
                }
            }
        ...
        protected async Task<JsonBoolResult> EditRow() { // DN
            ...
            // CoverallSizeDescription
            CoverallSizeDescription.SetDbValue(rsnew, CoverallSizeDescription.CurrentValue, CoverallSizeDescription.ReadOnly);

            // ShirtSizeDescription
            ShirtSizeDescription.SetDbValue(rsnew, ShirtSizeDescription.CurrentValue, ShirtSizeDescription.ReadOnly);

            // TrouserSizeDescription
            TrouserSizeDescription.SetDbValue(rsnew, TrouserSizeDescription.CurrentValue, TrouserSizeDescription.ReadOnly);
        ...
        protected void SetupBreadcrumb() {
            ...
            breadcrumb.Add("list", TableVar, AppPath(AddMasterUrl("MtClothesSizeList")), "", TableVar, true);
            string pageId = "edit";
