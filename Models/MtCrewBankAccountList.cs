...
public partial class PCM {
    /// <summary>
    /// mtCrewBankAccountList
    /// </summary>
    public static MtCrewBankAccountList mtCrewBankAccountList
    {
        get => HttpData.Get<MtCrewBankAccountList>("mtCrewBankAccountList")!;
        set => HttpData["mtCrewBankAccountList"] = value;
    }

    /// <summary>
    /// Page class for MTCrewBankAccount
    /// </summary>
    public class MtCrewBankAccountList : MtCrewBankAccountListBase
    {
        // Constructor
        public MtCrewBankAccountList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public MtCrewBankAccountList() : base()
        {
        }
    ...
    /// Page base class
    /// </summary>
    public class MtCrewBankAccountListBase : MtCrewBankAccount
    {
        // Page ID
        ...
        // Table name
        public string TableName { get; set; } = "MTCrewBankAccount";

        // Page object name
        public string PageObjName = "mtCrewBankAccountList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fMTCrewBankAccountlist";
        ...
        // Constructor
        public MtCrewBankAccountListBase()
        {
            // CSS class name as context
            TableVar = "MTCrewBankAccount";
            ...
            // Table object (mtCrewBankAccount)
            if (mtCrewBankAccount == null || mtCrewBankAccount is MtCrewBankAccount)
                mtCrewBankAccount = this;

            // Initialize URLs
            AddUrl = "MtCrewBankAccountAdd";
            ...
            MultiDeleteUrl = "MtCrewBankAccountDelete";
            MultiUpdateUrl = "MtCrewBankAccountUpdate";
        ...
        // Page name
        public string PageName => "MtCrewBankAccountList";
        ...
        // Constructor
        public MtCrewBankAccountListBase(Controller? controller = null): this() { // DN
            if (controller != null)
                ...
                if (Response != null && !Response.HasStarted) {
                    ...
                    if (IsModal) { // Show as modal
                        ...
                        if (pageName != ListUrl) { // Not List page
                            ...
                            result.Add("view", pageName == "MtCrewBankAccountView" ? "1" : "0"); // If View page, no primary button
                        } else { // List page
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (IsModal)
                FormName = "fMTCrewBankAccountgrid";
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                mtCrewBankAccountList?.PageRender();
            }
            return PageResult();
        }
        #pragma warning restore 219
        ...
        public string AdvancedSearchWhere(bool def = false) {
            ...
            BuildSearchSql(ref where, BankName, def, true); // BankName
            BuildSearchSql(ref where, MTBankID, def, false); // MTBankID
            BuildSearchSql(ref where, OtherBankName, def, false); // OtherBankName
            BuildSearchSql(ref where, AccountNumber, def, true); // AccountNumber
            BuildSearchSql(ref where, Beneficiary, def, true); // Beneficiary
            BuildSearchSql(ref where, MTCurrencyID, def, true); // MTCurrencyID
            BuildSearchSql(ref where, MainAcc, def, true); // MainAcc
        ...
        public void ShowFilterList()
        {
            ...
            if (Empty(filter))
                BuildSearchSql(ref filter, AccountNumber, false, true);
            if (!Empty(filter))
            ...
            if (Empty(filter))
                BuildSearchSql(ref filter, Beneficiary, false, true);
            if (!Empty(filter))
            ...
            if (Empty(filter))
                BuildSearchSql(ref filter, MainAcc, false, true);
            if (!Empty(filter))
            ...
            if (listOption != null && !IsExport() && CurrentAction == "") {
                ...
                foreach (var (key, act) in ListActions.Items) {
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        string link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fMTCrewBankAccountlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + " " + caption + "</button></li>";
                        if (!Empty(link)) {
                            links.Add(link);
                            if (Empty(body)) // Setup first button
                                body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fMTCrewBankAccountlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + caption + "</button>";
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fMTCrewBankAccountsrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fMTCrewBankAccountsrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fMTCrewBankAccountlist\"" + act.ToDataAttrs() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_MTCrewBankAccount");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(mtCrewBankAccountList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(mtCrewBankAccountList.RowCount) + "_MTCrewBankAccount");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(mtCrewBankAccountList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && mtCrewBankAccountList.RowType == RowType.Add || IsEdit && mtCrewBankAccountList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                // AccountNumber
                if (AccountNumber.UseFilter && !Empty(AccountNumber.AdvancedSearch.SearchValue)) {
                    AccountNumber.EditValue = ConvertToString(AccountNumber.AdvancedSearch.SearchValue).Split(Config.MultipleOptionSeparator).ToList();
                }

                // Beneficiary
                if (Beneficiary.UseFilter && !Empty(Beneficiary.AdvancedSearch.SearchValue)) {
                    Beneficiary.EditValue = ConvertToString(Beneficiary.AdvancedSearch.SearchValue).Split(Config.MultipleOptionSeparator).ToList();
                }
                ...
                // MainAcc
                if (MainAcc.UseFilter && !Empty(MainAcc.AdvancedSearch.SearchValue)) {
                    MainAcc.EditValue = ConvertToString(MainAcc.AdvancedSearch.SearchValue).Split(Config.MultipleOptionSeparator).ToList();
                }
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fMTCrewBankAccountsrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
            ...
            if (ModalSearch && !IsMobile())
                item.Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-table=\"MTCrewBankAccount\" data-caption=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-ew-action=\"modal\" data-url=\"" + AppPath("MtCrewBankAccountSearch") + "\" data-btn=\"SearchBtn\">" + Language.Phrase("AdvancedSearch", false) + "</a>";
            else
                item.Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-caption=\"" + Language.Phrase("AdvancedSearch", true) + "\" href=\"" + AppPath("MtCrewBankAccountSearch") + "\">" + Language.Phrase("AdvancedSearch", false) + "</a>";
            item.Visible = true;
