...
public partial class PCM {
    ...
    /// mtVesselTypeEdit
    /// </summary>
    public static MtVesselTypeEdit mtVesselTypeEdit
    {
        get => HttpData.Get<MtVesselTypeEdit>("mtVesselTypeEdit")!;
        set => HttpData["mtVesselTypeEdit"] = value;
    }

    /// <summary>
    /// Page class for MTVesselType
    /// </summary>
    public class MtVesselTypeEdit : MtVesselTypeEditBase
    {
        // Constructor
        public MtVesselTypeEdit(Controller controller) : base(controller)
        {
        }

        // Constructor
        public MtVesselTypeEdit() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class MtVesselTypeEditBase : MtVesselType
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "mtVesselTypeEdit";
        ...
        // Page name
        public string PageName => "MtVesselTypeEdit";
        ...
        // Constructor
        public MtVesselTypeEditBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        switch (CurrentAction) {
                case "show": // Get a record to display
                        if (!loaded) { // Load record based on key
                            ...
                            return Terminate("MtVesselTypeList"); // No matching record, return to list
                        }
                    break;
                case "update": // Update // DN
                    CloseRecordset(); // DN
                    string returnUrl = ReturnUrl;
                    if (GetPageName(returnUrl) == "MtVesselTypeList")
                    ...
                    if (res) {
                        ...
                        if (IsModal && UseAjaxActions) {
                            ...
                            if (GetPageName(returnUrl) != "MtVesselTypeList") {
                                ...
                                returnUrl = "MtVesselTypeList"; // Return list page content
                            }
                        }
        ...
        protected async Task LoadFormValues() {
            ...
            // Check field name 'Abbreviation' before field var 'x_Abbreviation'
            val = CurrentForm.HasValue("Abbreviation") ? CurrentForm.GetValue("Abbreviation") : CurrentForm.GetValue("x_Abbreviation");
            if (!Abbreviation.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("Abbreviation") && !CurrentForm.HasValue("x_Abbreviation")) // DN
                    Abbreviation.Visible = false; // Disable update for API request
                else
                    Abbreviation.SetFormValue(val);
            }
        ...
        public void RestoreFormValues()
        {
            ...
            Abbreviation.CurrentValue = Abbreviation.FormValue;
        ...
        protected async Task<bool> ValidateForm() {
            ...
            if (Abbreviation.Required) {
                if (!Abbreviation.IsDetailKey && Empty(Abbreviation.FormValue)) {
                    Abbreviation.AddErrorMessage(ConvertToString(Abbreviation.RequiredErrorMessage).Replace("%s", Abbreviation.Caption));
                }
            }
            if (_Name.Required) {
        ...
        protected async Task<JsonBoolResult> EditRow() { // DN
            ...
            // Abbreviation
            Abbreviation.SetDbValue(rsnew, Abbreviation.CurrentValue, Abbreviation.ReadOnly);
            ...
            // Check field with unique index (Abbreviation)
            if (!Empty(Abbreviation.CurrentValue)) {
                string filterChk = "([Abbreviation] = '" + AdjustSql(Abbreviation.CurrentValue, DbId) + "')";
                filterChk = filterChk + " AND NOT (" + filter + ")";
                try {
                    using var rschk = await LoadReader(filterChk);
                    if (rschk?.HasRows ?? false) {
                        var idxErrMsg = Language.Phrase("DupIndex").Replace("%f", Abbreviation.Caption);
                        idxErrMsg = idxErrMsg.Replace("%v", ConvertToString(Abbreviation.CurrentValue));
        ...
        protected void SetupBreadcrumb() {
            ...
            breadcrumb.Add("list", TableVar, AppPath(AddMasterUrl("MtVesselTypeList")), "", TableVar, true);
            string pageId = "edit";
