...
public partial class PCM {
    /// <summary>
    /// trEmployeeStatusList
    /// </summary>
    public static TrEmployeeStatusList trEmployeeStatusList
    {
        get => HttpData.Get<TrEmployeeStatusList>("trEmployeeStatusList")!;
        set => HttpData["trEmployeeStatusList"] = value;
    }

    /// <summary>
    /// Page class for TREmployeeStatus
    /// </summary>
    public class TrEmployeeStatusList : TrEmployeeStatusListBase
    {
        // Constructor
        public TrEmployeeStatusList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public TrEmployeeStatusList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class TrEmployeeStatusListBase : TrEmployeeStatus
    {
        // Page ID
        ...
        // Table name
        public string TableName { get; set; } = "TREmployeeStatus";

        // Page object name
        public string PageObjName = "trEmployeeStatusList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fTREmployeeStatuslist";
        ...
        // Constructor
        public TrEmployeeStatusListBase()
        {
            // CSS class name as context
            TableVar = "TREmployeeStatus";
            ...
            // Table object (trEmployeeStatus)
            if (trEmployeeStatus == null || trEmployeeStatus is TrEmployeeStatus)
                trEmployeeStatus = this;

            // Initialize URLs
            AddUrl = "TrEmployeeStatusAdd";
            ...
            MultiDeleteUrl = "TrEmployeeStatusDelete";
            MultiUpdateUrl = "TrEmployeeStatusUpdate";
        ...
        // Page name
        public string PageName => "TrEmployeeStatusList";
        ...
        public void SetVisibility()
        {
            ...
            PreviousStatus.SetVisibility();
            CurrentStatus.SetVisibility();
            ChangedDateTime.SetVisibility();
        }

        // Constructor
        public TrEmployeeStatusListBase(Controller? controller = null): this() { // DN
            if (controller != null)
                ...
                if (Response != null && !Response.HasStarted) {
                    ...
                    if (IsModal) { // Show as modal
                        ...
                        if (pageName != ListUrl) { // Not List page
                            ...
                            result.Add("view", pageName == "TrEmployeeStatusView" ? "1" : "0"); // If View page, no primary button
                        } else { // List page
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (IsModal)
                FormName = "fTREmployeeStatusgrid";
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                trEmployeeStatusList?.PageRender();
            }
            return PageResult();
        }
        #pragma warning restore 219
        ...
        public async Task<string> GetFilterList()
        {
            ...
            filters.Merge(JObject.Parse(MTUserID.AdvancedSearch.ToJson())); // Field MTUserID
            filters.Merge(JObject.Parse(PreviousStatus.AdvancedSearch.ToJson())); // Field PreviousStatus
            filters.Merge(JObject.Parse(CurrentStatus.AdvancedSearch.ToJson())); // Field CurrentStatus
            filters.Merge(JObject.Parse(ChangedDateTime.AdvancedSearch.ToJson())); // Field ChangedDateTime
        ...
        protected bool RestoreFilterList() {
            ...
            // Field MTUserID
            if (filter?.TryGetValue("x_MTUserID", out sv) ?? false) {
                MTUserID.AdvancedSearch.SearchValue = sv;
                MTUserID.AdvancedSearch.SearchOperator = filter["z_MTUserID"];
                MTUserID.AdvancedSearch.SearchCondition = filter["v_MTUserID"];
                MTUserID.AdvancedSearch.SearchValue2 = filter["y_MTUserID"];
                MTUserID.AdvancedSearch.SearchOperator2 = filter["w_MTUserID"];
                MTUserID.AdvancedSearch.Save();
            }

            // Field PreviousStatus
            if (filter?.TryGetValue("x_PreviousStatus", out sv) ?? false) {
                PreviousStatus.AdvancedSearch.SearchValue = sv;
                PreviousStatus.AdvancedSearch.SearchOperator = filter["z_PreviousStatus"];
                PreviousStatus.AdvancedSearch.SearchCondition = filter["v_PreviousStatus"];
                PreviousStatus.AdvancedSearch.SearchValue2 = filter["y_PreviousStatus"];
                PreviousStatus.AdvancedSearch.SearchOperator2 = filter["w_PreviousStatus"];
                PreviousStatus.AdvancedSearch.Save();
            }

            // Field CurrentStatus
            if (filter?.TryGetValue("x_CurrentStatus", out sv) ?? false) {
                CurrentStatus.AdvancedSearch.SearchValue = sv;
                CurrentStatus.AdvancedSearch.SearchOperator = filter["z_CurrentStatus"];
                CurrentStatus.AdvancedSearch.SearchCondition = filter["v_CurrentStatus"];
                CurrentStatus.AdvancedSearch.SearchValue2 = filter["y_CurrentStatus"];
                CurrentStatus.AdvancedSearch.SearchOperator2 = filter["w_CurrentStatus"];
                CurrentStatus.AdvancedSearch.Save();
            }

            // Field ChangedDateTime
            if (filter?.TryGetValue("x_ChangedDateTime", out sv) ?? false) {
                ChangedDateTime.AdvancedSearch.SearchValue = sv;
                ChangedDateTime.AdvancedSearch.SearchOperator = filter["z_ChangedDateTime"];
                ChangedDateTime.AdvancedSearch.SearchCondition = filter["v_ChangedDateTime"];
                ChangedDateTime.AdvancedSearch.SearchValue2 = filter["y_ChangedDateTime"];
                ChangedDateTime.AdvancedSearch.SearchOperator2 = filter["w_ChangedDateTime"];
                ChangedDateTime.AdvancedSearch.Save();
            }
        ...
        public string AdvancedSearchWhere(bool def = false) {
            ...
            BuildSearchSql(ref where, MTUserID, def, true); // MTUserID
            BuildSearchSql(ref where, PreviousStatus, def, true); // PreviousStatus
            BuildSearchSql(ref where, CurrentStatus, def, true); // CurrentStatus
            BuildSearchSql(ref where, ChangedDateTime, def, true); // ChangedDateTime
            ...
            if (!def && Command == "search") {
                ...
                MTUserID.AdvancedSearch.Save(); // MTUserID
                PreviousStatus.AdvancedSearch.Save(); // PreviousStatus
                CurrentStatus.AdvancedSearch.Save(); // CurrentStatus
                ChangedDateTime.AdvancedSearch.Save(); // ChangedDateTime
        ...
        public void ShowFilterList()
        {
            ...
            // Field MTUserID
            filter = QueryBuilderWhere("MTUserID");
            if (Empty(filter))
                BuildSearchSql(ref filter, MTUserID, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + MTUserID.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field PreviousStatus
            filter = QueryBuilderWhere("PreviousStatus");
            if (Empty(filter))
                BuildSearchSql(ref filter, PreviousStatus, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + PreviousStatus.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field CurrentStatus
            filter = QueryBuilderWhere("CurrentStatus");
            if (Empty(filter))
                BuildSearchSql(ref filter, CurrentStatus, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + CurrentStatus.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field ChangedDateTime
            filter = QueryBuilderWhere("ChangedDateTime");
            if (Empty(filter))
                BuildSearchSql(ref filter, ChangedDateTime, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + ChangedDateTime.Caption + "</span>" + captionSuffix + filter + "</div>";
            if (!Empty(BasicSearch.Keyword))
        ...
        public string BasicSearchWhere(bool def = false) {
            ...
            searchFlds.Add(PreviousStatus);
            searchFlds.Add(CurrentStatus);
        ...
        protected bool CheckSearchParms() {
            ...
            if (MTCrewID.AdvancedSearch.IssetSession)
                return true;
            if (MTUserID.AdvancedSearch.IssetSession)
                return true;
            if (PreviousStatus.AdvancedSearch.IssetSession)
                return true;
            if (CurrentStatus.AdvancedSearch.IssetSession)
                return true;
            if (ChangedDateTime.AdvancedSearch.IssetSession)
                return true;
            return false;
        }
        ...
        protected void ResetAdvancedSearchParms() {
            ...
            MTUserID.AdvancedSearch.UnsetSession();
            PreviousStatus.AdvancedSearch.UnsetSession();
            CurrentStatus.AdvancedSearch.UnsetSession();
            ChangedDateTime.AdvancedSearch.UnsetSession();
        }
        ...
        protected void RestoreSearchParms() {
            ...
            MTUserID.AdvancedSearch.Load();
            PreviousStatus.AdvancedSearch.Load();
            CurrentStatus.AdvancedSearch.Load();
            ChangedDateTime.AdvancedSearch.Load();
        }
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(MTUserID, ctrl); // MTUserID
                UpdateSort(PreviousStatus, ctrl); // PreviousStatus
                UpdateSort(CurrentStatus, ctrl); // CurrentStatus
                UpdateSort(ChangedDateTime, ctrl); // ChangedDateTime
            ...
            if (listOption != null && !IsExport() && CurrentAction == "") {
                ...
                foreach (var (key, act) in ListActions.Items) {
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        string link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fTREmployeeStatuslist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + " " + caption + "</button></li>";
                        if (!Empty(link)) {
                            links.Add(link);
                            if (Empty(body)) // Setup first button
                                body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fTREmployeeStatuslist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + caption + "</button>";
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("MTUserID")); // DN
                CreateColumnOption(option.Add("PreviousStatus")); // DN
                CreateColumnOption(option.Add("CurrentStatus")); // DN
                CreateColumnOption(option.Add("ChangedDateTime")); // DN
            }
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fTREmployeeStatussrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fTREmployeeStatussrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fTREmployeeStatuslist\"" + act.ToDataAttrs() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_TREmployeeStatus");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(trEmployeeStatusList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(trEmployeeStatusList.RowCount) + "_TREmployeeStatus");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(trEmployeeStatusList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && trEmployeeStatusList.RowType == RowType.Add || IsEdit && trEmployeeStatusList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        protected void LoadSearchValues() {
            ...
            // MTUserID
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_MTUserID[]"))
                    MTUserID.AdvancedSearch.SearchValue = Get("x_MTUserID[]");
                else
                    MTUserID.AdvancedSearch.SearchValue = Get("MTUserID"); // Default Value // DN
            if (!Empty(MTUserID.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_MTUserID"))
                MTUserID.AdvancedSearch.SearchOperator = Get("z_MTUserID");

            // PreviousStatus
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_PreviousStatus[]"))
                    PreviousStatus.AdvancedSearch.SearchValue = Get("x_PreviousStatus[]");
                else
                    PreviousStatus.AdvancedSearch.SearchValue = Get("PreviousStatus"); // Default Value // DN
            if (!Empty(PreviousStatus.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_PreviousStatus"))
                PreviousStatus.AdvancedSearch.SearchOperator = Get("z_PreviousStatus");

            // CurrentStatus
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_CurrentStatus[]"))
                    CurrentStatus.AdvancedSearch.SearchValue = Get("x_CurrentStatus[]");
                else
                    CurrentStatus.AdvancedSearch.SearchValue = Get("CurrentStatus"); // Default Value // DN
            if (!Empty(CurrentStatus.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_CurrentStatus"))
                CurrentStatus.AdvancedSearch.SearchOperator = Get("z_CurrentStatus");

            // ChangedDateTime
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_ChangedDateTime[]"))
                    ChangedDateTime.AdvancedSearch.SearchValue = Get("x_ChangedDateTime[]");
                else
                    ChangedDateTime.AdvancedSearch.SearchValue = Get("ChangedDateTime"); // Default Value // DN
            if (!Empty(ChangedDateTime.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_ChangedDateTime"))
                ChangedDateTime.AdvancedSearch.SearchOperator = Get("z_ChangedDateTime");
        }
        ...
        public async Task LoadRowValues(Dictionary<string, object>? row)
        {
            ...
            PreviousStatus.SetDbValue(row["PreviousStatus"]);
            CurrentStatus.SetDbValue(row["CurrentStatus"]);
            ChangedDateTime.SetDbValue(row["ChangedDateTime"]);
        }
        ...
        protected Dictionary<string, object> NewRow() {
            ...
            row.Add("PreviousStatus", PreviousStatus.DefaultValue ?? DbNullValue); // DN
            row.Add("CurrentStatus", CurrentStatus.DefaultValue ?? DbNullValue); // DN
            row.Add("ChangedDateTime", ChangedDateTime.DefaultValue ?? DbNullValue); // DN
            return row;
        }
        ...
        public async Task RenderRow()
        {
            ...
            // PreviousStatus
            PreviousStatus.CellCssStyle = "white-space: nowrap;";

            // CurrentStatus
            CurrentStatus.CellCssStyle = "white-space: nowrap;";

            // ChangedDateTime
            ChangedDateTime.CellCssStyle = "white-space: nowrap;";
            ...
            if (RowType == RowType.View) {
                ...
                // PreviousStatus
                PreviousStatus.ViewValue = ConvertToString(PreviousStatus.CurrentValue); // DN
                PreviousStatus.ViewCustomAttributes = "";

                // CurrentStatus
                CurrentStatus.ViewValue = ConvertToString(CurrentStatus.CurrentValue); // DN
                CurrentStatus.ViewCustomAttributes = "";

                // ChangedDateTime
                ChangedDateTime.ViewValue = ChangedDateTime.CurrentValue;
                ChangedDateTime.ViewValue = FormatDateTime(ChangedDateTime.ViewValue, ChangedDateTime.FormatPattern);
                ChangedDateTime.ViewCustomAttributes = "";
                ...
                // PreviousStatus
                PreviousStatus.HrefValue = "";
                PreviousStatus.TooltipValue = "";

                // CurrentStatus
                CurrentStatus.HrefValue = "";
                CurrentStatus.TooltipValue = "";

                // ChangedDateTime
                ChangedDateTime.HrefValue = "";
                ChangedDateTime.TooltipValue = "";
                ...
                // MTUserID
                if (MTUserID.UseFilter && !Empty(MTUserID.AdvancedSearch.SearchValue)) {
                    MTUserID.EditValue = ConvertToString(MTUserID.AdvancedSearch.SearchValue).Split(Config.MultipleOptionSeparator).ToList();
                }

                // PreviousStatus
                if (PreviousStatus.UseFilter && !Empty(PreviousStatus.AdvancedSearch.SearchValue)) {
                    PreviousStatus.EditValue = ConvertToString(PreviousStatus.AdvancedSearch.SearchValue).Split(Config.MultipleOptionSeparator).ToList();
                }

                // CurrentStatus
                if (CurrentStatus.UseFilter && !Empty(CurrentStatus.AdvancedSearch.SearchValue)) {
                    CurrentStatus.EditValue = ConvertToString(CurrentStatus.AdvancedSearch.SearchValue).Split(Config.MultipleOptionSeparator).ToList();
                }

                // ChangedDateTime
                if (ChangedDateTime.UseFilter && !Empty(ChangedDateTime.AdvancedSearch.SearchValue)) {
                    ChangedDateTime.EditValue = ConvertToString(ChangedDateTime.AdvancedSearch.SearchValue).Split(Config.MultipleOptionSeparator).ToList();
                }
            }
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fTREmployeeStatussrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
            ...
            if (ModalSearch && !IsMobile())
                item.Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-table=\"TREmployeeStatus\" data-caption=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-ew-action=\"modal\" data-url=\"" + AppPath("TrEmployeeStatusSearch") + "\" data-btn=\"SearchBtn\">" + Language.Phrase("AdvancedSearch", false) + "</a>";
            else
                item.Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-caption=\"" + Language.Phrase("AdvancedSearch", true) + "\" href=\"" + AppPath("TrEmployeeStatusSearch") + "\">" + Language.Phrase("AdvancedSearch", false) + "</a>";
            item.Visible = true;
