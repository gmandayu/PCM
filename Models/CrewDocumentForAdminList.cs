...
public partial class PCM {
    /// <summary>
    /// crewDocumentForAdminList
    /// </summary>
    public static CrewDocumentForAdminList crewDocumentForAdminList
    {
        get => HttpData.Get<CrewDocumentForAdminList>("crewDocumentForAdminList")!;
        set => HttpData["crewDocumentForAdminList"] = value;
    }

    /// <summary>
    /// Page class for CrewDocumentForAdmin
    /// </summary>
    public class CrewDocumentForAdminList : CrewDocumentForAdminListBase
    {
        // Constructor
        public CrewDocumentForAdminList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public CrewDocumentForAdminList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class CrewDocumentForAdminListBase : CrewDocumentForAdmin
    {
        // Page ID
        ...
        // Table name
        public string TableName { get; set; } = "CrewDocumentForAdmin";

        // Page object name
        public string PageObjName = "crewDocumentForAdminList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fCrewDocumentForAdminlist";
        ...
        // Constructor
        public CrewDocumentForAdminListBase()
        {
            ...
            // Table object (crewDocumentForAdmin)
            if (crewDocumentForAdmin == null || crewDocumentForAdmin is CrewDocumentForAdmin)
                crewDocumentForAdmin = this;

            // Initialize URLs
            AddUrl = "CrewDocumentForAdminAdd";
            ...
            MultiDeleteUrl = "CrewDocumentForAdminDelete";
            MultiUpdateUrl = "CrewDocumentForAdminUpdate";
        ...
        // Page name
        public string PageName => "CrewDocumentForAdminList";
        ...
        // Constructor
        public CrewDocumentForAdminListBase(Controller? controller = null): this() { // DN
            if (controller != null)
                ...
                if (Response != null && !Response.HasStarted) {
                    ...
                    if (IsModal) { // Show as modal
                        ...
                        if (pageName != ListUrl) { // Not List page
                            ...
                            result.Add("view", pageName == "CrewDocumentForAdminView" ? "1" : "0"); // If View page, no primary button
                        } else { // List page
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (IsModal)
                FormName = "fCrewDocumentForAdmingrid";
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                crewDocumentForAdminList?.PageRender();
            }
            return PageResult();
        }
        #pragma warning restore 219
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""CrewDocumentForAdmin"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
            ...
            if (listOption != null && !IsExport() && CurrentAction == "") {
                ...
                foreach (var (key, act) in ListActions.Items) {
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        string link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fCrewDocumentForAdminlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + " " + caption + "</button></li>";
                        if (!Empty(link)) {
                            links.Add(link);
                            if (Empty(body)) // Setup first button
                                body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fCrewDocumentForAdminlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + caption + "</button>";
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""CrewDocumentForAdmin"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fCrewDocumentForAdminsrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fCrewDocumentForAdminsrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fCrewDocumentForAdminlist\"" + act.ToDataAttrs() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_CrewDocumentForAdmin");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(crewDocumentForAdminList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(crewDocumentForAdminList.RowCount) + "_CrewDocumentForAdmin");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(crewDocumentForAdminList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && crewDocumentForAdminList.RowType == RowType.Add || IsEdit && crewDocumentForAdminList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                if (Image.UseColorbox) {
                    ...
                    Image.LinkAttrs["data-rel"] = "CrewDocumentForAdmin_x" + RowCount + "_Image";
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fCrewDocumentForAdminsrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
