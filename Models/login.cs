...
public partial class PCM {
    ...
    public class Login : LoginBase
    {
        ...
        public override void UserLoggedIn(string usr) { // usr contains user email address that is used to login
            if (CurrentUserLevel() != "-1")
            { // if user other than admin login
                //Session[Config.SessionStatus] = "login"; // set login status in session    
                ...
                dynamic sameEmailResult = QueryBuilder("MTUser").Where("Email", usr).Select("Email").FirstOrDefault();
                if ((object)sameEmailResult == null)
                { // there is no existing email address in db
                    // insert email and name to db
                    int rowAffected = ExecuteInsert("MTUser", new
                    {
                        Email = usr,
                        Password = Config.EncryptedPassword ? EncryptPassword(pass) : pass
                    });
                    // check insert status
                    if (rowAffected == 1)
                    {
                        ...
                        model.Username = usr;
                        model.Password = pass;
                        var task = Task.Run(async () => await Security.ValidateUser(model, true));
                        task.Wait();
                        var rs = ExecuteRow("SELECT TOP 1 ID,EmployeeStatus FROM MTCREW WHERE MTUserID= " + CurrentUserID().ToString() + " ORDER BY ID DESC");
                        if (rs == null)
                        {
                            ...
                            dynamic crewQueryResult = QueryBuilder("MTCrew").Where("MTUserID", CurrentUserID().ToString()).Select("ID").FirstOrDefault();
                    ...
                    {
                        Log("Insert user failed when login with google");
                    }
                }
            }
        }
    ...
    public class LoginBase : MtUser
    {
        ...
        public DbField<SqlDbType> LoginUsername = new ("MTUser", "username", 202, SqlDbType.NVarChar) {
                TableName = "MTUser",
                Name = "username",
                Expression = "username"
            }; // DN

        public DbField<SqlDbType> LoginPassword = new ("MTUser", "password", 202, SqlDbType.NVarChar) {
                TableName = "MTUser",
                Name = "password",
                Expression = "password"
            }; // DN

        public DbField<SqlDbType> LoginType = new ("MTUser", "type", 202, SqlDbType.NVarChar) {
                TableName = "MTUser",
                Name = "logintype",
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (IsLoggingIn()) { // After changing password
                ...
                provider = TitleCase(provider.Trim());
                if (User != null) { // Set up claims in profile
                    Profile.Assign(User.Claims.ToDictionary(claim => claim.Type, claim => claim.Value));
                    Profile.Assign(User.Claims.Where(claim => claim.Type.StartsWith("http://")).ToDictionary(claim => claim.Type.Split('/').Last(), claim => claim.Value));
                }
