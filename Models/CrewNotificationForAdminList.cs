...
public partial class PCM {
    /// <summary>
    /// crewNotificationForAdminList
    /// </summary>
    public static CrewNotificationForAdminList crewNotificationForAdminList
    {
        get => HttpData.Get<CrewNotificationForAdminList>("crewNotificationForAdminList")!;
        set => HttpData["crewNotificationForAdminList"] = value;
    }

    /// <summary>
    /// Page class for CrewNotificationForAdmin
    /// </summary>
    public class CrewNotificationForAdminList : CrewNotificationForAdminListBase
    {
        // Constructor
        public CrewNotificationForAdminList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public CrewNotificationForAdminList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class CrewNotificationForAdminListBase : CrewNotificationForAdmin
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "crewNotificationForAdminList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fCrewNotificationForAdminlist";
        ...
        // Constructor
        public CrewNotificationForAdminListBase()
        {
            ...
            // Initialize URLs
            AddUrl = "CrewNotificationForAdminAdd";
            ...
            MultiDeleteUrl = "CrewNotificationForAdminDelete";
            MultiUpdateUrl = "CrewNotificationForAdminUpdate";
        ...
        // Page name
        public string PageName => "CrewNotificationForAdminList";
        ...
        // Constructor
        public CrewNotificationForAdminListBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (IsModal)
                FormName = "fCrewNotificationForAdmingrid";
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                crewNotificationForAdminList?.PageRender();
            }
            return PageResult();
        }
        #pragma warning restore 219
            ...
            if (listOption != null && !IsExport() && CurrentAction == "") {
                ...
                foreach (var (key, act) in ListActions.Items) {
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        string link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fCrewNotificationForAdminlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + " " + caption + "</button></li>";
                        if (!Empty(link)) {
                            links.Add(link);
                            if (Empty(body)) // Setup first button
                                body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fCrewNotificationForAdminlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + caption + "</button>";
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""CrewNotificationForAdmin"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fCrewNotificationForAdminsrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = false;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fCrewNotificationForAdminsrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = false;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fCrewNotificationForAdminlist\"" + act.ToDataAttrs() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_CrewNotificationForAdmin");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(crewNotificationForAdminList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(crewNotificationForAdminList.RowCount) + "_CrewNotificationForAdmin");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(crewNotificationForAdminList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && crewNotificationForAdminList.RowType == RowType.Add || IsEdit && crewNotificationForAdminList.RowType == RowType.Edit) // Inline-Add/Edit row
