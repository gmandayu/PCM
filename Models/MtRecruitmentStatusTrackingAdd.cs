...
public partial class PCM {
    /// <summary>
    /// mtRecruitmentStatusTrackingAdd
    /// </summary>
    public static MtRecruitmentStatusTrackingAdd mtRecruitmentStatusTrackingAdd
    {
        get => HttpData.Get<MtRecruitmentStatusTrackingAdd>("mtRecruitmentStatusTrackingAdd")!;
        set => HttpData["mtRecruitmentStatusTrackingAdd"] = value;
    }

    /// <summary>
    /// Page class for MTRecruitmentStatusTracking
    /// </summary>
    public class MtRecruitmentStatusTrackingAdd : MtRecruitmentStatusTrackingAddBase
    {
        // Constructor
        public MtRecruitmentStatusTrackingAdd(Controller controller) : base(controller)
        {
        }

        // Constructor
        public MtRecruitmentStatusTrackingAdd() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class MtRecruitmentStatusTrackingAddBase : MtRecruitmentStatusTracking
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "mtRecruitmentStatusTrackingAdd";
        ...
        // Constructor
        public MtRecruitmentStatusTrackingAddBase()
        {
            // Initialize
            CurrentPage = this;
        ...
        // Page name
        public string PageName => "MtRecruitmentStatusTrackingAdd";
        ...
        // Constructor
        public MtRecruitmentStatusTrackingAddBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            switch (CurrentAction) {
                case "copy": // Copy an existing record
                    if (rsold == null) { // Record not loaded
                        ...
                        return Terminate("MtRecruitmentStatusTrackingList"); // No matching record, return to List page // DN
                    }
                    break;
                case "insert": // Add new record // DN
                    ...
                    if (res) { // Add successful
                        ...
                        string returnUrl = "";
                        returnUrl = ReturnUrl;
                        if (GetPageName(returnUrl) == "MtRecruitmentStatusTrackingList")
                            returnUrl = AddMasterUrl(ListUrl); // List page, return to List page with correct master key if necessary
                        else if (GetPageName(returnUrl) == "MtRecruitmentStatusTrackingView")
                        ...
                        if (IsModal && UseAjaxActions) {
                            ...
                            if (GetPageName(returnUrl) != "MtRecruitmentStatusTrackingList") {
                                ...
                                returnUrl = "MtRecruitmentStatusTrackingList"; // Return list page content
                            }
                        }
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                mtRecruitmentStatusTrackingAdd?.PageRender();
            }
            return PageResult();
        }
        ...
        protected async Task LoadFormValues() {
            ...
            // Check field name 'MTRecruitmentStatusID' before field var 'x_MTRecruitmentStatusID'
            val = CurrentForm.HasValue("MTRecruitmentStatusID") ? CurrentForm.GetValue("MTRecruitmentStatusID") : CurrentForm.GetValue("x_MTRecruitmentStatusID");
            if (!MTRecruitmentStatusID.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("MTRecruitmentStatusID") && !CurrentForm.HasValue("x_MTRecruitmentStatusID")) // DN
                    MTRecruitmentStatusID.Visible = false; // Disable update for API request
                else
                    MTRecruitmentStatusID.SetFormValue(val, true, validate);
            }
            ...
            if (!Flag.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("Flag") && !CurrentForm.HasValue("x_Flag")) // DN
                    ...
                    Flag.SetFormValue(val, true, validate);
            }

            // Check field name 'FlagDescription' before field var 'x_FlagDescription'
            val = CurrentForm.HasValue("FlagDescription") ? CurrentForm.GetValue("FlagDescription") : CurrentForm.GetValue("x_FlagDescription");
            if (!FlagDescription.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("FlagDescription") && !CurrentForm.HasValue("x_FlagDescription")) // DN
                    FlagDescription.Visible = false; // Disable update for API request
                else
                    FlagDescription.SetFormValue(val);
            }
        ...
        public void RestoreFormValues()
        {
            ...
            MTRecruitmentStatusID.CurrentValue = MTRecruitmentStatusID.FormValue;
            Flag.CurrentValue = Flag.FormValue;
            FlagDescription.CurrentValue = FlagDescription.FormValue;
        ...
        public async Task RenderRow()
        {
            ...
            MTRecruitmentStatusID.RowCssClass = "row";

            // Flag
            Flag.RowCssClass = "row";

            // FlagDescription
            FlagDescription.RowCssClass = "row";
            ...
            if (RowType == RowType.View) {
                ...
                // MTRecruitmentStatusID
                MTRecruitmentStatusID.SetupEditAttributes();
                MTRecruitmentStatusID.EditValue = MTRecruitmentStatusID.CurrentValue; // DN
                MTRecruitmentStatusID.PlaceHolder = RemoveHtml(MTRecruitmentStatusID.Caption);
                if (!Empty(MTRecruitmentStatusID.EditValue) && IsNumeric(MTRecruitmentStatusID.EditValue))
                    MTRecruitmentStatusID.EditValue = FormatNumber(MTRecruitmentStatusID.EditValue, null);

                // Flag
                Flag.SetupEditAttributes();
                Flag.EditValue = Flag.CurrentValue; // DN
                Flag.PlaceHolder = RemoveHtml(Flag.Caption);
                if (!Empty(Flag.EditValue) && IsNumeric(Flag.EditValue))
                    Flag.EditValue = FormatNumber(Flag.EditValue, null);

                // FlagDescription
                FlagDescription.SetupEditAttributes();
                if (!FlagDescription.Raw)
                    FlagDescription.CurrentValue = HtmlDecode(FlagDescription.CurrentValue);
                FlagDescription.EditValue = HtmlEncode(FlagDescription.CurrentValue);
                FlagDescription.PlaceHolder = RemoveHtml(FlagDescription.Caption);
        ...
        protected async Task<bool> ValidateForm() {
            ...
            if (MTRecruitmentStatusID.Required) {
                if (!MTRecruitmentStatusID.IsDetailKey && Empty(MTRecruitmentStatusID.FormValue)) {
                    MTRecruitmentStatusID.AddErrorMessage(ConvertToString(MTRecruitmentStatusID.RequiredErrorMessage).Replace("%s", MTRecruitmentStatusID.Caption));
                }
            }
            if (!CheckInteger(MTRecruitmentStatusID.FormValue)) {
                MTRecruitmentStatusID.AddErrorMessage(MTRecruitmentStatusID.GetErrorMessage(false));
            }
            if (Flag.Required) {
            ...
            if (!CheckInteger(Flag.FormValue)) {
                Flag.AddErrorMessage(Flag.GetErrorMessage(false));
            }
            if (FlagDescription.Required) {
                if (!FlagDescription.IsDetailKey && Empty(FlagDescription.FormValue)) {
                    FlagDescription.AddErrorMessage(ConvertToString(FlagDescription.RequiredErrorMessage).Replace("%s", FlagDescription.Caption));
                }
            }
        ...
        protected async Task<JsonBoolResult> AddRow(Dictionary<string, object>? rsold = null) { // DN
            ...
            try {
                ...
                // MTRecruitmentStatusID
                MTRecruitmentStatusID.SetDbValue(rsnew, MTRecruitmentStatusID.CurrentValue);

                // Flag
                Flag.SetDbValue(rsnew, Flag.CurrentValue);

                // FlagDescription
                FlagDescription.SetDbValue(rsnew, FlagDescription.CurrentValue);
        ...
        protected void SetupBreadcrumb() {
            ...
            breadcrumb.Add("list", TableVar, AppPath(AddMasterUrl("MtRecruitmentStatusTrackingList")), "", TableVar, true);
