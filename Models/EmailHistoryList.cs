...
public partial class PCM {
    ...
    /// emailHistoryList
    /// </summary>
    public static EmailHistoryList emailHistoryList
    {
        get => HttpData.Get<EmailHistoryList>("emailHistoryList")!;
        set => HttpData["emailHistoryList"] = value;
    }

    /// <summary>
    /// Page class for EmailHistory
    /// </summary>
    public class EmailHistoryList : EmailHistoryListBase
    {
        // Constructor
        public EmailHistoryList(Controller controller) : base(controller)
        {
        }
        ...
        public override void ListOptionsRendered() {
            //Example:
            //ListOptions["new"].Body = "xxx";
            string resendTooltipText = (CurrentLanguage == "en-US") ? "Resend" : "Kirim Ulang";
            ListOptions["view"].Body = "<a email-id=\"" + ConvertToString(Id.CurrentValue) + "\" class=\"ew-row-link resend-email\" title=\"" + resendTooltipText + "\" href=\"#\"><i class=\"fa-solid fa-share ew-icon\"></i></a>";
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class EmailHistoryListBase : EmailHistory
    {
        // Page ID
        ...
        // Table name
        public string TableName { get; set; } = "EmailHistory";

        // Page object name
        public string PageObjName = "emailHistoryList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fEmailHistorylist";
        ...
        // Constructor
        public EmailHistoryListBase()
        {
            // CSS class name as context
            TableVar = "EmailHistory";
            ...
            // Table object (emailHistory)
            if (emailHistory == null || emailHistory is EmailHistory)
                emailHistory = this;

            // Initialize URLs
            AddUrl = "EmailHistoryAdd";
            ...
            MultiDeleteUrl = "EmailHistoryDelete";
            MultiUpdateUrl = "EmailHistoryUpdate";
        ...
        // Page name
        public string PageName => "EmailHistoryList";
        ...
        {
            Id.Visible = false;
            CrewSeafarerIDWithName.SetVisibility();
            To.SetVisibility();
        ...
        // Constructor
        public EmailHistoryListBase(Controller? controller = null): this() { // DN
            if (controller != null)
                ...
                if (Response != null && !Response.HasStarted) {
                    ...
                    if (IsModal) { // Show as modal
                        ...
                        if (pageName != ListUrl) { // Not List page
                            ...
                            result.Add("view", pageName == "EmailHistoryView" ? "1" : "0"); // If View page, no primary button
                        } else { // List page
        ...
        protected void HideFieldsForAddEdit() {
            if (IsAddOrEdit)
                CrewSeafarerIDWithName.Visible = false;
        }
        ...
        public async Task<string> GetFilterList()
        {
            ...
            filters.Merge(JObject.Parse(CrewSeafarerIDWithName.AdvancedSearch.ToJson())); // Field CrewSeafarerIDWithName
        ...
        protected bool RestoreFilterList() {
            ...
            // Field CrewSeafarerIDWithName
            if (filter?.TryGetValue("x_CrewSeafarerIDWithName", out sv) ?? false) {
                CrewSeafarerIDWithName.AdvancedSearch.SearchValue = sv;
                CrewSeafarerIDWithName.AdvancedSearch.SearchOperator = filter["z_CrewSeafarerIDWithName"];
                CrewSeafarerIDWithName.AdvancedSearch.SearchCondition = filter["v_CrewSeafarerIDWithName"];
                CrewSeafarerIDWithName.AdvancedSearch.SearchValue2 = filter["y_CrewSeafarerIDWithName"];
                CrewSeafarerIDWithName.AdvancedSearch.SearchOperator2 = filter["w_CrewSeafarerIDWithName"];
                CrewSeafarerIDWithName.AdvancedSearch.Save();
            }

            // Field To
            if (filter?.TryGetValue("x_To", out sv) ?? false) {
                To.AdvancedSearch.SearchValue = sv;
                To.AdvancedSearch.SearchOperator = filter["z_To"];
                To.AdvancedSearch.SearchCondition = filter["v_To"];
                To.AdvancedSearch.SearchValue2 = filter["y_To"];
                To.AdvancedSearch.SearchOperator2 = filter["w_To"];
        ...
        public string AdvancedSearchWhere(bool def = false) {
            ...
            if (!Security.CanSearch)
                return "";
            BuildSearchSql(ref where, CrewSeafarerIDWithName, def, true); // CrewSeafarerIDWithName
            BuildSearchSql(ref where, To, def, true); // To
            BuildSearchSql(ref where, Subject, def, true); // Subject
            BuildSearchSql(ref where, IsSent, def, true); // IsSent
            BuildSearchSql(ref where, SentDateTime, def, true); // SentDateTime

            // Set up search command
            if (!def && !Empty(where) && (new[] { "", "reset", "resetall" }).Contains(Command))
                Command = "search";
            if (!def && Command == "search") {
                CrewSeafarerIDWithName.AdvancedSearch.Save(); // CrewSeafarerIDWithName
                To.AdvancedSearch.Save(); // To
                Subject.AdvancedSearch.Save(); // Subject
                IsSent.AdvancedSearch.Save(); // IsSent
                SentDateTime.AdvancedSearch.Save(); // SentDateTime
        ...
        public void ShowFilterList()
        {
            ...
            // Field CrewSeafarerIDWithName
            filter = QueryBuilderWhere("CrewSeafarerIDWithName");
            if (Empty(filter))
                BuildSearchSql(ref filter, CrewSeafarerIDWithName, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + CrewSeafarerIDWithName.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field To
            filter = QueryBuilderWhere("To");
            if (Empty(filter))
                BuildSearchSql(ref filter, To, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + To.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field Subject
            filter = QueryBuilderWhere("Subject");
            if (Empty(filter))
                BuildSearchSql(ref filter, Subject, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + Subject.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field IsSent
            filter = QueryBuilderWhere("IsSent");
            if (Empty(filter))
                BuildSearchSql(ref filter, IsSent, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + IsSent.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field SentDateTime
            filter = QueryBuilderWhere("SentDateTime");
            if (Empty(filter))
                BuildSearchSql(ref filter, SentDateTime, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + SentDateTime.Caption + "</span>" + captionSuffix + filter + "</div>";
            if (!Empty(BasicSearch.Keyword))
        ...
        public string BasicSearchWhere(bool def = false) {
            ...
            searchFlds.Add(CrewSeafarerIDWithName);
            searchFlds.Add(To);
        ...
        protected bool CheckSearchParms() {
            ...
            if (BasicSearch.IssetSession)
                return true;
            if (CrewSeafarerIDWithName.AdvancedSearch.IssetSession)
                return true;
            if (To.AdvancedSearch.IssetSession)
                return true;
            if (Subject.AdvancedSearch.IssetSession)
                return true;
            if (IsSent.AdvancedSearch.IssetSession)
                return true;
            if (SentDateTime.AdvancedSearch.IssetSession)
                return true;
            return false;
        }
        ...
        protected void ResetAdvancedSearchParms() {
            CrewSeafarerIDWithName.AdvancedSearch.UnsetSession();
            To.AdvancedSearch.UnsetSession();
            Subject.AdvancedSearch.UnsetSession();
            IsSent.AdvancedSearch.UnsetSession();
            SentDateTime.AdvancedSearch.UnsetSession();
        }
        ...
        protected void RestoreSearchParms() {
            ...
            CrewSeafarerIDWithName.AdvancedSearch.Load();
            To.AdvancedSearch.Load();
            Subject.AdvancedSearch.Load();
            IsSent.AdvancedSearch.Load();
            SentDateTime.AdvancedSearch.Load();
        }
        ...
        protected void SetupSortOrder() {
            ...
            if (Command != "json") {
                string defaultSort = SentDateTime.Expression + " DESC"; // Set up default sort
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(CrewSeafarerIDWithName, ctrl); // CrewSeafarerIDWithName
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalView && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-view"" title=""{viewcaption}"" data-table=""EmailHistory"" data-caption=""{viewcaption}"" data-ew-action=""modal"" data-action=""view"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(ViewUrl)) + "\" data-btn=\"null\">" + Language.Phrase("ViewLink") + "</a>");
                else
            ...
            if (listOption != null && !IsExport() && CurrentAction == "") {
                ...
                foreach (var (key, act) in ListActions.Items) {
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        string link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fEmailHistorylist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + " " + caption + "</button></li>";
                        if (!Empty(link)) {
                            links.Add(link);
                            if (Empty(body)) // Setup first button
                                body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fEmailHistorylist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + caption + "</button>";
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("CrewSeafarerIDWithName")); // DN
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fEmailHistorysrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fEmailHistorysrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fEmailHistorylist\"" + act.ToDataAttrs() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_EmailHistory");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(emailHistoryList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(emailHistoryList.RowCount) + "_EmailHistory");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(emailHistoryList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && emailHistoryList.RowType == RowType.Add || IsEdit && emailHistoryList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        protected void LoadSearchValues() {
            ...
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_CrewSeafarerIDWithName[]"))
                    CrewSeafarerIDWithName.AdvancedSearch.SearchValue = Get("x_CrewSeafarerIDWithName[]");
                else
                    CrewSeafarerIDWithName.AdvancedSearch.SearchValue = Get("CrewSeafarerIDWithName"); // Default Value // DN
            if (!Empty(CrewSeafarerIDWithName.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_CrewSeafarerIDWithName"))
                CrewSeafarerIDWithName.AdvancedSearch.SearchOperator = Get("z_CrewSeafarerIDWithName");

            // To
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_To[]"))
                    To.AdvancedSearch.SearchValue = Get("x_To[]");
                else
                    To.AdvancedSearch.SearchValue = Get("To"); // Default Value // DN
            if (!Empty(To.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_To"))
                To.AdvancedSearch.SearchOperator = Get("z_To");

            // Subject
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_Subject[]"))
                    Subject.AdvancedSearch.SearchValue = Get("x_Subject[]");
                else
                    Subject.AdvancedSearch.SearchValue = Get("Subject"); // Default Value // DN
            if (!Empty(Subject.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_Subject"))
                Subject.AdvancedSearch.SearchOperator = Get("z_Subject");

            // IsSent
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_IsSent[]"))
                    IsSent.AdvancedSearch.SearchValue = Get("x_IsSent[]");
                else
                    IsSent.AdvancedSearch.SearchValue = Get("IsSent"); // Default Value // DN
            if (!Empty(IsSent.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_IsSent"))
                IsSent.AdvancedSearch.SearchOperator = Get("z_IsSent");

            // SentDateTime
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_SentDateTime[]"))
                    SentDateTime.AdvancedSearch.SearchValue = Get("x_SentDateTime[]");
                else
                    SentDateTime.AdvancedSearch.SearchValue = Get("SentDateTime"); // Default Value // DN
            if (!Empty(SentDateTime.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_SentDateTime"))
                SentDateTime.AdvancedSearch.SearchOperator = Get("z_SentDateTime");
        }
        ...
        public async Task LoadRowValues(Dictionary<string, object>? row)
        {
            ...
            CrewSeafarerIDWithName.SetDbValue(row["CrewSeafarerIDWithName"]);
        ...
        protected Dictionary<string, object> NewRow() {
            ...
            row.Add("CrewSeafarerIDWithName", CrewSeafarerIDWithName.DefaultValue ?? DbNullValue); // DN
        ...
        public async Task RenderRow()
        {
            ...
            // Message
            _Message.CellCssStyle = "white-space: nowrap;";
            ...
            if (RowType == RowType.View) {
                // CrewSeafarerIDWithName
                CrewSeafarerIDWithName.ViewValue = ConvertToString(CrewSeafarerIDWithName.CurrentValue); // DN
                CrewSeafarerIDWithName.ViewCustomAttributes = "";
                ...
                // CrewSeafarerIDWithName
                CrewSeafarerIDWithName.HrefValue = "";
                CrewSeafarerIDWithName.TooltipValue = "";
                ...
                // CrewSeafarerIDWithName
                if (CrewSeafarerIDWithName.UseFilter && !Empty(CrewSeafarerIDWithName.AdvancedSearch.SearchValue)) {
                    CrewSeafarerIDWithName.EditValue = ConvertToString(CrewSeafarerIDWithName.AdvancedSearch.SearchValue).Split(Config.MultipleOptionSeparator).ToList();
                }

                // To
                if (To.UseFilter && !Empty(To.AdvancedSearch.SearchValue)) {
                    To.EditValue = ConvertToString(To.AdvancedSearch.SearchValue).Split(Config.MultipleOptionSeparator).ToList();
                }

                // Subject
                if (Subject.UseFilter && !Empty(Subject.AdvancedSearch.SearchValue)) {
                    Subject.EditValue = ConvertToString(Subject.AdvancedSearch.SearchValue).Split(Config.MultipleOptionSeparator).ToList();
                }

                // IsSent
                if (IsSent.UseFilter && !Empty(IsSent.AdvancedSearch.SearchValue)) {
                    IsSent.EditValue = ConvertToString(IsSent.AdvancedSearch.SearchValue).Split(Config.MultipleOptionSeparator).ToList();
                }

                // SentDateTime
                if (SentDateTime.UseFilter && !Empty(SentDateTime.AdvancedSearch.SearchValue)) {
                    SentDateTime.EditValue = ConvertToString(SentDateTime.AdvancedSearch.SearchValue).Split(Config.MultipleOptionSeparator).ToList();
                }
            }
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fEmailHistorysrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
            ...
            if (ModalSearch && !IsMobile())
                item.Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-table=\"EmailHistory\" data-caption=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-ew-action=\"modal\" data-url=\"" + AppPath("EmailHistorySearch") + "\" data-btn=\"SearchBtn\">" + Language.Phrase("AdvancedSearch", false) + "</a>";
            else
                item.Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-caption=\"" + Language.Phrase("AdvancedSearch", true) + "\" href=\"" + AppPath("EmailHistorySearch") + "\">" + Language.Phrase("AdvancedSearch", false) + "</a>";
            item.Visible = true;
