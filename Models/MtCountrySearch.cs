...
public partial class PCM {
    ...
    /// mtCountrySearch
    /// </summary>
    public static MtCountrySearch mtCountrySearch
    {
        get => HttpData.Get<MtCountrySearch>("mtCountrySearch")!;
        set => HttpData["mtCountrySearch"] = value;
    }

    /// <summary>
    /// Page class for MTCountry
    /// </summary>
    public class MtCountrySearch : MtCountrySearchBase
    {
        // Constructor
        public MtCountrySearch(Controller controller) : base(controller)
        {
        }

        // Constructor
        public MtCountrySearch() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class MtCountrySearchBase : MtCountry
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "mtCountrySearch";
        ...
        // Page name
        public string PageName => "MtCountrySearch";
        ...
        // Constructor
        public MtCountrySearchBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        protected string BuildAdvancedSearch() {
            ...
            BuildSearchUrl(ref srchUrl, Nationality); // Nationality
            BuildSearchUrl(ref srchUrl, CallingCode); // CallingCode
        ...
        protected void LoadSearchValues() {
            ...
            // Nationality
            if (!IsAddOrEdit)
                if (Form.ContainsKey("x_Nationality"))
                    Nationality.AdvancedSearch.SearchValue = CurrentForm.GetValue("x_Nationality");
            if (Form.ContainsKey("z_Nationality"))
                Nationality.AdvancedSearch.SearchOperator = CurrentForm.GetValue("z_Nationality");

            // CallingCode
            if (!IsAddOrEdit)
                if (Form.ContainsKey("x_CallingCode"))
                    CallingCode.AdvancedSearch.SearchValue = CurrentForm.GetValue("x_CallingCode");
            if (Form.ContainsKey("z_CallingCode"))
                CallingCode.AdvancedSearch.SearchOperator = CurrentForm.GetValue("z_CallingCode");
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                if (!Nationality.Raw)
                    Nationality.AdvancedSearch.SearchValue = HtmlDecode(Nationality.AdvancedSearch.SearchValue);
                Nationality.EditValue = HtmlEncode(Nationality.AdvancedSearch.SearchValue);
                Nationality.PlaceHolder = RemoveHtml(Nationality.Caption);

                // CallingCode
                CallingCode.SetupEditAttributes();
                if (!CallingCode.Raw)
                    CallingCode.AdvancedSearch.SearchValue = HtmlDecode(CallingCode.AdvancedSearch.SearchValue);
                CallingCode.EditValue = HtmlEncode(CallingCode.AdvancedSearch.SearchValue);
