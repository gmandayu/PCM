...
public partial class PCM {
    /// <summary>
    /// mtCrewExperienceList
    /// </summary>
    public static MtCrewExperienceList mtCrewExperienceList
    {
        get => HttpData.Get<MtCrewExperienceList>("mtCrewExperienceList")!;
        set => HttpData["mtCrewExperienceList"] = value;
    }

    /// <summary>
    /// Page class for MTCrewExperience
    /// </summary>
    public class MtCrewExperienceList : MtCrewExperienceListBase
    {
        // Constructor
        public MtCrewExperienceList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public MtCrewExperienceList() : base()
        {
        }
    ...
    /// Page base class
    /// </summary>
    public class MtCrewExperienceListBase : MtCrewExperience
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "mtCrewExperienceList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fMTCrewExperiencelist";
        ...
        // Constructor
        public MtCrewExperienceListBase()
        {
            ...
            MultiDeleteUrl = "MtCrewExperienceDelete";
            MultiUpdateUrl = "MtCrewExperienceUpdate";
        ...
        // Page name
        public string PageName => "MtCrewExperienceList";
        ...
        // Constructor
        public MtCrewExperienceListBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                mtCrewExperienceList?.PageRender();
            }
            return PageResult();
        }
        #pragma warning restore 219
            ...
            if (listOption != null && !IsExport() && CurrentAction == "") {
                ...
                foreach (var (key, act) in ListActions.Items) {
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        string link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fMTCrewExperiencelist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + " " + caption + "</button></li>";
                        if (!Empty(link)) {
                            links.Add(link);
                            if (Empty(body)) // Setup first button
                                body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fMTCrewExperiencelist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + caption + "</button>";
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fMTCrewExperiencesrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fMTCrewExperiencesrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fMTCrewExperiencelist\"" + act.ToDataAttrs() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(mtCrewExperienceList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(mtCrewExperienceList.RowCount) + "_MTCrewExperience");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(mtCrewExperienceList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && mtCrewExperienceList.RowType == RowType.Add || IsEdit && mtCrewExperienceList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fMTCrewExperiencesrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
            ...
            if (ModalSearch && !IsMobile())
                item.Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-table=\"MTCrewExperience\" data-caption=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-ew-action=\"modal\" data-url=\"" + AppPath("MtCrewExperienceSearch") + "\" data-btn=\"SearchBtn\">" + Language.Phrase("AdvancedSearch", false) + "</a>";
            else
                item.Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-caption=\"" + Language.Phrase("AdvancedSearch", true) + "\" href=\"" + AppPath("MtCrewExperienceSearch") + "\">" + Language.Phrase("AdvancedSearch", false) + "</a>";
            item.Visible = true;
        ...
        public async Task ExportData(dynamic? doc)
        {
            ...
            if (Config.ExportMasterRecord && !Empty(MasterFilterFromSession) && CurrentMasterTable == "MTCrew") {
                mtCrew = new MtCrewList();
                if (mtCrew != null) {
                    var c = await GetConnection2Async(mtCrew.DbId); // Note: Use new connection for master record // DN
                    using var rsmaster = await mtCrew.LoadReader(DbMasterFilter, "", c); // Load master record
                    if (rsmaster?.HasRows ?? false) { // DN
                        exportStyle = doc.Style;
                        doc.SetStyle("v"); // Change to vertical
                        if (!IsExport("csv") || Config.ExportMasterRecordForCsv) {
                            doc.Table = mtCrew;
                            await mtCrew.ExportDocument(doc, rsmaster, 1, 1);
                            doc.ExportEmptyRow();
                            doc.Table = this;
                        }
        ...
        protected void SetupMasterParms() {
            ...
            if (Query.TryGetValue(Config.TableShowMaster, out masterTblVar) || Query.TryGetValue(Config.TableMaster, out masterTblVar)) { // Do not use Get()
                ...
                if (masterTblVar == "MTCrew") {
                    validMaster = true;
                    if (mtCrew != null && (Get("fk_ID", out fk) || Get("MTCrewID", out fk))) {
                        mtCrew.ID.QueryValue = fk;
                        MTCrewID.QueryValue = mtCrew.ID.QueryValue;
                        MTCrewID.SessionValue = MTCrewID.QueryValue;
                        foreignKeys.Add("MTCrewID", fk);
                        if (!IsNumeric(MTCrewID.QueryValue))
                            validMaster = false;
                    } else {
                        validMaster = false;
                    }
                ...
                if (masterTblVar == "MTCrew") {
                    validMaster = true;
                    if (mtCrew != null && (Post("fk_ID", out fk) || Post("MTCrewID", out fk))) {
                        mtCrew.ID.FormValue = fk;
                        MTCrewID.FormValue = mtCrew.ID.FormValue;
                        MTCrewID.SessionValue = MTCrewID.FormValue;
                        foreignKeys.Add("MTCrewID", fk);
                        if (!IsNumeric(MTCrewID.FormValue))
                            validMaster = false;
                    } else {
                        validMaster = false;
                    }
            ...
            if (validMaster) {
                ...
                if (masterTblVar != "MTCrew") {
                    if (!foreignKeys.ContainsKey("MTCrewID")) // Not current foreign key
                        MTCrewID.SessionValue = "";
                }
            }
