...
public partial class PCM {
    /// <summary>
    /// vCrewBlacklistForAdminList
    /// </summary>
    public static VCrewBlacklistForAdminList vCrewBlacklistForAdminList
    {
        get => HttpData.Get<VCrewBlacklistForAdminList>("vCrewBlacklistForAdminList")!;
        set => HttpData["vCrewBlacklistForAdminList"] = value;
    }

    /// <summary>
    /// Page class for v_CrewBlacklistForAdmin
    /// </summary>
    public class VCrewBlacklistForAdminList : VCrewBlacklistForAdminListBase
    {
        // Constructor
        public VCrewBlacklistForAdminList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public VCrewBlacklistForAdminList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class VCrewBlacklistForAdminListBase : VCrewBlacklistForAdmin
    {
        // Page ID
        ...
        // Table name
        public string TableName { get; set; } = "v_CrewBlacklistForAdmin";

        // Page object name
        public string PageObjName = "vCrewBlacklistForAdminList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fv_CrewBlacklistForAdminlist";
        ...
        // Constructor
        public VCrewBlacklistForAdminListBase()
        {
            // CSS class name as context
            TableVar = "v_CrewBlacklistForAdmin";
            ...
            // Table object (vCrewBlacklistForAdmin)
            if (vCrewBlacklistForAdmin == null || vCrewBlacklistForAdmin is VCrewBlacklistForAdmin)
                vCrewBlacklistForAdmin = this;

            // Initialize URLs
            AddUrl = "VCrewBlacklistForAdminAdd";
            ...
            MultiDeleteUrl = "VCrewBlacklistForAdminDelete";
            MultiUpdateUrl = "VCrewBlacklistForAdminUpdate";
        ...
        // Page name
        public string PageName => "VCrewBlacklistForAdminList";
        ...
        // Constructor
        public VCrewBlacklistForAdminListBase(Controller? controller = null): this() { // DN
            if (controller != null)
                ...
                if (Response != null && !Response.HasStarted) {
                    ...
                    if (IsModal) { // Show as modal
                        ...
                        if (pageName != ListUrl) { // Not List page
                            ...
                            result.Add("view", pageName == "VCrewBlacklistForAdminView" ? "1" : "0"); // If View page, no primary button
                        } else { // List page
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (IsModal)
                FormName = "fv_CrewBlacklistForAdmingrid";
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                vCrewBlacklistForAdminList?.PageRender();
            }
            return PageResult();
        }
        #pragma warning restore 219
        ...
        public string AdvancedSearchWhere(bool def = false) {
            ...
            BuildSearchSql(ref where, CityOfBirth, def, false); // CityOfBirth
            BuildSearchSql(ref where, DateOfBirth, def, false); // DateOfBirth
            BuildSearchSql(ref where, Gender, def, false); // Gender
            BuildSearchSql(ref where, RankAppliedFor_RankID, def, false); // RankAppliedFor_RankID
        ...
        public void ShowFilterList()
        {
            ...
            if (Empty(filter))
                BuildSearchSql(ref filter, CityOfBirth, false, false);
            if (!Empty(filter))
            ...
            if (Empty(filter))
                BuildSearchSql(ref filter, DateOfBirth, false, false);
            if (!Empty(filter))
            ...
            if (Empty(filter))
                BuildSearchSql(ref filter, RankAppliedFor_RankID, false, false);
            if (!Empty(filter))
            ...
            if (Empty(filter))
                BuildSearchSql(ref filter, WillAcceptLowRank, false, false);
            if (!Empty(filter))
            ...
            if (Empty(filter))
                BuildSearchSql(ref filter, AvailableFrom, false, false);
            if (!Empty(filter))
            ...
            if (Empty(filter))
                BuildSearchSql(ref filter, AvailableUntil, false, false);
            if (!Empty(filter))
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalView && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-view"" title=""{viewcaption}"" data-table=""v_CrewBlacklistForAdmin"" data-caption=""{viewcaption}"" data-ew-action=""modal"" data-action=""view"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(ViewUrl)) + "\" data-btn=\"null\">" + Language.Phrase("ViewLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""v_CrewBlacklistForAdmin"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
            ...
            if (listOption != null && !IsExport() && CurrentAction == "") {
                ...
                foreach (var (key, act) in ListActions.Items) {
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        string link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fv_CrewBlacklistForAdminlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + " " + caption + "</button></li>";
                        if (!Empty(link)) {
                            links.Add(link);
                            if (Empty(body)) // Setup first button
                                body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fv_CrewBlacklistForAdminlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + caption + "</button>";
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fv_CrewBlacklistForAdminsrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fv_CrewBlacklistForAdminsrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fv_CrewBlacklistForAdminlist\"" + act.ToDataAttrs() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_v_CrewBlacklistForAdmin");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(vCrewBlacklistForAdminList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(vCrewBlacklistForAdminList.RowCount) + "_v_CrewBlacklistForAdmin");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(vCrewBlacklistForAdminList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && vCrewBlacklistForAdminList.RowType == RowType.Add || IsEdit && vCrewBlacklistForAdminList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        protected string GetExportTag(string type, bool custom = false) {
            ...
            if (SameText(type, "excel")) {
                if (custom)
                    return "<button type=\"button\" class=\"btn btn-default ew-export-link ew-excel\" title=\"" + HtmlEncode(Language.Phrase("ExportToExcel", true)) + "\" data-caption=\"" + HtmlEncode(Language.Phrase("ExportToExcel", true)) + "\" form=\"fv_CrewBlacklistForAdminlist\" data-url=\"" + exportUrl + "\" data-ew-action=\"export\" data-export=\"excel\" data-custom=\"true\" data-export-selected=\"false\">" + Language.Phrase("ExportToExcel") + "</button>";
                else
                    return "<a href=\"" + exportUrl + "\" class=\"btn btn-default ew-export-link ew-excel\" title=\"" + HtmlEncode(Language.Phrase("ExportToExcel", true)) + "\" data-caption=\"" + HtmlEncode(Language.Phrase("ExportToExcel", true)) + "\">" + Language.Phrase("ExportToExcel") + "</a>";
            } else if (SameText(type, "word")) {
                if (custom)
                    return "<button type=\"button\" class=\"btn btn-default ew-export-link ew-word\" title=\"" + HtmlEncode(Language.Phrase("ExportToWord", true)) + "\" data-caption=\"" + HtmlEncode(Language.Phrase("ExportToWord", true)) + "\" form=\"fv_CrewBlacklistForAdminlist\" data-url=\"" + exportUrl + "\" data-ew-action=\"export\" data-export=\"word\" data-custom=\"true\" data-export-selected=\"false\">" + Language.Phrase("ExportToWord") + "</button>";
                else
                    return "<a href=\"" + exportUrl + "\" class=\"btn btn-default ew-export-link ew-word\" title=\"" + HtmlEncode(Language.Phrase("ExportToWord", true)) + "\" data-caption=\"" + HtmlEncode(Language.Phrase("ExportToWord", true)) + "\">" + Language.Phrase("ExportToWord") + "</a>";
            } else if (SameText(type, "pdf")) {
                if (custom)
                    return "<button type=\"button\" class=\"btn btn-default ew-export-link ew-pdf\" title=\"" + HtmlEncode(Language.Phrase("ExportToPdf", true)) + "\" data-caption=\"" + HtmlEncode(Language.Phrase("ExportToPdf", true)) + "\" form=\"fv_CrewBlacklistForAdminlist\" data-url=\"" + exportUrl + "\" data-ew-action=\"export\" data-export=\"pdf\" data-custom=\"true\" data-export-selected=\"false\">" + Language.Phrase("ExportToPDF") + "</button>";
                else
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fv_CrewBlacklistForAdminsrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
            ...
            if (ModalSearch && !IsMobile())
                item.Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-table=\"v_CrewBlacklistForAdmin\" data-caption=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-ew-action=\"modal\" data-url=\"" + AppPath("VCrewBlacklistForAdminSearch") + "\" data-btn=\"SearchBtn\">" + Language.Phrase("AdvancedSearch", false) + "</a>";
            else
                item.Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-caption=\"" + Language.Phrase("AdvancedSearch", true) + "\" href=\"" + AppPath("VCrewBlacklistForAdminSearch") + "\">" + Language.Phrase("AdvancedSearch", false) + "</a>";
            item.Visible = true;
