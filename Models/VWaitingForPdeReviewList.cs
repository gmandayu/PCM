...
public partial class PCM {
    /// <summary>
    /// vWaitingForPdeReviewList
    /// </summary>
    public static VWaitingForPdeReviewList vWaitingForPdeReviewList
    {
        get => HttpData.Get<VWaitingForPdeReviewList>("vWaitingForPdeReviewList")!;
        set => HttpData["vWaitingForPdeReviewList"] = value;
    }

    /// <summary>
    /// Page class for v_WaitingForPDEReview
    /// </summary>
    public class VWaitingForPdeReviewList : VWaitingForPdeReviewListBase
    {
        // Constructor
        public VWaitingForPdeReviewList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public VWaitingForPdeReviewList() : base()
        {
        }
    ...
    /// Page base class
    /// </summary>
    public class VWaitingForPdeReviewListBase : VWaitingForPdeReview
    {
        // Page ID
        ...
        // Table name
        public string TableName { get; set; } = "v_WaitingForPDEReview";

        // Page object name
        public string PageObjName = "vWaitingForPdeReviewList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fv_WaitingForPDEReviewlist";
        ...
        // Constructor
        public VWaitingForPdeReviewListBase()
        {
            ...
            // Table object (vWaitingForPdeReview)
            if (vWaitingForPdeReview == null || vWaitingForPdeReview is VWaitingForPdeReview)
                vWaitingForPdeReview = this;

            // Initialize URLs
            AddUrl = "VWaitingForPdeReviewAdd";
            ...
            MultiDeleteUrl = "VWaitingForPdeReviewDelete";
            MultiUpdateUrl = "VWaitingForPdeReviewUpdate";
        ...
        // Page name
        public string PageName => "VWaitingForPdeReviewList";
        ...
        // Constructor
        public VWaitingForPdeReviewListBase(Controller? controller = null): this() { // DN
            if (controller != null)
                ...
                if (Response != null && !Response.HasStarted) {
                    ...
                    if (IsModal) { // Show as modal
                        ...
                        if (pageName != ListUrl) { // Not List page
                            ...
                            result.Add("view", pageName == "VWaitingForPdeReviewView" ? "1" : "0"); // If View page, no primary button
                        } else { // List page
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (IsModal)
                FormName = "fv_WaitingForPDEReviewgrid";
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                vWaitingForPdeReviewList?.PageRender();
            }
            return PageResult();
        }
        #pragma warning restore 219
        ...
        protected void SetupSortOrder() {
            ...
            if (Command != "json") {
                string defaultSort = FormSubmittedDateTime.Expression + " ASC" + ", " + CreatedDateTime.Expression + " ASC"; // Set up default sort
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalView && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-view"" title=""{viewcaption}"" data-table=""v_WaitingForPDEReview"" data-caption=""{viewcaption}"" data-ew-action=""modal"" data-action=""view"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(ViewUrl)) + "\" data-btn=\"null\">" + Language.Phrase("ViewLink") + "</a>");
                else
            ...
            if (listOption != null && !IsExport() && CurrentAction == "") {
                ...
                foreach (var (key, act) in ListActions.Items) {
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        string link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fv_WaitingForPDEReviewlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + " " + caption + "</button></li>";
                        if (!Empty(link)) {
                            links.Add(link);
                            if (Empty(body)) // Setup first button
                                body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fv_WaitingForPDEReviewlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + caption + "</button>";
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fv_WaitingForPDEReviewsrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fv_WaitingForPDEReviewsrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fv_WaitingForPDEReviewlist\"" + act.ToDataAttrs() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_v_WaitingForPDEReview");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(vWaitingForPdeReviewList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(vWaitingForPdeReviewList.RowCount) + "_v_WaitingForPDEReview");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(vWaitingForPdeReviewList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && vWaitingForPdeReviewList.RowType == RowType.Add || IsEdit && vWaitingForPdeReviewList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fv_WaitingForPDEReviewsrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
            ...
            if (ModalSearch && !IsMobile())
                item.Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-table=\"v_WaitingForPDEReview\" data-caption=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-ew-action=\"modal\" data-url=\"" + AppPath("VWaitingForPdeReviewSearch") + "\" data-btn=\"SearchBtn\">" + Language.Phrase("AdvancedSearch", false) + "</a>";
            else
                item.Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-caption=\"" + Language.Phrase("AdvancedSearch", true) + "\" href=\"" + AppPath("VWaitingForPdeReviewSearch") + "\">" + Language.Phrase("AdvancedSearch", false) + "</a>";
            item.Visible = true;
