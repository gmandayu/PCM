...
public partial class PCM {
    ...
    /// mtCrewAppraisalList
    /// </summary>
    public static MtCrewAppraisalList mtCrewAppraisalList
    {
        get => HttpData.Get<MtCrewAppraisalList>("mtCrewAppraisalList")!;
        set => HttpData["mtCrewAppraisalList"] = value;
    }

    /// <summary>
    /// Page class for MTCrewAppraisal
    /// </summary>
    public class MtCrewAppraisalList : MtCrewAppraisalListBase
    {
        // Constructor
        public MtCrewAppraisalList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public MtCrewAppraisalList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class MtCrewAppraisalListBase : MtCrewAppraisal
    {
        // Page ID
        ...
        // Table name
        public string TableName { get; set; } = "MTCrewAppraisal";

        // Page object name
        public string PageObjName = "mtCrewAppraisalList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fMTCrewAppraisallist";
        ...
        // Constructor
        public MtCrewAppraisalListBase()
        {
            // CSS class name as context
            TableVar = "MTCrewAppraisal";
            ...
            // Table object (mtCrewAppraisal)
            if (mtCrewAppraisal == null || mtCrewAppraisal is MtCrewAppraisal)
                mtCrewAppraisal = this;

            // Initialize URLs
            AddUrl = "MtCrewAppraisalAdd";
            ...
            MultiDeleteUrl = "MtCrewAppraisalDelete";
            MultiUpdateUrl = "MtCrewAppraisalUpdate";
        ...
        // Page name
        public string PageName => "MtCrewAppraisalList";
        ...
        // Constructor
        public MtCrewAppraisalListBase(Controller? controller = null): this() { // DN
            if (controller != null)
                ...
                if (Response != null && !Response.HasStarted) {
                    ...
                    if (IsModal) { // Show as modal
                        ...
                        if (pageName != ListUrl) { // Not List page
                            ...
                            result.Add("view", pageName == "MtCrewAppraisalView" ? "1" : "0"); // If View page, no primary button
                        } else { // List page
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                mtCrewAppraisalList?.PageRender();
            }
            return PageResult();
        }
        #pragma warning restore 219
            ...
            if (listOption != null && !IsExport() && CurrentAction == "") {
                ...
                foreach (var (key, act) in ListActions.Items) {
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        string link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fMTCrewAppraisallist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + " " + caption + "</button></li>";
                        if (!Empty(link)) {
                            links.Add(link);
                            if (Empty(body)) // Setup first button
                                body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fMTCrewAppraisallist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + caption + "</button>";
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fMTCrewAppraisalsrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fMTCrewAppraisalsrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fMTCrewAppraisallist\"" + act.ToDataAttrs() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_MTCrewAppraisal");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(mtCrewAppraisalList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(mtCrewAppraisalList.RowCount) + "_MTCrewAppraisal");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(mtCrewAppraisalList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && mtCrewAppraisalList.RowType == RowType.Add || IsEdit && mtCrewAppraisalList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                if (ConvertToBool(Rehire.CurrentValue)) {
                    Rehire.ViewValue = Rehire.TagCaption(1) != "" ? Rehire.TagCaption(1) : "Yes";
                } else {
                    Rehire.ViewValue = Rehire.TagCaption(2) != "" ? Rehire.TagCaption(2) : "No";
                }
                Rehire.ViewCustomAttributes = "";

                // Promote
                if (ConvertToBool(Promote.CurrentValue)) {
                    Promote.ViewValue = Promote.TagCaption(1) != "" ? Promote.TagCaption(1) : "Yes";
                } else {
                    Promote.ViewValue = Promote.TagCaption(2) != "" ? Promote.TagCaption(2) : "No";
                }
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fMTCrewAppraisalsrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
            ...
            if (ModalSearch && !IsMobile())
                item.Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-table=\"MTCrewAppraisal\" data-caption=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-ew-action=\"modal\" data-url=\"" + AppPath("MtCrewAppraisalSearch") + "\" data-btn=\"SearchBtn\">" + Language.Phrase("AdvancedSearch", false) + "</a>";
            else
                item.Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-caption=\"" + Language.Phrase("AdvancedSearch", true) + "\" href=\"" + AppPath("MtCrewAppraisalSearch") + "\">" + Language.Phrase("AdvancedSearch", false) + "</a>";
            item.Visible = true;
