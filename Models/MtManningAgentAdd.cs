...
public partial class PCM {
    ...
    /// mtManningAgentAdd
    /// </summary>
    public static MtManningAgentAdd mtManningAgentAdd
    {
        get => HttpData.Get<MtManningAgentAdd>("mtManningAgentAdd")!;
        set => HttpData["mtManningAgentAdd"] = value;
    }

    /// <summary>
    /// Page class for MTManningAgent
    /// </summary>
    public class MtManningAgentAdd : MtManningAgentAddBase
    {
        // Constructor
        public MtManningAgentAdd(Controller controller) : base(controller)
        {
        }

        // Constructor
        public MtManningAgentAdd() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class MtManningAgentAddBase : MtManningAgent
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "mtManningAgentAdd";
        ...
        // Page name
        public string PageName => "MtManningAgentAdd";
        ...
        // Constructor
        public MtManningAgentAddBase(Controller? controller = null): this() { // DN
            if (controller != null)
            ...
            switch (CurrentAction) {
                case "copy": // Copy an existing record
                    if (rsold == null) { // Record not loaded
                        ...
                        return Terminate("MtManningAgentList"); // No matching record, return to List page // DN
                    }
                    break;
                case "insert": // Add new record // DN
                    ...
                    if (res) { // Add successful
                        ...
                        string returnUrl = "";
                        returnUrl = ReturnUrl;
                        if (GetPageName(returnUrl) == "MtManningAgentList")
                            returnUrl = AddMasterUrl(ListUrl); // List page, return to List page with correct master key if necessary
                        else if (GetPageName(returnUrl) == "MtManningAgentView")
                        ...
                        if (IsModal && UseAjaxActions) {
                            ...
                            if (GetPageName(returnUrl) != "MtManningAgentList") {
                                ...
                                returnUrl = "MtManningAgentList"; // Return list page content
                            }
                        }
        ...
        protected async Task<bool> ValidateForm() {
            ...
            if (Active.Required) {
                if (Empty(Active.FormValue)) {
                    Active.AddErrorMessage(ConvertToString(Active.RequiredErrorMessage).Replace("%s", Active.Caption));
                }
            }
        ...
        protected async Task<JsonBoolResult> AddRow(Dictionary<string, object>? rsold = null) { // DN
            ...
            try {
                ...
                // Active
                Active.SetDbValue(rsnew, ConvertToBool(Active.CurrentValue));
            } catch (Exception e) {
                if (Config.Debug)
                    throw;
            ...
            if (!Empty(Code.CurrentValue)) { // Check field with unique index
                var filter = "(Code = '" + AdjustSql(Code.CurrentValue, DbId) + "')";
                using var rschk = await LoadReader(filter);
                if (rschk?.HasRows ?? false) { // DN
                    FailureMessage = Language.Phrase("DupIndex").Replace("%f", Code.Caption).Replace("%v", ConvertToString(Code.CurrentValue));
                    return JsonBoolResult.FalseResult;
                }
            }
