...
public partial class PCM {
    ...
    /// mtManningAgentList
    /// </summary>
    public static MtManningAgentList mtManningAgentList
    {
        get => HttpData.Get<MtManningAgentList>("mtManningAgentList")!;
        set => HttpData["mtManningAgentList"] = value;
    }

    /// <summary>
    /// Page class for MTManningAgent
    /// </summary>
    public class MtManningAgentList : MtManningAgentListBase
    {
        // Constructor
        public MtManningAgentList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public MtManningAgentList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class MtManningAgentListBase : MtManningAgent
    {
        // Page ID
        ...
        // Table name
        public string TableName { get; set; } = "MTManningAgent";

        // Page object name
        public string PageObjName = "mtManningAgentList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fMTManningAgentlist";
        ...
        // Constructor
        public MtManningAgentListBase()
        {
            // CSS class name as context
            TableVar = "MTManningAgent";
            ...
            // Table object (mtManningAgent)
            if (mtManningAgent == null || mtManningAgent is MtManningAgent)
                mtManningAgent = this;

            // Initialize URLs
            AddUrl = "MtManningAgentAdd";
            ...
            MultiDeleteUrl = "MtManningAgentDelete";
            MultiUpdateUrl = "MtManningAgentUpdate";
        ...
        // Page name
        public string PageName => "MtManningAgentList";
        ...
        // Constructor
        public MtManningAgentListBase(Controller? controller = null): this() { // DN
            if (controller != null)
                ...
                if (Response != null && !Response.HasStarted) {
                    ...
                    if (IsModal) { // Show as modal
                        ...
                        if (pageName != ListUrl) { // Not List page
                            ...
                            result.Add("view", pageName == "MtManningAgentView" ? "1" : "0"); // If View page, no primary button
                        } else { // List page
        ...
        protected bool RestoreFilterList() {
            ...
            // Field Active
            if (filter?.TryGetValue("x_Active", out sv) ?? false) {
        ...
        public string AdvancedSearchWhere(bool def = false) {
            ...
            if (!Security.CanSearch)
                return "";
            BuildSearchSql(ref where, Code, def, true); // Code
            BuildSearchSql(ref where, _Name, def, true); // _Name
            BuildSearchSql(ref where, Active, def, true); // Active
            ...
            if (!def && Command == "search") {
                Code.AdvancedSearch.Save(); // Code
                _Name.AdvancedSearch.Save(); // Name
                Active.AdvancedSearch.Save(); // Active
        ...
        public void ShowFilterList()
        {
            ...
            // Field Code
            filter = QueryBuilderWhere("Code");
            if (Empty(filter))
                BuildSearchSql(ref filter, Code, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + Code.Caption + "</span>" + captionSuffix + filter + "</div>";
            ...
            // Field Active
            filter = QueryBuilderWhere("Active");
            if (Empty(filter))
                BuildSearchSql(ref filter, Active, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + Active.Caption + "</span>" + captionSuffix + filter + "</div>";
        ...
        protected bool CheckSearchParms() {
            ...
            if (BasicSearch.IssetSession)
                return true;
            if (Code.AdvancedSearch.IssetSession)
                return true;
            if (_Name.AdvancedSearch.IssetSession)
                return true;
            if (Active.AdvancedSearch.IssetSession)
                return true;
        ...
        protected void ResetAdvancedSearchParms() {
            Code.AdvancedSearch.UnsetSession();
            _Name.AdvancedSearch.UnsetSession();
            Active.AdvancedSearch.UnsetSession();
        ...
        protected void RestoreSearchParms() {
            ...
            Code.AdvancedSearch.Load();
            _Name.AdvancedSearch.Load();
            Active.AdvancedSearch.Load();
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(Code, ctrl); // Code
                UpdateSort(_Name, ctrl); // Name
                UpdateSort(Active, ctrl); // Active
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalView && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-view"" title=""{viewcaption}"" data-table=""MTManningAgent"" data-caption=""{viewcaption}"" data-ew-action=""modal"" data-action=""view"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(ViewUrl)) + "\" data-btn=\"null\">" + Language.Phrase("ViewLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""MTManningAgent"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
            ...
            if (listOption != null && !IsExport() && CurrentAction == "") {
                ...
                foreach (var (key, act) in ListActions.Items) {
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        string link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fMTManningAgentlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + " " + caption + "</button></li>";
                        if (!Empty(link)) {
                            links.Add(link);
                            if (Empty(body)) // Setup first button
                                body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fMTManningAgentlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + caption + "</button>";
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""MTManningAgent"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("Active")); // DN
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fMTManningAgentsrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fMTManningAgentsrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fMTManningAgentlist\"" + act.ToDataAttrs() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_MTManningAgent");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(mtManningAgentList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(mtManningAgentList.RowCount) + "_MTManningAgent");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(mtManningAgentList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && mtManningAgentList.RowType == RowType.Add || IsEdit && mtManningAgentList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        protected void LoadSearchValues() {
            ...
            // Code
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_Code[]"))
                    Code.AdvancedSearch.SearchValue = Get("x_Code[]");
                else
                    Code.AdvancedSearch.SearchValue = Get("Code"); // Default Value // DN
            if (!Empty(Code.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_Code"))
                Code.AdvancedSearch.SearchOperator = Get("z_Code");
            ...
            // Active
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_Active[]"))
                    Active.AdvancedSearch.SearchValue = Get("x_Active[]");
                else
                    Active.AdvancedSearch.SearchValue = Get("Active"); // Default Value // DN
            if (!Empty(Active.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_Active"))
                Active.AdvancedSearch.SearchOperator = Get("z_Active");
        ...
        protected Dictionary<string, object> NewRow() {
            ...
            row.Add("Active", Active.DefaultValue ?? DbNullValue); // DN
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                // Code
                if (Code.UseFilter && !Empty(Code.AdvancedSearch.SearchValue)) {
                    Code.EditValue = ConvertToString(Code.AdvancedSearch.SearchValue).Split(Config.MultipleOptionSeparator).ToList();
                }
                ...
                // Active
                if (Active.UseFilter && !Empty(Active.AdvancedSearch.SearchValue)) {
                    Active.EditValue = ConvertToString(Active.AdvancedSearch.SearchValue).Split(Config.MultipleOptionSeparator).ToList();
                }
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fMTManningAgentsrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
            ...
            if (ModalSearch && !IsMobile())
                item.Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-table=\"MTManningAgent\" data-caption=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-ew-action=\"modal\" data-url=\"" + AppPath("MtManningAgentSearch") + "\" data-btn=\"SearchBtn\">" + Language.Phrase("AdvancedSearch", false) + "</a>";
            else
                item.Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-caption=\"" + Language.Phrase("AdvancedSearch", true) + "\" href=\"" + AppPath("MtManningAgentSearch") + "\">" + Language.Phrase("AdvancedSearch", false) + "</a>";
            item.Visible = true;
