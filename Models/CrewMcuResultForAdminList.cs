...
public partial class PCM {
    /// <summary>
    /// crewMcuResultForAdminList
    /// </summary>
    public static CrewMcuResultForAdminList crewMcuResultForAdminList
    {
        get => HttpData.Get<CrewMcuResultForAdminList>("crewMcuResultForAdminList")!;
        set => HttpData["crewMcuResultForAdminList"] = value;
    }

    /// <summary>
    /// Page class for CrewMcuResultForAdmin
    /// </summary>
    public class CrewMcuResultForAdminList : CrewMcuResultForAdminListBase
    {
        // Constructor
        public CrewMcuResultForAdminList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public CrewMcuResultForAdminList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class CrewMcuResultForAdminListBase : CrewMcuResultForAdmin
    {
        // Page ID
        ...
        // Table name
        public string TableName { get; set; } = "CrewMcuResultForAdmin";

        // Page object name
        public string PageObjName = "crewMcuResultForAdminList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fCrewMcuResultForAdminlist";
        ...
        // Constructor
        public CrewMcuResultForAdminListBase()
        {
            ...
            // Table object (crewMcuResultForAdmin)
            if (crewMcuResultForAdmin == null || crewMcuResultForAdmin is CrewMcuResultForAdmin)
                crewMcuResultForAdmin = this;

            // Initialize URLs
            AddUrl = "CrewMcuResultForAdminAdd";
            ...
            MultiDeleteUrl = "CrewMcuResultForAdminDelete";
            MultiUpdateUrl = "CrewMcuResultForAdminUpdate";
        ...
        // Page name
        public string PageName => "CrewMcuResultForAdminList";
        ...
        // Constructor
        public CrewMcuResultForAdminListBase(Controller? controller = null): this() { // DN
            if (controller != null)
                ...
                if (Response != null && !Response.HasStarted) {
                    ...
                    if (IsModal) { // Show as modal
                        ...
                        if (pageName != ListUrl) { // Not List page
                            ...
                            result.Add("view", pageName == "CrewMcuResultForAdminView" ? "1" : "0"); // If View page, no primary button
                        } else { // List page
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            // Set up lookup cache
            await SetupLookupOptions(HealthStatus);

            // Update form name to avoid conflict
            if (IsModal)
                FormName = "fCrewMcuResultForAdmingrid";
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                crewMcuResultForAdminList?.PageRender();
            }
            return PageResult();
        }
        #pragma warning restore 219
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalView && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-view"" title=""{viewcaption}"" data-table=""CrewMcuResultForAdmin"" data-caption=""{viewcaption}"" data-ew-action=""modal"" data-action=""view"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(ViewUrl)) + "\" data-btn=\"null\">" + Language.Phrase("ViewLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""CrewMcuResultForAdmin"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
            ...
            if (listOption != null && !IsExport() && CurrentAction == "") {
                ...
                foreach (var (key, act) in ListActions.Items) {
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        string link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fCrewMcuResultForAdminlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + " " + caption + "</button></li>";
                        if (!Empty(link)) {
                            links.Add(link);
                            if (Empty(body)) // Setup first button
                                body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fCrewMcuResultForAdminlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + caption + "</button>";
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fCrewMcuResultForAdminsrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fCrewMcuResultForAdminsrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fCrewMcuResultForAdminlist\"" + act.ToDataAttrs() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_CrewMcuResultForAdmin");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(crewMcuResultForAdminList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(crewMcuResultForAdminList.RowCount) + "_CrewMcuResultForAdmin");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(crewMcuResultForAdminList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && crewMcuResultForAdminList.RowType == RowType.Add || IsEdit && crewMcuResultForAdminList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                if (McuAttachment.UseColorbox) {
                    ...
                    McuAttachment.LinkAttrs["data-rel"] = "CrewMcuResultForAdmin_x" + RowCount + "_McuAttachment";
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fCrewMcuResultForAdminsrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
            ...
            if (ModalSearch && !IsMobile())
                item.Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-table=\"CrewMcuResultForAdmin\" data-caption=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-ew-action=\"modal\" data-url=\"" + AppPath("CrewMcuResultForAdminSearch") + "\" data-btn=\"SearchBtn\">" + Language.Phrase("AdvancedSearch", false) + "</a>";
            else
                item.Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-caption=\"" + Language.Phrase("AdvancedSearch", true) + "\" href=\"" + AppPath("CrewMcuResultForAdminSearch") + "\">" + Language.Phrase("AdvancedSearch", false) + "</a>";
            item.Visible = true;
