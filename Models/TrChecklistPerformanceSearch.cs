...
public partial class PCM {
    /// <summary>
    /// trChecklistPerformanceSearch
    /// </summary>
    public static TrChecklistPerformanceSearch trChecklistPerformanceSearch
    {
        get => HttpData.Get<TrChecklistPerformanceSearch>("trChecklistPerformanceSearch")!;
        set => HttpData["trChecklistPerformanceSearch"] = value;
    }

    /// <summary>
    /// Page class for TRChecklistPerformance
    /// </summary>
    public class TrChecklistPerformanceSearch : TrChecklistPerformanceSearchBase
    {
        // Constructor
        public TrChecklistPerformanceSearch(Controller controller) : base(controller)
        {
        }

        // Constructor
        public TrChecklistPerformanceSearch() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class TrChecklistPerformanceSearchBase : TrChecklistPerformance
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "trChecklistPerformanceSearch";
        ...
        // Constructor
        public TrChecklistPerformanceSearchBase()
        {
            // Initialize
            CurrentPage = this;
        ...
        // Page name
        public string PageName => "TrChecklistPerformanceSearch";
        ...
        // Constructor
        public TrChecklistPerformanceSearchBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (IsSearch) {
                ...
                if (!Empty(srchStr)) {
                    srchStr = "TrChecklistPerformanceList?" + srchStr;
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                trChecklistPerformanceSearch?.PageRender();
            }
            return PageResult();
        }
        ...
        protected string BuildAdvancedSearch() {
            string srchUrl = "";
            BuildSearchUrl(ref srchUrl, ManningAgent); // ManningAgent
            BuildSearchUrl(ref srchUrl, VesselName); // VesselName
            BuildSearchUrl(ref srchUrl, Performance); // Performance
            BuildSearchUrl(ref srchUrl, DateChecked); // DateChecked
            if (!Empty(srchUrl))
                srchUrl += "&";
            srchUrl += "cmd=search";
            return srchUrl;
        ...
        protected void LoadSearchValues() {
            // ManningAgent
            if (!IsAddOrEdit)
                if (Form.ContainsKey("x_ManningAgent"))
                    ManningAgent.AdvancedSearch.SearchValue = CurrentForm.GetValue("x_ManningAgent");
            if (Form.ContainsKey("z_ManningAgent"))
                ManningAgent.AdvancedSearch.SearchOperator = CurrentForm.GetValue("z_ManningAgent");
            ...
            // Performance
            if (!IsAddOrEdit)
                if (Form.ContainsKey("x_Performance"))
                    Performance.AdvancedSearch.SearchValue = CurrentForm.GetValue("x_Performance");
            if (Form.ContainsKey("z_Performance"))
                Performance.AdvancedSearch.SearchOperator = CurrentForm.GetValue("z_Performance");

            // DateChecked
            if (!IsAddOrEdit)
                if (Form.ContainsKey("x_DateChecked"))
                    DateChecked.AdvancedSearch.SearchValue = CurrentForm.GetValue("x_DateChecked");
            if (Form.ContainsKey("z_DateChecked"))
                DateChecked.AdvancedSearch.SearchOperator = CurrentForm.GetValue("z_DateChecked");
        }
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                if (!ManningAgent.Raw)
                    ManningAgent.AdvancedSearch.SearchValue = HtmlDecode(ManningAgent.AdvancedSearch.SearchValue);
                ManningAgent.EditValue = HtmlEncode(ManningAgent.AdvancedSearch.SearchValue);
                ...
                if (!Performance.Raw)
                    Performance.AdvancedSearch.SearchValue = HtmlDecode(Performance.AdvancedSearch.SearchValue);
                Performance.EditValue = HtmlEncode(Performance.AdvancedSearch.SearchValue);
                Performance.PlaceHolder = RemoveHtml(Performance.Caption);

                // DateChecked
                DateChecked.SetupEditAttributes();
                DateChecked.EditValue = FormatDateTime(UnformatDateTime(DateChecked.AdvancedSearch.SearchValue, DateChecked.FormatPattern), DateChecked.FormatPattern); // DN
        ...
        protected bool ValidateSearch() {
            ...
            if (!Config.ServerValidate)
                return true;
            if (!CheckDate(ConvertToString(DateChecked.AdvancedSearch.SearchValue), DateChecked.FormatPattern)) {
