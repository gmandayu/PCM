...
public partial class PCM {
    /// <summary>
    /// trChecklistPerformanceList
    /// </summary>
    public static TrChecklistPerformanceList trChecklistPerformanceList
    {
        get => HttpData.Get<TrChecklistPerformanceList>("trChecklistPerformanceList")!;
        set => HttpData["trChecklistPerformanceList"] = value;
    }

    /// <summary>
    /// Page class for TRChecklistPerformance
    /// </summary>
    public class TrChecklistPerformanceList : TrChecklistPerformanceListBase
    {
        // Constructor
        public TrChecklistPerformanceList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public TrChecklistPerformanceList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class TrChecklistPerformanceListBase : TrChecklistPerformance
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "trChecklistPerformanceList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fTRChecklistPerformancelist";
        ...
        // Constructor
        public TrChecklistPerformanceListBase()
        {
            // CSS class name as context
            TableVar = "TRChecklistPerformance";
            ...
            // Initialize URLs
            AddUrl = "TrChecklistPerformanceAdd";
            ...
            MultiDeleteUrl = "TrChecklistPerformanceDelete";
            MultiUpdateUrl = "TrChecklistPerformanceUpdate";
        ...
        // Page name
        public string PageName => "TrChecklistPerformanceList";
        ...
        // Constructor
        public TrChecklistPerformanceListBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (IsModal)
                FormName = "fTRChecklistPerformancegrid";
            ...
            // Load master record
            if (CurrentMode != "add" && !Empty(MasterFilterFromSession) && CurrentMasterTable == "TRChecklist") {
                trChecklist = Resolve("TRChecklist")!;
                if (trChecklist != null) {
                    using (var rsmaster = await trChecklist.LoadReader(DbMasterFilter)) { // Note: Use {}
                        MasterRecordExists = rsmaster != null && await rsmaster.ReadAsync();
                        if (!MasterRecordExists) {
                            FailureMessage = Language.Phrase("NoRecord"); // Set no record found
                            return Terminate("TrChecklistList"); // Return to master page
                        } else {
                            trChecklist.LoadListRowValues(rsmaster);
                        }
                    }
                    trChecklist.RowType = RowType.Master; // Master row
                    await trChecklist.RenderListRow(); // Note: Do it outside "using" // DN
                }
            }

            // Load master record
            if (CurrentMode != "add" && !Empty(MasterFilterFromSession) && CurrentMasterTable == "CrewChecklistForAdmin") {
                crewChecklistForAdmin = Resolve("CrewChecklistForAdmin")!;
                if (crewChecklistForAdmin != null) {
                    using (var rsmaster = await crewChecklistForAdmin.LoadReader(DbMasterFilter)) { // Note: Use {}
                        MasterRecordExists = rsmaster != null && await rsmaster.ReadAsync();
                        if (!MasterRecordExists) {
                            FailureMessage = Language.Phrase("NoRecord"); // Set no record found
                            return Terminate("CrewChecklistForAdminList"); // Return to master page
                        } else {
                            crewChecklistForAdmin.LoadListRowValues(rsmaster);
                        }
                    }
                    crewChecklistForAdmin.RowType = RowType.Master; // Master row
                    await crewChecklistForAdmin.RenderListRow(); // Note: Do it outside "using" // DN
                }
            }
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                trChecklistPerformanceList?.PageRender();
            }
            return PageResult();
        }
        #pragma warning restore 219
        ...
        public async Task<string> GetFilterList()
        {
            ...
            filters.Merge(JObject.Parse(ManningAgent.AdvancedSearch.ToJson())); // Field ManningAgent
            filters.Merge(JObject.Parse(VesselName.AdvancedSearch.ToJson())); // Field VesselName
            filters.Merge(JObject.Parse(Performance.AdvancedSearch.ToJson())); // Field Performance
            filters.Merge(JObject.Parse(DateChecked.AdvancedSearch.ToJson())); // Field DateChecked
        ...
        protected bool RestoreFilterList() {
            ...
            // Field ManningAgent
            if (filter?.TryGetValue("x_ManningAgent", out sv) ?? false) {
                ManningAgent.AdvancedSearch.SearchValue = sv;
                ManningAgent.AdvancedSearch.SearchOperator = filter["z_ManningAgent"];
                ManningAgent.AdvancedSearch.SearchCondition = filter["v_ManningAgent"];
                ManningAgent.AdvancedSearch.SearchValue2 = filter["y_ManningAgent"];
                ManningAgent.AdvancedSearch.SearchOperator2 = filter["w_ManningAgent"];
                ManningAgent.AdvancedSearch.Save();
            }
            ...
            // Field Performance
            if (filter?.TryGetValue("x_Performance", out sv) ?? false) {
                Performance.AdvancedSearch.SearchValue = sv;
                Performance.AdvancedSearch.SearchOperator = filter["z_Performance"];
                Performance.AdvancedSearch.SearchCondition = filter["v_Performance"];
                Performance.AdvancedSearch.SearchValue2 = filter["y_Performance"];
                Performance.AdvancedSearch.SearchOperator2 = filter["w_Performance"];
                Performance.AdvancedSearch.Save();
            }

            // Field DateChecked
            if (filter?.TryGetValue("x_DateChecked", out sv) ?? false) {
                DateChecked.AdvancedSearch.SearchValue = sv;
                DateChecked.AdvancedSearch.SearchOperator = filter["z_DateChecked"];
                DateChecked.AdvancedSearch.SearchCondition = filter["v_DateChecked"];
                DateChecked.AdvancedSearch.SearchValue2 = filter["y_DateChecked"];
                DateChecked.AdvancedSearch.SearchOperator2 = filter["w_DateChecked"];
                DateChecked.AdvancedSearch.Save();
            }
        ...
        public string AdvancedSearchWhere(bool def = false) {
            ...
            if (!Security.CanSearch)
                return "";
            BuildSearchSql(ref where, ManningAgent, def, false); // ManningAgent
            BuildSearchSql(ref where, VesselName, def, false); // VesselName
            BuildSearchSql(ref where, Performance, def, false); // Performance
            BuildSearchSql(ref where, DateChecked, def, false); // DateChecked

            // Set up search command
            if (!def && !Empty(where) && (new[] { "", "reset", "resetall" }).Contains(Command))
                Command = "search";
            if (!def && Command == "search") {
                ManningAgent.AdvancedSearch.Save(); // ManningAgent
                VesselName.AdvancedSearch.Save(); // VesselName
                Performance.AdvancedSearch.Save(); // Performance
                DateChecked.AdvancedSearch.Save(); // DateChecked
        ...
        public void ShowFilterList()
        {
            ...
            // Field ManningAgent
            filter = QueryBuilderWhere("ManningAgent");
            if (Empty(filter))
                BuildSearchSql(ref filter, ManningAgent, false, false);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + ManningAgent.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field VesselName
            filter = QueryBuilderWhere("VesselName");
            if (Empty(filter))
                BuildSearchSql(ref filter, VesselName, false, false);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + VesselName.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field Performance
            filter = QueryBuilderWhere("Performance");
            if (Empty(filter))
                BuildSearchSql(ref filter, Performance, false, false);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + Performance.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field DateChecked
            filter = QueryBuilderWhere("DateChecked");
            if (Empty(filter))
                BuildSearchSql(ref filter, DateChecked, false, false);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + DateChecked.Caption + "</span>" + captionSuffix + filter + "</div>";
            if (!Empty(BasicSearch.Keyword))
        ...
        public string BasicSearchWhere(bool def = false) {
            ...
            searchFlds.Add(ManningAgent);
            searchFlds.Add(VesselName);
            searchFlds.Add(Performance);
            searchFlds.Add(DateChecked);
        ...
        protected bool CheckSearchParms() {
            ...
            if (BasicSearch.IssetSession)
                return true;
            if (ManningAgent.AdvancedSearch.IssetSession)
                return true;
            if (VesselName.AdvancedSearch.IssetSession)
                return true;
            if (Performance.AdvancedSearch.IssetSession)
                return true;
            if (DateChecked.AdvancedSearch.IssetSession)
                return true;
            return false;
        }
        ...
        protected void ResetAdvancedSearchParms() {
            ManningAgent.AdvancedSearch.UnsetSession();
            VesselName.AdvancedSearch.UnsetSession();
            Performance.AdvancedSearch.UnsetSession();
            DateChecked.AdvancedSearch.UnsetSession();
        }
        ...
        protected void RestoreSearchParms() {
            ...
            ManningAgent.AdvancedSearch.Load();
            VesselName.AdvancedSearch.Load();
            Performance.AdvancedSearch.Load();
            DateChecked.AdvancedSearch.Load();
        }
            ...
            if (listOption != null && !IsExport() && CurrentAction == "") {
                ...
                foreach (var (key, act) in ListActions.Items) {
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        string link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fTRChecklistPerformancelist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + " " + caption + "</button></li>";
                        if (!Empty(link)) {
                            links.Add(link);
                            if (Empty(body)) // Setup first button
                                body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fTRChecklistPerformancelist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + caption + "</button>";
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""TRChecklistPerformance"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("ManningAgent")); // DN
                CreateColumnOption(option.Add("VesselName")); // DN
                CreateColumnOption(option.Add("Performance")); // DN
                CreateColumnOption(option.Add("DateChecked")); // DN
            }
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fTRChecklistPerformancesrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fTRChecklistPerformancesrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fTRChecklistPerformancelist\"" + act.ToDataAttrs() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_TRChecklistPerformance");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(trChecklistPerformanceList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(trChecklistPerformanceList.RowCount) + "_TRChecklistPerformance");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(trChecklistPerformanceList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && trChecklistPerformanceList.RowType == RowType.Add || IsEdit && trChecklistPerformanceList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        protected void LoadSearchValues() {
            ...
            // ManningAgent
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_ManningAgent"))
                    ManningAgent.AdvancedSearch.SearchValue = Get("x_ManningAgent");
                else
                    ManningAgent.AdvancedSearch.SearchValue = Get("ManningAgent"); // Default Value // DN
            if (!Empty(ManningAgent.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_ManningAgent"))
                ManningAgent.AdvancedSearch.SearchOperator = Get("z_ManningAgent");
            ...
            // Performance
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_Performance"))
                    Performance.AdvancedSearch.SearchValue = Get("x_Performance");
                else
                    Performance.AdvancedSearch.SearchValue = Get("Performance"); // Default Value // DN
            if (!Empty(Performance.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_Performance"))
                Performance.AdvancedSearch.SearchOperator = Get("z_Performance");

            // DateChecked
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_DateChecked"))
                    DateChecked.AdvancedSearch.SearchValue = Get("x_DateChecked");
                else
                    DateChecked.AdvancedSearch.SearchValue = Get("DateChecked"); // Default Value // DN
            if (!Empty(DateChecked.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_DateChecked"))
                DateChecked.AdvancedSearch.SearchOperator = Get("z_DateChecked");
        }
        ...
        public async Task RenderRow()
        {
            ...
            // TRChecklistID
            TRChecklistID.CellCssStyle = "white-space: nowrap;";

            // ManningAgent
            ManningAgent.CellCssStyle = "white-space: nowrap;";

            // VesselName
            VesselName.CellCssStyle = "white-space: nowrap;";

            // Performance
            Performance.CellCssStyle = "white-space: nowrap;";

            // DateChecked
            DateChecked.CellCssStyle = "white-space: nowrap;";
        ...
        protected string GetExportTag(string type, bool custom = false) {
            ...
            if (SameText(type, "excel")) {
                if (custom)
                    return "<button type=\"button\" class=\"btn btn-default ew-export-link ew-excel\" title=\"" + HtmlEncode(Language.Phrase("ExportToExcel", true)) + "\" data-caption=\"" + HtmlEncode(Language.Phrase("ExportToExcel", true)) + "\" form=\"fTRChecklistPerformancelist\" data-url=\"" + exportUrl + "\" data-ew-action=\"export\" data-export=\"excel\" data-custom=\"true\" data-export-selected=\"false\">" + Language.Phrase("ExportToExcel") + "</button>";
                else
                    return "<a href=\"" + exportUrl + "\" class=\"btn btn-default ew-export-link ew-excel\" title=\"" + HtmlEncode(Language.Phrase("ExportToExcel", true)) + "\" data-caption=\"" + HtmlEncode(Language.Phrase("ExportToExcel", true)) + "\">" + Language.Phrase("ExportToExcel") + "</a>";
            } else if (SameText(type, "word")) {
                if (custom)
                    return "<button type=\"button\" class=\"btn btn-default ew-export-link ew-word\" title=\"" + HtmlEncode(Language.Phrase("ExportToWord", true)) + "\" data-caption=\"" + HtmlEncode(Language.Phrase("ExportToWord", true)) + "\" form=\"fTRChecklistPerformancelist\" data-url=\"" + exportUrl + "\" data-ew-action=\"export\" data-export=\"word\" data-custom=\"true\" data-export-selected=\"false\">" + Language.Phrase("ExportToWord") + "</button>";
                else
                    return "<a href=\"" + exportUrl + "\" class=\"btn btn-default ew-export-link ew-word\" title=\"" + HtmlEncode(Language.Phrase("ExportToWord", true)) + "\" data-caption=\"" + HtmlEncode(Language.Phrase("ExportToWord", true)) + "\">" + Language.Phrase("ExportToWord") + "</a>";
            } else if (SameText(type, "pdf")) {
                if (custom)
                    return "<button type=\"button\" class=\"btn btn-default ew-export-link ew-pdf\" title=\"" + HtmlEncode(Language.Phrase("ExportToPdf", true)) + "\" data-caption=\"" + HtmlEncode(Language.Phrase("ExportToPdf", true)) + "\" form=\"fTRChecklistPerformancelist\" data-url=\"" + exportUrl + "\" data-ew-action=\"export\" data-export=\"pdf\" data-custom=\"true\" data-export-selected=\"false\">" + Language.Phrase("ExportToPDF") + "</button>";
                else
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fTRChecklistPerformancesrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
            ...
            if (ModalSearch && !IsMobile())
                item.Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-table=\"TRChecklistPerformance\" data-caption=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-ew-action=\"modal\" data-url=\"" + AppPath("TrChecklistPerformanceSearch") + "\" data-btn=\"SearchBtn\">" + Language.Phrase("AdvancedSearch", false) + "</a>";
            else
                item.Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-caption=\"" + Language.Phrase("AdvancedSearch", true) + "\" href=\"" + AppPath("TrChecklistPerformanceSearch") + "\">" + Language.Phrase("AdvancedSearch", false) + "</a>";
            item.Visible = true;
        ...
        public async Task ExportData(dynamic? doc)
        {
            ...
            if (Config.ExportMasterRecord && !Empty(MasterFilterFromSession) && CurrentMasterTable == "TRChecklist") {
                trChecklist = new TrChecklistList();
                if (trChecklist != null) {
                    var c = await GetConnection2Async(trChecklist.DbId); // Note: Use new connection for master record // DN
                    using var rsmaster = await trChecklist.LoadReader(DbMasterFilter, "", c); // Load master record
                    if (rsmaster?.HasRows ?? false) { // DN
                        exportStyle = doc.Style;
                        doc.SetStyle("v"); // Change to vertical
                        if (!IsExport("csv") || Config.ExportMasterRecordForCsv) {
                            doc.Table = trChecklist;
                            await trChecklist.ExportDocument(doc, rsmaster, 1, 1);
                            doc.ExportEmptyRow();
                            doc.Table = this;
                        }
            ...
            // Export master record
            if (Config.ExportMasterRecord && !Empty(MasterFilterFromSession) && CurrentMasterTable == "CrewChecklistForAdmin") {
                crewChecklistForAdmin = new CrewChecklistForAdminList();
                if (crewChecklistForAdmin != null) {
                    var c = await GetConnection2Async(crewChecklistForAdmin.DbId); // Note: Use new connection for master record // DN
                    using var rsmaster = await crewChecklistForAdmin.LoadReader(DbMasterFilter, "", c); // Load master record
                    if (rsmaster?.HasRows ?? false) { // DN
                        exportStyle = doc.Style;
                        doc.SetStyle("v"); // Change to vertical
                        if (!IsExport("csv") || Config.ExportMasterRecordForCsv) {
                            doc.Table = crewChecklistForAdmin;
                            await crewChecklistForAdmin.ExportDocument(doc, rsmaster, 1, 1);
                            doc.ExportEmptyRow();
                            doc.Table = this;
                        }
                        doc.SetStyle(exportStyle); // Restore
