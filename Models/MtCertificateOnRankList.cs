...
public partial class PCM {
    /// <summary>
    /// mtCertificateOnRankList
    /// </summary>
    public static MtCertificateOnRankList mtCertificateOnRankList
    {
        get => HttpData.Get<MtCertificateOnRankList>("mtCertificateOnRankList")!;
        set => HttpData["mtCertificateOnRankList"] = value;
    }

    /// <summary>
    /// Page class for MTCertificateOnRank
    /// </summary>
    public class MtCertificateOnRankList : MtCertificateOnRankListBase
    {
        // Constructor
        public MtCertificateOnRankList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public MtCertificateOnRankList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class MtCertificateOnRankListBase : MtCertificateOnRank
    {
        // Page ID
        ...
        // Table name
        public string TableName { get; set; } = "MTCertificateOnRank";

        // Page object name
        public string PageObjName = "mtCertificateOnRankList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fMTCertificateOnRanklist";
        ...
        // Constructor
        public MtCertificateOnRankListBase()
        {
            // CSS class name as context
            TableVar = "MTCertificateOnRank";
            ...
            // Table object (mtCertificateOnRank)
            if (mtCertificateOnRank == null || mtCertificateOnRank is MtCertificateOnRank)
                mtCertificateOnRank = this;

            // Initialize URLs
            AddUrl = "MtCertificateOnRankAdd";
            ...
            MultiDeleteUrl = "MtCertificateOnRankDelete";
            MultiUpdateUrl = "MtCertificateOnRankUpdate";
        ...
        // Page name
        public string PageName => "MtCertificateOnRankList";
        ...
        // Constructor
        public MtCertificateOnRankListBase(Controller? controller = null): this() { // DN
            if (controller != null)
                ...
                if (Response != null && !Response.HasStarted) {
                    ...
                    if (IsModal) { // Show as modal
                        ...
                        if (pageName != ListUrl) { // Not List page
                            ...
                            result.Add("view", pageName == "MtCertificateOnRankView" ? "1" : "0"); // If View page, no primary button
                        } else { // List page
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (IsModal)
                FormName = "fMTCertificateOnRankgrid";
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                mtCertificateOnRankList?.PageRender();
            }
            return PageResult();
        }
        #pragma warning restore 219
        ...
        protected void SetupSortOrder() {
            ...
            if (Command != "json") {
                string defaultSort = MTRankID.Expression + " ASC" + ", " + MTCertificateID.Expression + " ASC"; // Set up default sort
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalView && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-view"" title=""{viewcaption}"" data-table=""MTCertificateOnRank"" data-caption=""{viewcaption}"" data-ew-action=""modal"" data-action=""view"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(ViewUrl)) + "\" data-btn=\"null\">" + Language.Phrase("ViewLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""MTCertificateOnRank"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
            ...
            if (listOption != null && !IsExport() && CurrentAction == "") {
                ...
                foreach (var (key, act) in ListActions.Items) {
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        string link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fMTCertificateOnRanklist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + " " + caption + "</button></li>";
                        if (!Empty(link)) {
                            links.Add(link);
                            if (Empty(body)) // Setup first button
                                body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fMTCertificateOnRanklist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + caption + "</button>";
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""MTCertificateOnRank"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fMTCertificateOnRanksrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fMTCertificateOnRanksrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fMTCertificateOnRanklist\"" + act.ToDataAttrs() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_MTCertificateOnRank");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(mtCertificateOnRankList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(mtCertificateOnRankList.RowCount) + "_MTCertificateOnRank");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(mtCertificateOnRankList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && mtCertificateOnRankList.RowType == RowType.Add || IsEdit && mtCertificateOnRankList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        protected void SetupSearchOptions() {
            ...
            if (ModalSearch && !IsMobile())
                item.Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-table=\"MTCertificateOnRank\" data-caption=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-ew-action=\"modal\" data-url=\"" + AppPath("MtCertificateOnRankSearch") + "\" data-btn=\"SearchBtn\">" + Language.Phrase("AdvancedSearch", false) + "</a>";
            else
                item.Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-caption=\"" + Language.Phrase("AdvancedSearch", true) + "\" href=\"" + AppPath("MtCertificateOnRankSearch") + "\">" + Language.Phrase("AdvancedSearch", false) + "</a>";
            item.Visible = true;
        ...
        {
            return MTRankID.Visible || MTCertificateID.Visible || CreatedByUserID.Visible || CreatedDateTime.Visible || LastUpdatedByUserID.Visible || LastUpdatedDateTime.Visible;
        }
