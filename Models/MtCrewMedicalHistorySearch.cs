...
public partial class PCM {
    /// <summary>
    /// mtCrewMedicalHistorySearch
    /// </summary>
    public static MtCrewMedicalHistorySearch mtCrewMedicalHistorySearch
    {
        get => HttpData.Get<MtCrewMedicalHistorySearch>("mtCrewMedicalHistorySearch")!;
        set => HttpData["mtCrewMedicalHistorySearch"] = value;
    }

    /// <summary>
    /// Page class for MTCrewMedicalHistory
    /// </summary>
    public class MtCrewMedicalHistorySearch : MtCrewMedicalHistorySearchBase
    {
        // Constructor
        public MtCrewMedicalHistorySearch(Controller controller) : base(controller)
        {
        }

        // Constructor
        public MtCrewMedicalHistorySearch() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class MtCrewMedicalHistorySearchBase : MtCrewMedicalHistory
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "mtCrewMedicalHistorySearch";
        ...
        // Constructor
        public MtCrewMedicalHistorySearchBase()
        {
            // Initialize
            CurrentPage = this;
        ...
        // Page name
        public string PageName => "MtCrewMedicalHistorySearch";
        ...
        // Constructor
        public MtCrewMedicalHistorySearchBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (IsSearch) {
                ...
                if (!Empty(srchStr)) {
                    srchStr = "MtCrewMedicalHistoryList?" + srchStr;
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                mtCrewMedicalHistorySearch?.PageRender();
            }
            return PageResult();
        }
        ...
        protected string BuildAdvancedSearch() {
            ...
            BuildSearchUrl(ref srchUrl, _Type); // Type
            BuildSearchUrl(ref srchUrl, VesselName); // VesselName
            BuildSearchUrl(ref srchUrl, DateOccurence); // DateOccurence
            BuildSearchUrl(ref srchUrl, PlaceOccurence); // PlaceOccurence
            BuildSearchUrl(ref srchUrl, PeriodOfDisability); // PeriodOfDisability
            BuildSearchUrl(ref srchUrl, PresentCondition); // PresentCondition
            BuildSearchUrl(ref srchUrl, Treatment); // Treatment
            BuildSearchUrl(ref srchUrl, Details); // Details
        ...
        protected void LoadSearchValues() {
            ...
            // _Type
            if (!IsAddOrEdit)
                if (Form.ContainsKey("x__Type"))
                    _Type.AdvancedSearch.SearchValue = CurrentForm.GetValue("x__Type");
            if (Form.ContainsKey("z__Type"))
                _Type.AdvancedSearch.SearchOperator = CurrentForm.GetValue("z__Type");
            ...
            // DateOccurence
            if (!IsAddOrEdit)
                if (Form.ContainsKey("x_DateOccurence"))
                    DateOccurence.AdvancedSearch.SearchValue = CurrentForm.GetValue("x_DateOccurence");
            if (Form.ContainsKey("z_DateOccurence"))
                DateOccurence.AdvancedSearch.SearchOperator = CurrentForm.GetValue("z_DateOccurence");

            // PlaceOccurence
            if (!IsAddOrEdit)
                if (Form.ContainsKey("x_PlaceOccurence"))
                    PlaceOccurence.AdvancedSearch.SearchValue = CurrentForm.GetValue("x_PlaceOccurence");
            if (Form.ContainsKey("z_PlaceOccurence"))
                PlaceOccurence.AdvancedSearch.SearchOperator = CurrentForm.GetValue("z_PlaceOccurence");

            // PeriodOfDisability
            if (!IsAddOrEdit)
                if (Form.ContainsKey("x_PeriodOfDisability"))
                    PeriodOfDisability.AdvancedSearch.SearchValue = CurrentForm.GetValue("x_PeriodOfDisability");
            if (Form.ContainsKey("z_PeriodOfDisability"))
                PeriodOfDisability.AdvancedSearch.SearchOperator = CurrentForm.GetValue("z_PeriodOfDisability");

            // PresentCondition
            if (!IsAddOrEdit)
                if (Form.ContainsKey("x_PresentCondition"))
                    PresentCondition.AdvancedSearch.SearchValue = CurrentForm.GetValue("x_PresentCondition");
            if (Form.ContainsKey("z_PresentCondition"))
                PresentCondition.AdvancedSearch.SearchOperator = CurrentForm.GetValue("z_PresentCondition");

            // Treatment
            if (!IsAddOrEdit)
                if (Form.ContainsKey("x_Treatment"))
                    Treatment.AdvancedSearch.SearchValue = CurrentForm.GetValue("x_Treatment");
            if (Form.ContainsKey("z_Treatment"))
                Treatment.AdvancedSearch.SearchOperator = CurrentForm.GetValue("z_Treatment");

            // Details
            if (!IsAddOrEdit)
                if (Form.ContainsKey("x_Details"))
                    Details.AdvancedSearch.SearchValue = CurrentForm.GetValue("x_Details");
            if (Form.ContainsKey("z_Details"))
                Details.AdvancedSearch.SearchOperator = CurrentForm.GetValue("z_Details");
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                DateOccurence.EditValue = FormatDateTime(UnformatDateTime(DateOccurence.AdvancedSearch.SearchValue, DateOccurence.FormatPattern), DateOccurence.FormatPattern); // DN
                DateOccurence.PlaceHolder = RemoveHtml(DateOccurence.Caption);

                // PlaceOccurence
                PlaceOccurence.SetupEditAttributes();
                if (!PlaceOccurence.Raw)
                    PlaceOccurence.AdvancedSearch.SearchValue = HtmlDecode(PlaceOccurence.AdvancedSearch.SearchValue);
                PlaceOccurence.EditValue = HtmlEncode(PlaceOccurence.AdvancedSearch.SearchValue);
                ...
                if (!PeriodOfDisability.Raw)
                    PeriodOfDisability.AdvancedSearch.SearchValue = HtmlDecode(PeriodOfDisability.AdvancedSearch.SearchValue);
                PeriodOfDisability.EditValue = HtmlEncode(PeriodOfDisability.AdvancedSearch.SearchValue);
                ...
                if (!PresentCondition.Raw)
                    PresentCondition.AdvancedSearch.SearchValue = HtmlDecode(PresentCondition.AdvancedSearch.SearchValue);
                PresentCondition.EditValue = HtmlEncode(PresentCondition.AdvancedSearch.SearchValue);
                PresentCondition.PlaceHolder = RemoveHtml(PresentCondition.Caption);

                // Treatment
                Treatment.SetupEditAttributes();
                Treatment.EditValue = Treatment.AdvancedSearch.SearchValue; // DN
                Treatment.PlaceHolder = RemoveHtml(Treatment.Caption);

                // Details
                Details.SetupEditAttributes();
                Details.EditValue = Details.AdvancedSearch.SearchValue; // DN
        ...
        protected bool ValidateSearch() {
            ...
            if (!Config.ServerValidate)
                return true;
            if (!CheckDate(ConvertToString(DateOccurence.AdvancedSearch.SearchValue), DateOccurence.FormatPattern)) {
        ...
        protected void SetupBreadcrumb() {
            ...
            breadcrumb.Add("list", TableVar, AppPath(AddMasterUrl("MtCrewMedicalHistoryList")), "", TableVar, true);
