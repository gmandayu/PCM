...
public partial class PCM {
    /// <summary>
    /// crewBankAccountForAdminViewModeList
    /// </summary>
    public static CrewBankAccountForAdminViewModeList crewBankAccountForAdminViewModeList
    {
        get => HttpData.Get<CrewBankAccountForAdminViewModeList>("crewBankAccountForAdminViewModeList")!;
        set => HttpData["crewBankAccountForAdminViewModeList"] = value;
    }

    /// <summary>
    /// Page class for CrewBankAccountForAdminViewMode
    /// </summary>
    public class CrewBankAccountForAdminViewModeList : CrewBankAccountForAdminViewModeListBase
    {
        // Constructor
        public CrewBankAccountForAdminViewModeList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public CrewBankAccountForAdminViewModeList() : base()
        {
        }
        ...
        public override void PageLoad() {
            ...
            MTBankID.DisplayValueSeparator = "/";
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class CrewBankAccountForAdminViewModeListBase : CrewBankAccountForAdminViewMode
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "crewBankAccountForAdminViewModeList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fCrewBankAccountForAdminViewModelist";
        ...
        // Constructor
        public CrewBankAccountForAdminViewModeListBase()
        {
            // CSS class name as context
            TableVar = "CrewBankAccountForAdminViewMode";
            ...
            // Initialize URLs
            AddUrl = "CrewBankAccountForAdminViewModeAdd";
            ...
            MultiDeleteUrl = "CrewBankAccountForAdminViewModeDelete";
            MultiUpdateUrl = "CrewBankAccountForAdminViewModeUpdate";
        ...
        // Page name
        public string PageName => "CrewBankAccountForAdminViewModeList";
        ...
        // Constructor
        public CrewBankAccountForAdminViewModeListBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (IsModal)
                FormName = "fCrewBankAccountForAdminViewModegrid";
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                crewBankAccountForAdminViewModeList?.PageRender();
            }
            return PageResult();
        }
        #pragma warning restore 219
        ...
        public async Task<string> GetFilterList()
        {
            ...
            filters.Merge(JObject.Parse(Beneficiary.AdvancedSearch.ToJson())); // Field Beneficiary
            filters.Merge(JObject.Parse(MTCurrencyID.AdvancedSearch.ToJson())); // Field MTCurrencyID
            filters.Merge(JObject.Parse(MainAcc.AdvancedSearch.ToJson())); // Field MainAcc
            filters.Merge(JObject.Parse(Attachment.AdvancedSearch.ToJson())); // Field Attachment
            ...
            filters.Merge(JObject.Parse(MTBankID.AdvancedSearch.ToJson())); // Field MTBankID
            filters.Merge(JObject.Parse(OtherBankName.AdvancedSearch.ToJson())); // Field OtherBankName
        ...
        protected bool RestoreFilterList() {
            ...
            // Field AccountNumber
            if (filter?.TryGetValue("x_AccountNumber", out sv) ?? false) {
            ...
            // Field Beneficiary
            if (filter?.TryGetValue("x_Beneficiary", out sv) ?? false) {
                Beneficiary.AdvancedSearch.SearchValue = sv;
                Beneficiary.AdvancedSearch.SearchOperator = filter["z_Beneficiary"];
                Beneficiary.AdvancedSearch.SearchCondition = filter["v_Beneficiary"];
                Beneficiary.AdvancedSearch.SearchValue2 = filter["y_Beneficiary"];
                Beneficiary.AdvancedSearch.SearchOperator2 = filter["w_Beneficiary"];
                Beneficiary.AdvancedSearch.Save();
            }

            // Field MTCurrencyID
            if (filter?.TryGetValue("x_MTCurrencyID", out sv) ?? false) {
                MTCurrencyID.AdvancedSearch.SearchValue = sv;
                MTCurrencyID.AdvancedSearch.SearchOperator = filter["z_MTCurrencyID"];
                MTCurrencyID.AdvancedSearch.SearchCondition = filter["v_MTCurrencyID"];
                MTCurrencyID.AdvancedSearch.SearchValue2 = filter["y_MTCurrencyID"];
                MTCurrencyID.AdvancedSearch.SearchOperator2 = filter["w_MTCurrencyID"];
                MTCurrencyID.AdvancedSearch.Save();
            }

            // Field MainAcc
            if (filter?.TryGetValue("x_MainAcc", out sv) ?? false) {
                MainAcc.AdvancedSearch.SearchValue = sv;
                MainAcc.AdvancedSearch.SearchOperator = filter["z_MainAcc"];
                MainAcc.AdvancedSearch.SearchCondition = filter["v_MainAcc"];
                MainAcc.AdvancedSearch.SearchValue2 = filter["y_MainAcc"];
                MainAcc.AdvancedSearch.SearchOperator2 = filter["w_MainAcc"];
                MainAcc.AdvancedSearch.Save();
            }

            // Field Attachment
            if (filter?.TryGetValue("x_Attachment", out sv) ?? false) {
                Attachment.AdvancedSearch.SearchValue = sv;
                Attachment.AdvancedSearch.SearchOperator = filter["z_Attachment"];
                Attachment.AdvancedSearch.SearchCondition = filter["v_Attachment"];
                Attachment.AdvancedSearch.SearchValue2 = filter["y_Attachment"];
                Attachment.AdvancedSearch.SearchOperator2 = filter["w_Attachment"];
                Attachment.AdvancedSearch.Save();
            }
            ...
            // Field MTBankID
            if (filter?.TryGetValue("x_MTBankID", out sv) ?? false) {
                MTBankID.AdvancedSearch.SearchValue = sv;
                MTBankID.AdvancedSearch.SearchOperator = filter["z_MTBankID"];
                MTBankID.AdvancedSearch.SearchCondition = filter["v_MTBankID"];
                MTBankID.AdvancedSearch.SearchValue2 = filter["y_MTBankID"];
                MTBankID.AdvancedSearch.SearchOperator2 = filter["w_MTBankID"];
                MTBankID.AdvancedSearch.Save();
            }

            // Field OtherBankName
            if (filter?.TryGetValue("x_OtherBankName", out sv) ?? false) {
                OtherBankName.AdvancedSearch.SearchValue = sv;
                OtherBankName.AdvancedSearch.SearchOperator = filter["z_OtherBankName"];
                OtherBankName.AdvancedSearch.SearchCondition = filter["v_OtherBankName"];
                OtherBankName.AdvancedSearch.SearchValue2 = filter["y_OtherBankName"];
                OtherBankName.AdvancedSearch.SearchOperator2 = filter["w_OtherBankName"];
                OtherBankName.AdvancedSearch.Save();
            }
        ...
        public string AdvancedSearchWhere(bool def = false) {
            ...
            if (!Security.CanSearch)
                return "";
            BuildSearchSql(ref where, AccountNumber, def, false); // AccountNumber
            BuildSearchSql(ref where, Beneficiary, def, false); // Beneficiary
            BuildSearchSql(ref where, MTCurrencyID, def, true); // MTCurrencyID
            BuildSearchSql(ref where, MainAcc, def, false); // MainAcc
            BuildSearchSql(ref where, Attachment, def, false); // Attachment
            ...
            BuildSearchSql(ref where, MTBankID, def, false); // MTBankID
            BuildSearchSql(ref where, OtherBankName, def, false); // OtherBankName

            // Set up search command
            if (!def && !Empty(where) && (new[] { "", "reset", "resetall" }).Contains(Command))
                Command = "search";
            if (!def && Command == "search") {
                AccountNumber.AdvancedSearch.Save(); // AccountNumber
                Beneficiary.AdvancedSearch.Save(); // Beneficiary
                MTCurrencyID.AdvancedSearch.Save(); // MTCurrencyID
                MainAcc.AdvancedSearch.Save(); // MainAcc
                Attachment.AdvancedSearch.Save(); // Attachment
                ...
                MTBankID.AdvancedSearch.Save(); // MTBankID
                OtherBankName.AdvancedSearch.Save(); // OtherBankName
        ...
        public void ShowFilterList()
        {
            ...
            // Field AccountNumber
            filter = QueryBuilderWhere("AccountNumber");
            if (Empty(filter))
                BuildSearchSql(ref filter, AccountNumber, false, false);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + AccountNumber.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field Beneficiary
            filter = QueryBuilderWhere("Beneficiary");
            if (Empty(filter))
                BuildSearchSql(ref filter, Beneficiary, false, false);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + Beneficiary.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field MTCurrencyID
            filter = QueryBuilderWhere("MTCurrencyID");
            if (Empty(filter))
                BuildSearchSql(ref filter, MTCurrencyID, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + MTCurrencyID.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field MainAcc
            filter = QueryBuilderWhere("MainAcc");
            if (Empty(filter))
                BuildSearchSql(ref filter, MainAcc, false, false);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + MainAcc.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field Attachment
            filter = QueryBuilderWhere("Attachment");
            if (Empty(filter))
                BuildSearchSql(ref filter, Attachment, false, false);
            if (!Empty(filter))
            ...
            // Field MTBankID
            filter = QueryBuilderWhere("MTBankID");
            if (Empty(filter))
                BuildSearchSql(ref filter, MTBankID, false, false);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + MTBankID.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field OtherBankName
            filter = QueryBuilderWhere("OtherBankName");
            if (Empty(filter))
                BuildSearchSql(ref filter, OtherBankName, false, false);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + OtherBankName.Caption + "</span>" + captionSuffix + filter + "</div>";
            if (!Empty(BasicSearch.Keyword))
        ...
        public string BasicSearchWhere(bool def = false) {
            ...
            searchFlds.Add(AccountNumber);
            searchFlds.Add(Beneficiary);
            searchFlds.Add(Attachment);
            searchFlds.Add(OtherBankName);
        ...
        protected bool CheckSearchParms() {
            ...
            if (BasicSearch.IssetSession)
                return true;
            if (AccountNumber.AdvancedSearch.IssetSession)
                return true;
            if (Beneficiary.AdvancedSearch.IssetSession)
                return true;
            if (MTCurrencyID.AdvancedSearch.IssetSession)
                return true;
            if (MainAcc.AdvancedSearch.IssetSession)
                return true;
            ...
            if (MTCrewID.AdvancedSearch.IssetSession)
                return true;
            if (MTBankID.AdvancedSearch.IssetSession)
                return true;
            if (OtherBankName.AdvancedSearch.IssetSession)
                return true;
            return false;
        }
        ...
        protected void ResetAdvancedSearchParms() {
            AccountNumber.AdvancedSearch.UnsetSession();
            Beneficiary.AdvancedSearch.UnsetSession();
            MTCurrencyID.AdvancedSearch.UnsetSession();
            MainAcc.AdvancedSearch.UnsetSession();
            ...
            MTBankID.AdvancedSearch.UnsetSession();
            OtherBankName.AdvancedSearch.UnsetSession();
        }
        ...
        protected void RestoreSearchParms() {
            ...
            Beneficiary.AdvancedSearch.Load();
            MTCurrencyID.AdvancedSearch.Load();
            MainAcc.AdvancedSearch.Load();
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(AccountNumber, ctrl); // AccountNumber
                UpdateSort(Beneficiary, ctrl); // Beneficiary
                UpdateSort(MTCurrencyID, ctrl); // MTCurrencyID
                UpdateSort(MainAcc, ctrl); // MainAcc
                ...
                UpdateSort(MTBankID, ctrl); // MTBankID
                UpdateSort(OtherBankName, ctrl); // OtherBankName
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalView && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-view"" title=""{viewcaption}"" data-table=""CrewBankAccountForAdminViewMode"" data-caption=""{viewcaption}"" data-ew-action=""modal"" data-action=""view"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(ViewUrl)) + "\" data-btn=\"null\">" + Language.Phrase("ViewLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""CrewBankAccountForAdminViewMode"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
            ...
            if (listOption != null && !IsExport() && CurrentAction == "") {
                ...
                foreach (var (key, act) in ListActions.Items) {
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        string link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fCrewBankAccountForAdminViewModelist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + " " + caption + "</button></li>";
                        if (!Empty(link)) {
                            links.Add(link);
                            if (Empty(body)) // Setup first button
                                body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fCrewBankAccountForAdminViewModelist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + caption + "</button>";
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""CrewBankAccountForAdminViewMode"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("AccountNumber")); // DN
                CreateColumnOption(option.Add("Beneficiary")); // DN
                CreateColumnOption(option.Add("MTCurrencyID")); // DN
                CreateColumnOption(option.Add("MainAcc")); // DN
                ...
                CreateColumnOption(option.Add("MTBankID")); // DN
                CreateColumnOption(option.Add("OtherBankName")); // DN
            }
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fCrewBankAccountForAdminViewModesrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fCrewBankAccountForAdminViewModesrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fCrewBankAccountForAdminViewModelist\"" + act.ToDataAttrs() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_CrewBankAccountForAdminViewMode");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(crewBankAccountForAdminViewModeList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(crewBankAccountForAdminViewModeList.RowCount) + "_CrewBankAccountForAdminViewMode");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(crewBankAccountForAdminViewModeList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && crewBankAccountForAdminViewModeList.RowType == RowType.Add || IsEdit && crewBankAccountForAdminViewModeList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        protected void LoadSearchValues() {
            ...
            // AccountNumber
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_AccountNumber"))
                    AccountNumber.AdvancedSearch.SearchValue = Get("x_AccountNumber");
                else
                    AccountNumber.AdvancedSearch.SearchValue = Get("AccountNumber"); // Default Value // DN
            if (!Empty(AccountNumber.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_AccountNumber"))
                AccountNumber.AdvancedSearch.SearchOperator = Get("z_AccountNumber");

            // Beneficiary
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_Beneficiary"))
                    Beneficiary.AdvancedSearch.SearchValue = Get("x_Beneficiary");
                else
                    Beneficiary.AdvancedSearch.SearchValue = Get("Beneficiary"); // Default Value // DN
            if (!Empty(Beneficiary.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_Beneficiary"))
                Beneficiary.AdvancedSearch.SearchOperator = Get("z_Beneficiary");

            // MTCurrencyID
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_MTCurrencyID[]"))
                    MTCurrencyID.AdvancedSearch.SearchValue = Get("x_MTCurrencyID[]");
                else
                    MTCurrencyID.AdvancedSearch.SearchValue = Get("MTCurrencyID"); // Default Value // DN
            if (!Empty(MTCurrencyID.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_MTCurrencyID"))
                MTCurrencyID.AdvancedSearch.SearchOperator = Get("z_MTCurrencyID");

            // MainAcc
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_MainAcc"))
                    MainAcc.AdvancedSearch.SearchValue = Get("x_MainAcc");
                else
                    MainAcc.AdvancedSearch.SearchValue = Get("MainAcc"); // Default Value // DN
            if (!Empty(MainAcc.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_MainAcc"))
                MainAcc.AdvancedSearch.SearchOperator = Get("z_MainAcc");
            ...
            // MTBankID
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_MTBankID"))
                    MTBankID.AdvancedSearch.SearchValue = Get("x_MTBankID");
                else
                    MTBankID.AdvancedSearch.SearchValue = Get("MTBankID"); // Default Value // DN
            if (!Empty(MTBankID.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_MTBankID"))
                MTBankID.AdvancedSearch.SearchOperator = Get("z_MTBankID");

            // OtherBankName
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_OtherBankName"))
                    OtherBankName.AdvancedSearch.SearchValue = Get("x_OtherBankName");
                else
                    OtherBankName.AdvancedSearch.SearchValue = Get("OtherBankName"); // Default Value // DN
            if (!Empty(OtherBankName.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_OtherBankName"))
                OtherBankName.AdvancedSearch.SearchOperator = Get("z_OtherBankName");
        }
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                if (Attachment.UseColorbox) {
                    ...
                    Attachment.LinkAttrs["data-rel"] = "CrewBankAccountForAdminViewMode_x" + RowCount + "_Attachment";
                ...
                if (!AccountNumber.Raw)
                    AccountNumber.AdvancedSearch.SearchValue = HtmlDecode(AccountNumber.AdvancedSearch.SearchValue);
                AccountNumber.EditValue = HtmlEncode(AccountNumber.AdvancedSearch.SearchValue);
                AccountNumber.PlaceHolder = RemoveHtml(AccountNumber.Caption);

                // Beneficiary
                Beneficiary.SetupEditAttributes();
                if (!Beneficiary.Raw)
                    Beneficiary.AdvancedSearch.SearchValue = HtmlDecode(Beneficiary.AdvancedSearch.SearchValue);
                Beneficiary.EditValue = HtmlEncode(Beneficiary.AdvancedSearch.SearchValue);
                Beneficiary.PlaceHolder = RemoveHtml(Beneficiary.Caption);

                // MTCurrencyID
                if (MTCurrencyID.UseFilter && !Empty(MTCurrencyID.AdvancedSearch.SearchValue)) {
                    MTCurrencyID.EditValue = ConvertToString(MTCurrencyID.AdvancedSearch.SearchValue).Split(Config.MultipleOptionSeparator).ToList();
                }
                ...
                if (!Attachment.Raw)
                    Attachment.AdvancedSearch.SearchValue = HtmlDecode(Attachment.AdvancedSearch.SearchValue);
                Attachment.EditValue = HtmlEncode(Attachment.AdvancedSearch.SearchValue);
                Attachment.PlaceHolder = RemoveHtml(Attachment.Caption);
                ...
                if (!OtherBankName.Raw)
                    OtherBankName.AdvancedSearch.SearchValue = HtmlDecode(OtherBankName.AdvancedSearch.SearchValue);
                OtherBankName.EditValue = HtmlEncode(OtherBankName.AdvancedSearch.SearchValue);
        ...
        protected string GetExportTag(string type, bool custom = false) {
            ...
            if (SameText(type, "excel")) {
                if (custom)
                    return "<button type=\"button\" class=\"btn btn-default ew-export-link ew-excel\" title=\"" + HtmlEncode(Language.Phrase("ExportToExcel", true)) + "\" data-caption=\"" + HtmlEncode(Language.Phrase("ExportToExcel", true)) + "\" form=\"fCrewBankAccountForAdminViewModelist\" data-url=\"" + exportUrl + "\" data-ew-action=\"export\" data-export=\"excel\" data-custom=\"true\" data-export-selected=\"false\">" + Language.Phrase("ExportToExcel") + "</button>";
                else
                    return "<a href=\"" + exportUrl + "\" class=\"btn btn-default ew-export-link ew-excel\" title=\"" + HtmlEncode(Language.Phrase("ExportToExcel", true)) + "\" data-caption=\"" + HtmlEncode(Language.Phrase("ExportToExcel", true)) + "\">" + Language.Phrase("ExportToExcel") + "</a>";
            } else if (SameText(type, "word")) {
                if (custom)
                    return "<button type=\"button\" class=\"btn btn-default ew-export-link ew-word\" title=\"" + HtmlEncode(Language.Phrase("ExportToWord", true)) + "\" data-caption=\"" + HtmlEncode(Language.Phrase("ExportToWord", true)) + "\" form=\"fCrewBankAccountForAdminViewModelist\" data-url=\"" + exportUrl + "\" data-ew-action=\"export\" data-export=\"word\" data-custom=\"true\" data-export-selected=\"false\">" + Language.Phrase("ExportToWord") + "</button>";
                else
                    return "<a href=\"" + exportUrl + "\" class=\"btn btn-default ew-export-link ew-word\" title=\"" + HtmlEncode(Language.Phrase("ExportToWord", true)) + "\" data-caption=\"" + HtmlEncode(Language.Phrase("ExportToWord", true)) + "\">" + Language.Phrase("ExportToWord") + "</a>";
            } else if (SameText(type, "pdf")) {
                if (custom)
                    return "<button type=\"button\" class=\"btn btn-default ew-export-link ew-pdf\" title=\"" + HtmlEncode(Language.Phrase("ExportToPdf", true)) + "\" data-caption=\"" + HtmlEncode(Language.Phrase("ExportToPdf", true)) + "\" form=\"fCrewBankAccountForAdminViewModelist\" data-url=\"" + exportUrl + "\" data-ew-action=\"export\" data-export=\"pdf\" data-custom=\"true\" data-export-selected=\"false\">" + Language.Phrase("ExportToPDF") + "</button>";
                else
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fCrewBankAccountForAdminViewModesrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
            ...
            if (ModalSearch && !IsMobile())
                item.Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-table=\"CrewBankAccountForAdminViewMode\" data-caption=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-ew-action=\"modal\" data-url=\"" + AppPath("CrewBankAccountForAdminViewModeSearch") + "\" data-btn=\"SearchBtn\">" + Language.Phrase("AdvancedSearch", false) + "</a>";
            else
                item.Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-caption=\"" + Language.Phrase("AdvancedSearch", true) + "\" href=\"" + AppPath("CrewBankAccountForAdminViewModeSearch") + "\">" + Language.Phrase("AdvancedSearch", false) + "</a>";
            item.Visible = true;
