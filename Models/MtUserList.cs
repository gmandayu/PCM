...
public partial class PCM {
    ...
    public class MtUserListBase : MtUser
    {
        ...
        public string FormName = "fMTUserlist";
        ...
        protected bool RestoreFilterList() {
            ...
            // Field MTUserLevelID
            if (filter?.TryGetValue("x_MTUserLevelID", out sv) ?? false) {
                MTUserLevelID.AdvancedSearch.SearchValue = sv;
                MTUserLevelID.AdvancedSearch.SearchOperator = filter["z_MTUserLevelID"];
                MTUserLevelID.AdvancedSearch.SearchCondition = filter["v_MTUserLevelID"];
                MTUserLevelID.AdvancedSearch.SearchValue2 = filter["y_MTUserLevelID"];
                MTUserLevelID.AdvancedSearch.SearchOperator2 = filter["w_MTUserLevelID"];
                MTUserLevelID.AdvancedSearch.Save();
            }

            // Field SeafarerID
            if (filter?.TryGetValue("x_SeafarerID", out sv) ?? false) {
                SeafarerID.AdvancedSearch.SearchValue = sv;
                SeafarerID.AdvancedSearch.SearchOperator = filter["z_SeafarerID"];
                SeafarerID.AdvancedSearch.SearchCondition = filter["v_SeafarerID"];
                SeafarerID.AdvancedSearch.SearchValue2 = filter["y_SeafarerID"];
                SeafarerID.AdvancedSearch.SearchOperator2 = filter["w_SeafarerID"];
        ...
        public void ShowFilterList()
        {
            ...
            // Field MTUserLevelID
            filter = QueryBuilderWhere("MTUserLevelID");
            if (Empty(filter))
                BuildSearchSql(ref filter, MTUserLevelID, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + MTUserLevelID.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field SeafarerID
            filter = QueryBuilderWhere("SeafarerID");
            if (Empty(filter))
                BuildSearchSql(ref filter, SeafarerID, false, true);
            if (!Empty(filter))
        ...
        protected bool CheckSearchParms() {
            ...
            if (FullName.AdvancedSearch.IssetSession)
                return true;
            if (MTUserLevelID.AdvancedSearch.IssetSession)
                return true;
        ...
        protected void SetupSortOrder() {
            ...
            if (Command != "json") {
                string defaultSort = _Email.Expression + " ASC"; // Set up default sort
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalView && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-view"" title=""{viewcaption}"" data-table=""MTUser"" data-caption=""{viewcaption}"" data-ew-action=""modal"" data-action=""view"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(ViewUrl)) + "\" data-btn=\"null\">" + Language.Phrase("ViewLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""MTUser"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
            ...
            if (listOption != null && !IsExport() && CurrentAction == "") {
                ...
                foreach (var (key, act) in ListActions.Items) {
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        string link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fMTUserlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + " " + caption + "</button></li>";
                        if (!Empty(link)) {
                            links.Add(link);
                            if (Empty(body)) // Setup first button
                                body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fMTUserlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + caption + "</button>";
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""MTUser"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fMTUsersrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fMTUsersrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fMTUserlist\"" + act.ToDataAttrs() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            ...
            RowAttrs.Add("id", "r" + ConvertToString(mtUserList.RowCount) + "_MTUser");
        ...
        protected void LoadSearchValues() {
            ...
            // MTUserLevelID
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_MTUserLevelID[]"))
                    MTUserLevelID.AdvancedSearch.SearchValue = Get("x_MTUserLevelID[]");
                else
                    MTUserLevelID.AdvancedSearch.SearchValue = Get("MTUserLevelID"); // Default Value // DN
            if (!Empty(MTUserLevelID.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_MTUserLevelID"))
                MTUserLevelID.AdvancedSearch.SearchOperator = Get("z_MTUserLevelID");

            // SeafarerID
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_SeafarerID[]"))
                    SeafarerID.AdvancedSearch.SearchValue = Get("x_SeafarerID[]");
                else
                    SeafarerID.AdvancedSearch.SearchValue = Get("SeafarerID"); // Default Value // DN
            if (!Empty(SeafarerID.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
        ...
        public async Task RenderRow()
        {
            ...
            // MTUserLevelID
            MTUserLevelID.CellCssStyle = "white-space: nowrap;";

            // SeafarerID
            SeafarerID.CellCssStyle = "white-space: nowrap;";

            // IdentificationImage
            IdentificationImage.CellCssStyle = "white-space: nowrap;";
            ...
            if (RowType == RowType.View) {
                ...
                // MTUserLevelID
                if (MTUserLevelID.UseFilter && !Empty(MTUserLevelID.AdvancedSearch.SearchValue)) {
                    MTUserLevelID.EditValue = ConvertToString(MTUserLevelID.AdvancedSearch.SearchValue).Split(Config.MultipleOptionSeparator).ToList();
                }

                // SeafarerID
                if (SeafarerID.UseFilter && !Empty(SeafarerID.AdvancedSearch.SearchValue)) {
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fMTUsersrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
            ...
            if (ModalSearch && !IsMobile())
                item.Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-table=\"MTUser\" data-caption=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-ew-action=\"modal\" data-url=\"" + AppPath("MtUserSearch") + "\" data-btn=\"SearchBtn\">" + Language.Phrase("AdvancedSearch", false) + "</a>";
            else
                item.Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-caption=\"" + Language.Phrase("AdvancedSearch", true) + "\" href=\"" + AppPath("MtUserSearch") + "\">" + Language.Phrase("AdvancedSearch", false) + "</a>";
            item.Visible = true;
