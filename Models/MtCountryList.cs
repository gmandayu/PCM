...
public partial class PCM {
    ...
    /// mtCountryList
    /// </summary>
    public static MtCountryList mtCountryList
    {
        get => HttpData.Get<MtCountryList>("mtCountryList")!;
        set => HttpData["mtCountryList"] = value;
    }

    /// <summary>
    /// Page class for MTCountry
    /// </summary>
    public class MtCountryList : MtCountryListBase
    {
        // Constructor
        public MtCountryList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public MtCountryList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class MtCountryListBase : MtCountry
    {
        // Page ID
        ...
        // Table name
        public string TableName { get; set; } = "MTCountry";

        // Page object name
        public string PageObjName = "mtCountryList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fMTCountrylist";
        ...
        public MtCountryListBase()
        {
            ...
            // Table object (mtCountry)
            if (mtCountry == null || mtCountry is MtCountry)
                mtCountry = this;
            ...
            MultiDeleteUrl = "MtCountryDelete";
            MultiUpdateUrl = "MtCountryUpdate";
        ...
        // Page name
        public string PageName => "MtCountryList";
        ...
        public void SetVisibility()
        {
            ID.Visible = false;
            _Name.SetVisibility();
            Nationality.SetVisibility();
            CallingCode.SetVisibility();
        ...
        // Constructor
        public MtCountryListBase(Controller? controller = null): this() { // DN
            if (controller != null)
                ...
                if (Response != null && !Response.HasStarted) {
                    ...
                    if (IsModal) { // Show as modal
                        ...
                        if (pageName != ListUrl) { // Not List page
                            ...
                            result.Add("view", pageName == "MtCountryView" ? "1" : "0"); // If View page, no primary button
                        } else { // List page
        ...
        public async Task<string> GetFilterList()
        {
            ...
            filters.Merge(JObject.Parse(Nationality.AdvancedSearch.ToJson())); // Field Nationality
            filters.Merge(JObject.Parse(CallingCode.AdvancedSearch.ToJson())); // Field CallingCode
        ...
        protected bool RestoreFilterList() {
            ...
            // Field Nationality
            if (filter?.TryGetValue("x_Nationality", out sv) ?? false) {
                Nationality.AdvancedSearch.SearchValue = sv;
                Nationality.AdvancedSearch.SearchOperator = filter["z_Nationality"];
                Nationality.AdvancedSearch.SearchCondition = filter["v_Nationality"];
                Nationality.AdvancedSearch.SearchValue2 = filter["y_Nationality"];
                Nationality.AdvancedSearch.SearchOperator2 = filter["w_Nationality"];
                Nationality.AdvancedSearch.Save();
            }

            // Field CallingCode
            if (filter?.TryGetValue("x_CallingCode", out sv) ?? false) {
                CallingCode.AdvancedSearch.SearchValue = sv;
                CallingCode.AdvancedSearch.SearchOperator = filter["z_CallingCode"];
                CallingCode.AdvancedSearch.SearchCondition = filter["v_CallingCode"];
                CallingCode.AdvancedSearch.SearchValue2 = filter["y_CallingCode"];
                CallingCode.AdvancedSearch.SearchOperator2 = filter["w_CallingCode"];
                CallingCode.AdvancedSearch.Save();
            }
        ...
        public string AdvancedSearchWhere(bool def = false) {
            ...
            BuildSearchSql(ref where, Nationality, def, true); // Nationality
            BuildSearchSql(ref where, CallingCode, def, true); // CallingCode
            ...
            if (!def && Command == "search") {
                ...
                Nationality.AdvancedSearch.Save(); // Nationality
                CallingCode.AdvancedSearch.Save(); // CallingCode
        ...
        public void ShowFilterList()
        {
            ...
            // Field Nationality
            filter = QueryBuilderWhere("Nationality");
            if (Empty(filter))
                BuildSearchSql(ref filter, Nationality, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + Nationality.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field CallingCode
            filter = QueryBuilderWhere("CallingCode");
            if (Empty(filter))
                BuildSearchSql(ref filter, CallingCode, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + CallingCode.Caption + "</span>" + captionSuffix + filter + "</div>";
        ...
        public string BasicSearchWhere(bool def = false) {
            ...
            searchFlds.Add(Nationality);
            searchFlds.Add(CallingCode);
        ...
        protected bool CheckSearchParms() {
            ...
            if (_Name.AdvancedSearch.IssetSession)
                return true;
            if (Nationality.AdvancedSearch.IssetSession)
                return true;
            if (CallingCode.AdvancedSearch.IssetSession)
                return true;
        ...
        protected void ResetAdvancedSearchParms() {
            ...
            Nationality.AdvancedSearch.UnsetSession();
            CallingCode.AdvancedSearch.UnsetSession();
        ...
        protected void RestoreSearchParms() {
            ...
            Nationality.AdvancedSearch.Load();
            CallingCode.AdvancedSearch.Load();
        ...
        protected void SetupSortOrder() {
            ...
            if (Command != "json") {
                string defaultSort = _Name.Expression + " ASC"; // Set up default sort
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(Nationality, ctrl); // Nationality
                UpdateSort(CallingCode, ctrl); // CallingCode
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalView && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-view"" title=""{viewcaption}"" data-table=""MTCountry"" data-caption=""{viewcaption}"" data-ew-action=""modal"" data-action=""view"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(ViewUrl)) + "\" data-btn=\"null\">" + Language.Phrase("ViewLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""MTCountry"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
            ...
            if (listOption != null && !IsExport() && CurrentAction == "") {
                ...
                foreach (var (key, act) in ListActions.Items) {
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        string link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fMTCountrylist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + " " + caption + "</button></li>";
                        if (!Empty(link)) {
                            links.Add(link);
                            if (Empty(body)) // Setup first button
                                body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fMTCountrylist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + caption + "</button>";
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""MTCountry"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("Nationality")); // DN
                CreateColumnOption(option.Add("CallingCode")); // DN
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fMTCountrysrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fMTCountrysrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fMTCountrylist\"" + act.ToDataAttrs() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(mtCountryList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(mtCountryList.RowCount) + "_MTCountry");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(mtCountryList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && mtCountryList.RowType == RowType.Add || IsEdit && mtCountryList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        protected void LoadSearchValues() {
            ...
            // Nationality
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_Nationality[]"))
                    Nationality.AdvancedSearch.SearchValue = Get("x_Nationality[]");
                else
                    Nationality.AdvancedSearch.SearchValue = Get("Nationality"); // Default Value // DN
            if (!Empty(Nationality.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_Nationality"))
                Nationality.AdvancedSearch.SearchOperator = Get("z_Nationality");

            // CallingCode
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_CallingCode[]"))
                    CallingCode.AdvancedSearch.SearchValue = Get("x_CallingCode[]");
                else
                    CallingCode.AdvancedSearch.SearchValue = Get("CallingCode"); // Default Value // DN
            if (!Empty(CallingCode.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_CallingCode"))
                CallingCode.AdvancedSearch.SearchOperator = Get("z_CallingCode");
        ...
        public async Task LoadRowValues(Dictionary<string, object>? row)
        {
            ...
            Nationality.SetDbValue(row["Nationality"]);
            CallingCode.SetDbValue(row["CallingCode"]);
        ...
        protected Dictionary<string, object> NewRow() {
            ...
            row.Add("Nationality", Nationality.DefaultValue ?? DbNullValue); // DN
            row.Add("CallingCode", CallingCode.DefaultValue ?? DbNullValue); // DN
        ...
        public async Task RenderRow()
        {
            ...
            // Nationality
            Nationality.CellCssStyle = "white-space: nowrap;";

            // CallingCode
            CallingCode.CellCssStyle = "white-space: nowrap;";
            ...
            if (RowType == RowType.View) {
                ...
                // Nationality
                Nationality.ViewValue = ConvertToString(Nationality.CurrentValue); // DN
                Nationality.ViewCustomAttributes = "";

                // CallingCode
                CallingCode.ViewValue = ConvertToString(CallingCode.CurrentValue); // DN
                CallingCode.ViewCustomAttributes = "";
                ...
                // Nationality
                Nationality.HrefValue = "";
                Nationality.TooltipValue = "";

                // CallingCode
                CallingCode.HrefValue = "";
                CallingCode.TooltipValue = "";
                ...
                // Nationality
                if (Nationality.UseFilter && !Empty(Nationality.AdvancedSearch.SearchValue)) {
                    Nationality.EditValue = ConvertToString(Nationality.AdvancedSearch.SearchValue).Split(Config.MultipleOptionSeparator).ToList();
                }

                // CallingCode
                if (CallingCode.UseFilter && !Empty(CallingCode.AdvancedSearch.SearchValue)) {
                    CallingCode.EditValue = ConvertToString(CallingCode.AdvancedSearch.SearchValue).Split(Config.MultipleOptionSeparator).ToList();
                }
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fMTCountrysrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
            ...
            if (ModalSearch && !IsMobile())
                item.Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-table=\"MTCountry\" data-caption=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-ew-action=\"modal\" data-url=\"" + AppPath("MtCountrySearch") + "\" data-btn=\"SearchBtn\">" + Language.Phrase("AdvancedSearch", false) + "</a>";
            else
                item.Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-caption=\"" + Language.Phrase("AdvancedSearch", true) + "\" href=\"" + AppPath("MtCountrySearch") + "\">" + Language.Phrase("AdvancedSearch", false) + "</a>";
            item.Visible = true;
