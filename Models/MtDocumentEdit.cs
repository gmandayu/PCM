...
public partial class PCM {
    ...
    /// mtDocumentEdit
    /// </summary>
    public static MtDocumentEdit mtDocumentEdit
    {
        get => HttpData.Get<MtDocumentEdit>("mtDocumentEdit")!;
        set => HttpData["mtDocumentEdit"] = value;
    }

    /// <summary>
    /// Page class for MTDocument
    /// </summary>
    public class MtDocumentEdit : MtDocumentEditBase
    {
        // Constructor
        public MtDocumentEdit(Controller controller) : base(controller)
        {
        }

        // Constructor
        public MtDocumentEdit() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class MtDocumentEditBase : MtDocument
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "mtDocumentEdit";
        ...
        // Page name
        public string PageName => "MtDocumentEdit";
        ...
        // Constructor
        public MtDocumentEditBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        switch (CurrentAction) {
                case "show": // Get a record to display
                        if (!loaded) { // Load record based on key
                            ...
                            return Terminate("MtDocumentList"); // No matching record, return to list
                        }
                    break;
        ...
        protected async Task<JsonBoolResult> EditRow() { // DN
            ...
            // Check field with unique index (Type)
            if (!Empty(_Type.CurrentValue)) {
                string filterChk = "([Type] = '" + AdjustSql(_Type.CurrentValue, DbId) + "')";
                filterChk = filterChk + " AND NOT (" + filter + ")";
                try {
                    using var rschk = await LoadReader(filterChk);
                    if (rschk?.HasRows ?? false) {
                        var idxErrMsg = Language.Phrase("DupIndex").Replace("%f", _Type.Caption);
                        idxErrMsg = idxErrMsg.Replace("%v", ConvertToString(_Type.CurrentValue));
