...
public partial class PCM {
    /// <summary>
    /// crewAppraisalForAdminViewModeList
    /// </summary>
    public static CrewAppraisalForAdminViewModeList crewAppraisalForAdminViewModeList
    {
        get => HttpData.Get<CrewAppraisalForAdminViewModeList>("crewAppraisalForAdminViewModeList")!;
        set => HttpData["crewAppraisalForAdminViewModeList"] = value;
    }

    /// <summary>
    /// Page class for CrewAppraisalForAdminViewMode
    /// </summary>
    public class CrewAppraisalForAdminViewModeList : CrewAppraisalForAdminViewModeListBase
    {
        // Constructor
        public CrewAppraisalForAdminViewModeList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public CrewAppraisalForAdminViewModeList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class CrewAppraisalForAdminViewModeListBase : CrewAppraisalForAdminViewMode
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "crewAppraisalForAdminViewModeList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fCrewAppraisalForAdminViewModelist";
        ...
        // Constructor
        public CrewAppraisalForAdminViewModeListBase()
        {
            ...
            // Initialize URLs
            AddUrl = "CrewAppraisalForAdminViewModeAdd";
            ...
            MultiDeleteUrl = "CrewAppraisalForAdminViewModeDelete";
            MultiUpdateUrl = "CrewAppraisalForAdminViewModeUpdate";
        ...
        // Page name
        public string PageName => "CrewAppraisalForAdminViewModeList";
        ...
        // Constructor
        public CrewAppraisalForAdminViewModeListBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (IsModal)
                FormName = "fCrewAppraisalForAdminViewModegrid";
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                crewAppraisalForAdminViewModeList?.PageRender();
            }
            return PageResult();
        }
        #pragma warning restore 219
        ...
        protected void ClearInlineMode() {
            AverageRating.SetFormValue("", false); // Clear form value
        ...
        public async Task<string> GetFilterList()
        {
            ...
            filters.Merge(JObject.Parse(Questionnaire.AdvancedSearch.ToJson())); // Field Questionnaire
            filters.Merge(JObject.Parse(Approved.AdvancedSearch.ToJson())); // Field Approved
            filters.Merge(JObject.Parse(AverageRating.AdvancedSearch.ToJson())); // Field AverageRating
            filters.Merge(JObject.Parse(Rehire.AdvancedSearch.ToJson())); // Field Rehire
            filters.Merge(JObject.Parse(Promote.AdvancedSearch.ToJson())); // Field Promote
            filters.Merge(JObject.Parse(Appraiser.AdvancedSearch.ToJson())); // Field Appraiser
            filters.Merge(JObject.Parse(DateInput.AdvancedSearch.ToJson())); // Field DateInput
        ...
        protected bool RestoreFilterList() {
            ...
            // Field Questionnaire
            if (filter?.TryGetValue("x_Questionnaire", out sv) ?? false) {
                Questionnaire.AdvancedSearch.SearchValue = sv;
                Questionnaire.AdvancedSearch.SearchOperator = filter["z_Questionnaire"];
                Questionnaire.AdvancedSearch.SearchCondition = filter["v_Questionnaire"];
                Questionnaire.AdvancedSearch.SearchValue2 = filter["y_Questionnaire"];
                Questionnaire.AdvancedSearch.SearchOperator2 = filter["w_Questionnaire"];
                Questionnaire.AdvancedSearch.Save();
            }
            ...
            // Field AverageRating
            if (filter?.TryGetValue("x_AverageRating", out sv) ?? false) {
                AverageRating.AdvancedSearch.SearchValue = sv;
                AverageRating.AdvancedSearch.SearchOperator = filter["z_AverageRating"];
                AverageRating.AdvancedSearch.SearchCondition = filter["v_AverageRating"];
                AverageRating.AdvancedSearch.SearchValue2 = filter["y_AverageRating"];
                AverageRating.AdvancedSearch.SearchOperator2 = filter["w_AverageRating"];
                AverageRating.AdvancedSearch.Save();
            }

            // Field Rehire
            if (filter?.TryGetValue("x_Rehire", out sv) ?? false) {
                Rehire.AdvancedSearch.SearchValue = sv;
                Rehire.AdvancedSearch.SearchOperator = filter["z_Rehire"];
                Rehire.AdvancedSearch.SearchCondition = filter["v_Rehire"];
                Rehire.AdvancedSearch.SearchValue2 = filter["y_Rehire"];
                Rehire.AdvancedSearch.SearchOperator2 = filter["w_Rehire"];
                Rehire.AdvancedSearch.Save();
            }

            // Field Promote
            if (filter?.TryGetValue("x_Promote", out sv) ?? false) {
                Promote.AdvancedSearch.SearchValue = sv;
                Promote.AdvancedSearch.SearchOperator = filter["z_Promote"];
                Promote.AdvancedSearch.SearchCondition = filter["v_Promote"];
                Promote.AdvancedSearch.SearchValue2 = filter["y_Promote"];
                Promote.AdvancedSearch.SearchOperator2 = filter["w_Promote"];
                Promote.AdvancedSearch.Save();
            }

            // Field Appraiser
            if (filter?.TryGetValue("x_Appraiser", out sv) ?? false) {
                Appraiser.AdvancedSearch.SearchValue = sv;
                Appraiser.AdvancedSearch.SearchOperator = filter["z_Appraiser"];
                Appraiser.AdvancedSearch.SearchCondition = filter["v_Appraiser"];
                Appraiser.AdvancedSearch.SearchValue2 = filter["y_Appraiser"];
                Appraiser.AdvancedSearch.SearchOperator2 = filter["w_Appraiser"];
                Appraiser.AdvancedSearch.Save();
            }

            // Field DateInput
            if (filter?.TryGetValue("x_DateInput", out sv) ?? false) {
                DateInput.AdvancedSearch.SearchValue = sv;
                DateInput.AdvancedSearch.SearchOperator = filter["z_DateInput"];
                DateInput.AdvancedSearch.SearchCondition = filter["v_DateInput"];
                DateInput.AdvancedSearch.SearchValue2 = filter["y_DateInput"];
                DateInput.AdvancedSearch.SearchOperator2 = filter["w_DateInput"];
                DateInput.AdvancedSearch.Save();
            }
        ...
        public string AdvancedSearchWhere(bool def = false) {
            ...
            BuildSearchSql(ref where, Questionnaire, def, true); // Questionnaire
            BuildSearchSql(ref where, Approved, def, true); // Approved
            BuildSearchSql(ref where, AverageRating, def, true); // AverageRating
            BuildSearchSql(ref where, Rehire, def, true); // Rehire
            BuildSearchSql(ref where, Promote, def, true); // Promote
            BuildSearchSql(ref where, Appraiser, def, true); // Appraiser
            BuildSearchSql(ref where, DateInput, def, true); // DateInput
            ...
            if (!def && Command == "search") {
                ...
                Questionnaire.AdvancedSearch.Save(); // Questionnaire
                Approved.AdvancedSearch.Save(); // Approved
                AverageRating.AdvancedSearch.Save(); // AverageRating
                Rehire.AdvancedSearch.Save(); // Rehire
                Promote.AdvancedSearch.Save(); // Promote
                Appraiser.AdvancedSearch.Save(); // Appraiser
                DateInput.AdvancedSearch.Save(); // DateInput
        ...
        public void ShowFilterList()
        {
            ...
            // Field Questionnaire
            filter = QueryBuilderWhere("Questionnaire");
            if (Empty(filter))
                BuildSearchSql(ref filter, Questionnaire, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + Questionnaire.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field Approved
            filter = QueryBuilderWhere("Approved");
            if (Empty(filter))
                BuildSearchSql(ref filter, Approved, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + Approved.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field AverageRating
            filter = QueryBuilderWhere("AverageRating");
            if (Empty(filter))
                BuildSearchSql(ref filter, AverageRating, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + AverageRating.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field Rehire
            filter = QueryBuilderWhere("Rehire");
            if (Empty(filter))
                BuildSearchSql(ref filter, Rehire, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + Rehire.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field Promote
            filter = QueryBuilderWhere("Promote");
            if (Empty(filter))
                BuildSearchSql(ref filter, Promote, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + Promote.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field Appraiser
            filter = QueryBuilderWhere("Appraiser");
            if (Empty(filter))
                BuildSearchSql(ref filter, Appraiser, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + Appraiser.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field DateInput
            filter = QueryBuilderWhere("DateInput");
            if (Empty(filter))
                BuildSearchSql(ref filter, DateInput, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + DateInput.Caption + "</span>" + captionSuffix + filter + "</div>";
        ...
        public string BasicSearchWhere(bool def = false) {
            ...
            searchFlds.Add(Questionnaire);
            searchFlds.Add(Rehire);
            searchFlds.Add(Promote);
            searchFlds.Add(Appraiser);
        ...
        protected bool CheckSearchParms() {
            ...
            if (MTRankID.AdvancedSearch.IssetSession)
                return true;
            if (Questionnaire.AdvancedSearch.IssetSession)
                return true;
            if (Approved.AdvancedSearch.IssetSession)
                return true;
            if (AverageRating.AdvancedSearch.IssetSession)
                return true;
            if (Rehire.AdvancedSearch.IssetSession)
                return true;
            if (Promote.AdvancedSearch.IssetSession)
                return true;
            if (Appraiser.AdvancedSearch.IssetSession)
                return true;
            if (DateInput.AdvancedSearch.IssetSession)
                return true;
        ...
        protected void ResetAdvancedSearchParms() {
            ...
            Questionnaire.AdvancedSearch.UnsetSession();
            Approved.AdvancedSearch.UnsetSession();
            AverageRating.AdvancedSearch.UnsetSession();
            Rehire.AdvancedSearch.UnsetSession();
            Promote.AdvancedSearch.UnsetSession();
            Appraiser.AdvancedSearch.UnsetSession();
            DateInput.AdvancedSearch.UnsetSession();
        ...
        protected void RestoreSearchParms() {
            ...
            Questionnaire.AdvancedSearch.Load();
            Approved.AdvancedSearch.Load();
            AverageRating.AdvancedSearch.Load();
            Rehire.AdvancedSearch.Load();
            Promote.AdvancedSearch.Load();
            Appraiser.AdvancedSearch.Load();
            DateInput.AdvancedSearch.Load();
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(Questionnaire, ctrl); // Questionnaire
                UpdateSort(Approved, ctrl); // Approved
                UpdateSort(AverageRating, ctrl); // AverageRating
                UpdateSort(Rehire, ctrl); // Rehire
                UpdateSort(Promote, ctrl); // Promote
                UpdateSort(Appraiser, ctrl); // Appraiser
                UpdateSort(DateInput, ctrl); // DateInput
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalView && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-view"" title=""{viewcaption}"" data-table=""CrewAppraisalForAdminViewMode"" data-caption=""{viewcaption}"" data-ew-action=""modal"" data-action=""view"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(ViewUrl)) + "\" data-btn=\"null\">" + Language.Phrase("ViewLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""CrewAppraisalForAdminViewMode"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
            ...
            if (listOption != null && !IsExport() && CurrentAction == "") {
                ...
                foreach (var (key, act) in ListActions.Items) {
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        string link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fCrewAppraisalForAdminViewModelist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + " " + caption + "</button></li>";
                        if (!Empty(link)) {
                            links.Add(link);
                            if (Empty(body)) // Setup first button
                                body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fCrewAppraisalForAdminViewModelist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + caption + "</button>";
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""CrewAppraisalForAdminViewMode"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("Questionnaire")); // DN
                CreateColumnOption(option.Add("Approved")); // DN
                CreateColumnOption(option.Add("AverageRating")); // DN
                CreateColumnOption(option.Add("Rehire")); // DN
                CreateColumnOption(option.Add("Promote")); // DN
                CreateColumnOption(option.Add("Appraiser")); // DN
                CreateColumnOption(option.Add("DateInput")); // DN
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fCrewAppraisalForAdminViewModesrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fCrewAppraisalForAdminViewModesrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fCrewAppraisalForAdminViewModelist\"" + act.ToDataAttrs() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_CrewAppraisalForAdminViewMode");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(crewAppraisalForAdminViewModeList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(crewAppraisalForAdminViewModeList.RowCount) + "_CrewAppraisalForAdminViewMode");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(crewAppraisalForAdminViewModeList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && crewAppraisalForAdminViewModeList.RowType == RowType.Add || IsEdit && crewAppraisalForAdminViewModeList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        protected void LoadSearchValues() {
            ...
            // Questionnaire
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_Questionnaire[]"))
                    Questionnaire.AdvancedSearch.SearchValue = Get("x_Questionnaire[]");
                else
                    Questionnaire.AdvancedSearch.SearchValue = Get("Questionnaire"); // Default Value // DN
            if (!Empty(Questionnaire.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_Questionnaire"))
                Questionnaire.AdvancedSearch.SearchOperator = Get("z_Questionnaire");
            ...
            // AverageRating
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_AverageRating[]"))
                    AverageRating.AdvancedSearch.SearchValue = Get("x_AverageRating[]");
                else
                    AverageRating.AdvancedSearch.SearchValue = Get("AverageRating"); // Default Value // DN
            if (!Empty(AverageRating.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_AverageRating"))
                AverageRating.AdvancedSearch.SearchOperator = Get("z_AverageRating");

            // Rehire
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_Rehire[]"))
                    Rehire.AdvancedSearch.SearchValue = Get("x_Rehire[]");
                else
                    Rehire.AdvancedSearch.SearchValue = Get("Rehire"); // Default Value // DN
            if (!Empty(Rehire.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_Rehire"))
                Rehire.AdvancedSearch.SearchOperator = Get("z_Rehire");

            // Promote
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_Promote[]"))
                    Promote.AdvancedSearch.SearchValue = Get("x_Promote[]");
                else
                    Promote.AdvancedSearch.SearchValue = Get("Promote"); // Default Value // DN
            if (!Empty(Promote.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_Promote"))
                Promote.AdvancedSearch.SearchOperator = Get("z_Promote");

            // Appraiser
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_Appraiser[]"))
                    Appraiser.AdvancedSearch.SearchValue = Get("x_Appraiser[]");
                else
                    Appraiser.AdvancedSearch.SearchValue = Get("Appraiser"); // Default Value // DN
            if (!Empty(Appraiser.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_Appraiser"))
                Appraiser.AdvancedSearch.SearchOperator = Get("z_Appraiser");

            // DateInput
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_DateInput[]"))
                    DateInput.AdvancedSearch.SearchValue = Get("x_DateInput[]");
                else
                    DateInput.AdvancedSearch.SearchValue = Get("DateInput"); // Default Value // DN
            if (!Empty(DateInput.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_DateInput"))
                DateInput.AdvancedSearch.SearchOperator = Get("z_DateInput");
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                // Questionnaire
                if (Questionnaire.UseFilter && !Empty(Questionnaire.AdvancedSearch.SearchValue)) {
                    Questionnaire.EditValue = ConvertToString(Questionnaire.AdvancedSearch.SearchValue).Split(Config.MultipleOptionSeparator).ToList();
                }

                // Approved
                if (Approved.UseFilter && !Empty(Approved.AdvancedSearch.SearchValue)) {
                    Approved.EditValue = ConvertToString(Approved.AdvancedSearch.SearchValue).Split(Config.MultipleOptionSeparator).ToList();
                }

                // AverageRating
                if (AverageRating.UseFilter && !Empty(AverageRating.AdvancedSearch.SearchValue)) {
                    AverageRating.EditValue = ConvertToString(AverageRating.AdvancedSearch.SearchValue).Split(Config.MultipleOptionSeparator).ToList();
                }

                // Rehire
                if (Rehire.UseFilter && !Empty(Rehire.AdvancedSearch.SearchValue)) {
                    Rehire.EditValue = ConvertToString(Rehire.AdvancedSearch.SearchValue).Split(Config.MultipleOptionSeparator).ToList();
                }

                // Promote
                if (Promote.UseFilter && !Empty(Promote.AdvancedSearch.SearchValue)) {
                    Promote.EditValue = ConvertToString(Promote.AdvancedSearch.SearchValue).Split(Config.MultipleOptionSeparator).ToList();
                }

                // Appraiser
                if (Appraiser.UseFilter && !Empty(Appraiser.AdvancedSearch.SearchValue)) {
                    Appraiser.EditValue = ConvertToString(Appraiser.AdvancedSearch.SearchValue).Split(Config.MultipleOptionSeparator).ToList();
                }

                // DateInput
                if (DateInput.UseFilter && !Empty(DateInput.AdvancedSearch.SearchValue)) {
                    DateInput.EditValue = ConvertToString(DateInput.AdvancedSearch.SearchValue).Split(Config.MultipleOptionSeparator).ToList();
                }
        ...
        protected string GetExportTag(string type, bool custom = false) {
            ...
            if (SameText(type, "excel")) {
                if (custom)
                    return "<button type=\"button\" class=\"btn btn-default ew-export-link ew-excel\" title=\"" + HtmlEncode(Language.Phrase("ExportToExcel", true)) + "\" data-caption=\"" + HtmlEncode(Language.Phrase("ExportToExcel", true)) + "\" form=\"fCrewAppraisalForAdminViewModelist\" data-url=\"" + exportUrl + "\" data-ew-action=\"export\" data-export=\"excel\" data-custom=\"true\" data-export-selected=\"false\">" + Language.Phrase("ExportToExcel") + "</button>";
                else
                    return "<a href=\"" + exportUrl + "\" class=\"btn btn-default ew-export-link ew-excel\" title=\"" + HtmlEncode(Language.Phrase("ExportToExcel", true)) + "\" data-caption=\"" + HtmlEncode(Language.Phrase("ExportToExcel", true)) + "\">" + Language.Phrase("ExportToExcel") + "</a>";
            } else if (SameText(type, "word")) {
                if (custom)
                    return "<button type=\"button\" class=\"btn btn-default ew-export-link ew-word\" title=\"" + HtmlEncode(Language.Phrase("ExportToWord", true)) + "\" data-caption=\"" + HtmlEncode(Language.Phrase("ExportToWord", true)) + "\" form=\"fCrewAppraisalForAdminViewModelist\" data-url=\"" + exportUrl + "\" data-ew-action=\"export\" data-export=\"word\" data-custom=\"true\" data-export-selected=\"false\">" + Language.Phrase("ExportToWord") + "</button>";
                else
                    return "<a href=\"" + exportUrl + "\" class=\"btn btn-default ew-export-link ew-word\" title=\"" + HtmlEncode(Language.Phrase("ExportToWord", true)) + "\" data-caption=\"" + HtmlEncode(Language.Phrase("ExportToWord", true)) + "\">" + Language.Phrase("ExportToWord") + "</a>";
            } else if (SameText(type, "pdf")) {
                if (custom)
                    return "<button type=\"button\" class=\"btn btn-default ew-export-link ew-pdf\" title=\"" + HtmlEncode(Language.Phrase("ExportToPdf", true)) + "\" data-caption=\"" + HtmlEncode(Language.Phrase("ExportToPdf", true)) + "\" form=\"fCrewAppraisalForAdminViewModelist\" data-url=\"" + exportUrl + "\" data-ew-action=\"export\" data-export=\"pdf\" data-custom=\"true\" data-export-selected=\"false\">" + Language.Phrase("ExportToPDF") + "</button>";
                else
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fCrewAppraisalForAdminViewModesrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
            ...
            if (ModalSearch && !IsMobile())
                item.Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-table=\"CrewAppraisalForAdminViewMode\" data-caption=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-ew-action=\"modal\" data-url=\"" + AppPath("CrewAppraisalForAdminViewModeSearch") + "\" data-btn=\"SearchBtn\">" + Language.Phrase("AdvancedSearch", false) + "</a>";
            else
                item.Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-caption=\"" + Language.Phrase("AdvancedSearch", true) + "\" href=\"" + AppPath("CrewAppraisalForAdminViewModeSearch") + "\">" + Language.Phrase("AdvancedSearch", false) + "</a>";
            item.Visible = true;
