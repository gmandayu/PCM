...
public partial class PCM {
    ...
    /// mtCrewDocumentList
    /// </summary>
    public static MtCrewDocumentList mtCrewDocumentList
    {
        get => HttpData.Get<MtCrewDocumentList>("mtCrewDocumentList")!;
        set => HttpData["mtCrewDocumentList"] = value;
    }

    /// <summary>
    /// Page class for MTCrewDocument
    /// </summary>
    public class MtCrewDocumentList : MtCrewDocumentListBase
    {
        // Constructor
        public MtCrewDocumentList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public MtCrewDocumentList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class MtCrewDocumentListBase : MtCrewDocument
    {
        // Page ID
        ...
        // Table name
        public string TableName { get; set; } = "MTCrewDocument";

        // Page object name
        public string PageObjName = "mtCrewDocumentList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fMTCrewDocumentlist";
        ...
        // Constructor
        public MtCrewDocumentListBase()
        {
            // CSS class name as context
            TableVar = "MTCrewDocument";
            ...
            // Table object (mtCrewDocument)
            if (mtCrewDocument == null || mtCrewDocument is MtCrewDocument)
                mtCrewDocument = this;

            // Initialize URLs
            AddUrl = "MtCrewDocumentAdd";
            ...
            MultiDeleteUrl = "MtCrewDocumentDelete";
            MultiUpdateUrl = "MtCrewDocumentUpdate";
        ...
        // Page name
        public string PageName => "MtCrewDocumentList";
        ...
        // Constructor
        public MtCrewDocumentListBase(Controller? controller = null): this() { // DN
            if (controller != null)
                ...
                if (Response != null && !Response.HasStarted) {
                    ...
                    if (IsModal) { // Show as modal
                        ...
                        if (pageName != ListUrl) { // Not List page
                            ...
                            result.Add("view", pageName == "MtCrewDocumentView" ? "1" : "0"); // If View page, no primary button
                        } else { // List page
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            await SetupLookupOptions(MTDocumentID);
        ...
        public async Task<string> GetFilterList()
        {
            ...
            filters.Merge(JObject.Parse(IsDraft.AdvancedSearch.ToJson())); // Field IsDraft
            filters.Merge(JObject.Parse(MTCrewID.AdvancedSearch.ToJson())); // Field MTCrewID
            filters.Merge(JObject.Parse(MTDocumentID.AdvancedSearch.ToJson())); // Field MTDocumentID
            ...
            filters.Merge(JObject.Parse(DateValidUntil.AdvancedSearch.ToJson())); // Field DateValidUntil
        ...
        protected bool RestoreFilterList() {
            ...
            // Field IsDraft
            if (filter?.TryGetValue("x_IsDraft", out sv) ?? false) {
                IsDraft.AdvancedSearch.SearchValue = sv;
                IsDraft.AdvancedSearch.SearchOperator = filter["z_IsDraft"];
                IsDraft.AdvancedSearch.SearchCondition = filter["v_IsDraft"];
                IsDraft.AdvancedSearch.SearchValue2 = filter["y_IsDraft"];
                IsDraft.AdvancedSearch.SearchOperator2 = filter["w_IsDraft"];
                IsDraft.AdvancedSearch.Save();
            }
            ...
            // Field MTDocumentID
            if (filter?.TryGetValue("x_MTDocumentID", out sv) ?? false) {
                MTDocumentID.AdvancedSearch.SearchValue = sv;
                MTDocumentID.AdvancedSearch.SearchOperator = filter["z_MTDocumentID"];
                MTDocumentID.AdvancedSearch.SearchCondition = filter["v_MTDocumentID"];
                MTDocumentID.AdvancedSearch.SearchValue2 = filter["y_MTDocumentID"];
                MTDocumentID.AdvancedSearch.SearchOperator2 = filter["w_MTDocumentID"];
                MTDocumentID.AdvancedSearch.Save();
            }
            ...
            if (filter?.TryGetValue("x_DateValidUntil", out sv) ?? false) {
                DateValidUntil.AdvancedSearch.SearchValue = sv;
                DateValidUntil.AdvancedSearch.SearchOperator = filter["z_DateValidUntil"];
                DateValidUntil.AdvancedSearch.SearchCondition = filter["v_DateValidUntil"];
                DateValidUntil.AdvancedSearch.SearchValue2 = filter["y_DateValidUntil"];
                DateValidUntil.AdvancedSearch.SearchOperator2 = filter["w_DateValidUntil"];
                DateValidUntil.AdvancedSearch.Save();
            }
        ...
        public string AdvancedSearchWhere(bool def = false) {
            ...
            if (!Security.CanSearch)
                return "";
            BuildSearchSql(ref where, IsDraft, def, true); // IsDraft
            BuildSearchSql(ref where, MTCrewID, def, true); // MTCrewID
            BuildSearchSql(ref where, MTDocumentID, def, true); // MTDocumentID
            ...
            BuildSearchSql(ref where, DateValidUntil, def, true); // DateValidUntil
            ...
            if (!def && Command == "search") {
                IsDraft.AdvancedSearch.Save(); // IsDraft
                MTCrewID.AdvancedSearch.Save(); // MTCrewID
                MTDocumentID.AdvancedSearch.Save(); // MTDocumentID
                ...
                DateValidUntil.AdvancedSearch.Save(); // DateValidUntil
        ...
        public void ShowFilterList()
        {
            ...
            // Field IsDraft
            filter = QueryBuilderWhere("IsDraft");
            if (Empty(filter))
                BuildSearchSql(ref filter, IsDraft, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + IsDraft.Caption + "</span>" + captionSuffix + filter + "</div>";
            ...
            // Field MTDocumentID
            filter = QueryBuilderWhere("MTDocumentID");
            if (Empty(filter))
                BuildSearchSql(ref filter, MTDocumentID, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + MTDocumentID.Caption + "</span>" + captionSuffix + filter + "</div>";
            ...
            // Field DateValidUntil
            filter = QueryBuilderWhere("DateValidUntil");
            if (Empty(filter))
                BuildSearchSql(ref filter, DateValidUntil, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + DateValidUntil.Caption + "</span>" + captionSuffix + filter + "</div>";
        ...
        protected bool CheckSearchParms() {
            ...
            if (BasicSearch.IssetSession)
                return true;
            if (IsDraft.AdvancedSearch.IssetSession)
                return true;
            if (MTCrewID.AdvancedSearch.IssetSession)
                return true;
            if (MTDocumentID.AdvancedSearch.IssetSession)
                return true;
            ...
            if (PlaceOfIssue.AdvancedSearch.IssetSession)
                return true;
            if (DateValidUntil.AdvancedSearch.IssetSession)
                return true;
        ...
        protected void ResetAdvancedSearchParms() {
            IsDraft.AdvancedSearch.UnsetSession();
            MTCrewID.AdvancedSearch.UnsetSession();
            MTDocumentID.AdvancedSearch.UnsetSession();
            ...
            DateValidUntil.AdvancedSearch.UnsetSession();
        ...
        protected void RestoreSearchParms() {
            ...
            IsDraft.AdvancedSearch.Load();
            MTCrewID.AdvancedSearch.Load();
            MTDocumentID.AdvancedSearch.Load();
            ...
            DateValidUntil.AdvancedSearch.Load();
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(IsDraft, ctrl); // IsDraft
                UpdateSort(MTCrewID, ctrl); // MTCrewID
                UpdateSort(MTDocumentID, ctrl); // MTDocumentID
                ...
                UpdateSort(DateValidUntil, ctrl); // DateValidUntil
            ...
            if (listOption != null && !IsExport() && CurrentAction == "") {
                ...
                foreach (var (key, act) in ListActions.Items) {
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        string link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fMTCrewDocumentlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + " " + caption + "</button></li>";
                        if (!Empty(link)) {
                            links.Add(link);
                            if (Empty(body)) // Setup first button
                                body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fMTCrewDocumentlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + caption + "</button>";
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("IsDraft")); // DN
                CreateColumnOption(option.Add("MTCrewID")); // DN
                CreateColumnOption(option.Add("MTDocumentID")); // DN
                ...
                CreateColumnOption(option.Add("DateValidUntil")); // DN
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fMTCrewDocumentsrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fMTCrewDocumentsrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fMTCrewDocumentlist\"" + act.ToDataAttrs() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_MTCrewDocument");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(mtCrewDocumentList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(mtCrewDocumentList.RowCount) + "_MTCrewDocument");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(mtCrewDocumentList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && mtCrewDocumentList.RowType == RowType.Add || IsEdit && mtCrewDocumentList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        protected void LoadSearchValues() {
            ...
            // IsDraft
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_IsDraft[]"))
                    IsDraft.AdvancedSearch.SearchValue = Get("x_IsDraft[]");
                else
                    IsDraft.AdvancedSearch.SearchValue = Get("IsDraft"); // Default Value // DN
            if (!Empty(IsDraft.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_IsDraft"))
                IsDraft.AdvancedSearch.SearchOperator = Get("z_IsDraft");
            ...
            // MTDocumentID
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_MTDocumentID[]"))
                    MTDocumentID.AdvancedSearch.SearchValue = Get("x_MTDocumentID[]");
                else
                    MTDocumentID.AdvancedSearch.SearchValue = Get("MTDocumentID"); // Default Value // DN
            if (!Empty(MTDocumentID.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_MTDocumentID"))
                MTDocumentID.AdvancedSearch.SearchOperator = Get("z_MTDocumentID");
            ...
            // DateValidUntil
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_DateValidUntil[]"))
                    DateValidUntil.AdvancedSearch.SearchValue = Get("x_DateValidUntil[]");
                else
                    DateValidUntil.AdvancedSearch.SearchValue = Get("DateValidUntil"); // Default Value // DN
            if (!Empty(DateValidUntil.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_DateValidUntil"))
                DateValidUntil.AdvancedSearch.SearchOperator = Get("z_DateValidUntil");
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                // MTDocumentID
                curVal = ConvertToString(MTDocumentID.CurrentValue);
                if (!Empty(curVal)) {
                    if (MTDocumentID.Lookup != null && IsDictionary(MTDocumentID.Lookup?.Options) && MTDocumentID.Lookup?.Options.Values.Count > 0) { // Load from cache // DN
                        MTDocumentID.ViewValue = MTDocumentID.LookupCacheOption(curVal);
                    } else { // Lookup from database // DN
                        filterWrk = SearchFilter("[ID]", "=", MTDocumentID.CurrentValue, DataType.Number, "");
                        sqlWrk = MTDocumentID.Lookup?.GetSql(false, filterWrk, null, this, true, true);
                        rswrk = sqlWrk != null ? Connection.GetRows(sqlWrk) : null; // Must use Sync to avoid overwriting ViewValue in RenderViewRow
                        if (rswrk?.Count > 0 && MTDocumentID.Lookup != null) { // Lookup values found
                            var listwrk = MTDocumentID.Lookup?.RenderViewRow(rswrk[0]);
                            MTDocumentID.ViewValue = MTDocumentID.HighlightLookup(ConvertToString(rswrk[0]), MTDocumentID.DisplayValue(listwrk));
                        } else {
                            MTDocumentID.ViewValue = FormatNumber(MTDocumentID.CurrentValue, MTDocumentID.FormatPattern);
                        }
                    }
                } else {
                    MTDocumentID.ViewValue = DbNullValue;
                }
                ...
                // IsDraft
                if (IsDraft.UseFilter && !Empty(IsDraft.AdvancedSearch.SearchValue)) {
                    IsDraft.EditValue = ConvertToString(IsDraft.AdvancedSearch.SearchValue).Split(Config.MultipleOptionSeparator).ToList();
                }
                ...
                // MTDocumentID
                if (MTDocumentID.UseFilter && !Empty(MTDocumentID.AdvancedSearch.SearchValue)) {
                    MTDocumentID.EditValue = ConvertToString(MTDocumentID.AdvancedSearch.SearchValue).Split(Config.MultipleOptionSeparator).ToList();
                }
                ...
                // DateValidUntil
                if (DateValidUntil.UseFilter && !Empty(DateValidUntil.AdvancedSearch.SearchValue)) {
                    DateValidUntil.EditValue = ConvertToString(DateValidUntil.AdvancedSearch.SearchValue).Split(Config.MultipleOptionSeparator).ToList();
                }
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fMTCrewDocumentsrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
            ...
            if (ModalSearch && !IsMobile())
                item.Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-table=\"MTCrewDocument\" data-caption=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-ew-action=\"modal\" data-url=\"" + AppPath("MtCrewDocumentSearch") + "\" data-btn=\"SearchBtn\">" + Language.Phrase("AdvancedSearch", false) + "</a>";
            else
                item.Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-caption=\"" + Language.Phrase("AdvancedSearch", true) + "\" href=\"" + AppPath("MtCrewDocumentSearch") + "\">" + Language.Phrase("AdvancedSearch", false) + "</a>";
            item.Visible = true;
