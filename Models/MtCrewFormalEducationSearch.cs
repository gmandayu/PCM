...
public partial class PCM {
    /// <summary>
    /// mtCrewFormalEducationSearch
    /// </summary>
    public static MtCrewFormalEducationSearch mtCrewFormalEducationSearch
    {
        get => HttpData.Get<MtCrewFormalEducationSearch>("mtCrewFormalEducationSearch")!;
        set => HttpData["mtCrewFormalEducationSearch"] = value;
    }

    /// <summary>
    /// Page class for MTCrewFormalEducation
    /// </summary>
    public class MtCrewFormalEducationSearch : MtCrewFormalEducationSearchBase
    {
        // Constructor
        public MtCrewFormalEducationSearch(Controller controller) : base(controller)
        {
        }

        // Constructor
        public MtCrewFormalEducationSearch() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class MtCrewFormalEducationSearchBase : MtCrewFormalEducation
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "mtCrewFormalEducationSearch";
        ...
        // Constructor
        public MtCrewFormalEducationSearchBase()
        {
            // Initialize
            CurrentPage = this;
        ...
        // Page name
        public string PageName => "MtCrewFormalEducationSearch";
        ...
        // Constructor
        public MtCrewFormalEducationSearchBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (IsSearch) {
                ...
                if (!Empty(srchStr)) {
                    srchStr = "MtCrewFormalEducationList?" + srchStr;
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                mtCrewFormalEducationSearch?.PageRender();
            }
            return PageResult();
        }
        ...
        protected string BuildAdvancedSearch() {
            ...
            BuildSearchUrl(ref srchUrl, EducationLevel); // EducationLevel
            BuildSearchUrl(ref srchUrl, SchoolName); // SchoolName
            BuildSearchUrl(ref srchUrl, City); // City
            BuildSearchUrl(ref srchUrl, StartDate); // StartDate
            BuildSearchUrl(ref srchUrl, EndDate); // EndDate
        ...
        protected void LoadSearchValues() {
            ...
            // EducationLevel
            if (!IsAddOrEdit)
                if (Form.ContainsKey("x_EducationLevel"))
                    EducationLevel.AdvancedSearch.SearchValue = CurrentForm.GetValue("x_EducationLevel");
            if (Form.ContainsKey("z_EducationLevel"))
                EducationLevel.AdvancedSearch.SearchOperator = CurrentForm.GetValue("z_EducationLevel");

            // SchoolName
            if (!IsAddOrEdit)
                if (Form.ContainsKey("x_SchoolName"))
                    SchoolName.AdvancedSearch.SearchValue = CurrentForm.GetValue("x_SchoolName");
            if (Form.ContainsKey("z_SchoolName"))
                SchoolName.AdvancedSearch.SearchOperator = CurrentForm.GetValue("z_SchoolName");

            // City
            if (!IsAddOrEdit)
                if (Form.ContainsKey("x_City"))
                    City.AdvancedSearch.SearchValue = CurrentForm.GetValue("x_City");
            if (Form.ContainsKey("z_City"))
                City.AdvancedSearch.SearchOperator = CurrentForm.GetValue("z_City");
            ...
            // StartDate
            if (!IsAddOrEdit)
                if (Form.ContainsKey("x_StartDate"))
                    StartDate.AdvancedSearch.SearchValue = CurrentForm.GetValue("x_StartDate");
            if (Form.ContainsKey("z_StartDate"))
                StartDate.AdvancedSearch.SearchOperator = CurrentForm.GetValue("z_StartDate");

            // EndDate
            if (!IsAddOrEdit)
                if (Form.ContainsKey("x_EndDate"))
                    EndDate.AdvancedSearch.SearchValue = CurrentForm.GetValue("x_EndDate");
            if (Form.ContainsKey("z_EndDate"))
                EndDate.AdvancedSearch.SearchOperator = CurrentForm.GetValue("z_EndDate");
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                if (!EducationLevel.Raw)
                    EducationLevel.AdvancedSearch.SearchValue = HtmlDecode(EducationLevel.AdvancedSearch.SearchValue);
                EducationLevel.EditValue = HtmlEncode(EducationLevel.AdvancedSearch.SearchValue);
                EducationLevel.PlaceHolder = RemoveHtml(EducationLevel.Caption);

                // SchoolName
                SchoolName.SetupEditAttributes();
                if (!SchoolName.Raw)
                    SchoolName.AdvancedSearch.SearchValue = HtmlDecode(SchoolName.AdvancedSearch.SearchValue);
                SchoolName.EditValue = HtmlEncode(SchoolName.AdvancedSearch.SearchValue);
                SchoolName.PlaceHolder = RemoveHtml(SchoolName.Caption);

                // City
                City.SetupEditAttributes();
                if (!City.Raw)
                    City.AdvancedSearch.SearchValue = HtmlDecode(City.AdvancedSearch.SearchValue);
                City.EditValue = HtmlEncode(City.AdvancedSearch.SearchValue);
                City.PlaceHolder = RemoveHtml(City.Caption);

                // StartDate
                StartDate.SetupEditAttributes();
                StartDate.EditValue = FormatDateTime(UnformatDateTime(StartDate.AdvancedSearch.SearchValue, StartDate.FormatPattern), StartDate.FormatPattern); // DN
                StartDate.PlaceHolder = RemoveHtml(StartDate.Caption);

                // EndDate
                EndDate.SetupEditAttributes();
                EndDate.EditValue = FormatDateTime(UnformatDateTime(EndDate.AdvancedSearch.SearchValue, EndDate.FormatPattern), EndDate.FormatPattern); // DN
        ...
        protected bool ValidateSearch() {
            ...
            if (!Config.ServerValidate)
                return true;
            if (!CheckDate(ConvertToString(StartDate.AdvancedSearch.SearchValue), StartDate.FormatPattern)) {
                StartDate.AddErrorMessage(StartDate.GetErrorMessage(false));
            }
            if (!CheckDate(ConvertToString(EndDate.AdvancedSearch.SearchValue), EndDate.FormatPattern)) {
                EndDate.AddErrorMessage(EndDate.GetErrorMessage(false));
            }
        ...
        protected void SetupBreadcrumb() {
            ...
            breadcrumb.Add("list", TableVar, AppPath(AddMasterUrl("MtCrewFormalEducationList")), "", TableVar, true);
