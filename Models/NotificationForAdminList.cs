...
public partial class PCM {
    /// <summary>
    /// notificationForAdminList
    /// </summary>
    public static NotificationForAdminList notificationForAdminList
    {
        get => HttpData.Get<NotificationForAdminList>("notificationForAdminList")!;
        set => HttpData["notificationForAdminList"] = value;
    }

    /// <summary>
    /// Page class for NotificationForAdmin
    /// </summary>
    public class NotificationForAdminList : NotificationForAdminListBase
    {
        // Constructor
        public NotificationForAdminList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public NotificationForAdminList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class NotificationForAdminListBase : NotificationForAdmin
    {
        // Page ID
        ...
        // Table name
        public string TableName { get; set; } = "NotificationForAdmin";

        // Page object name
        public string PageObjName = "notificationForAdminList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fNotificationForAdminlist";
        ...
        // Constructor
        public NotificationForAdminListBase()
        {
            // CSS class name as context
            TableVar = "NotificationForAdmin";
            ...
            // Table object (notificationForAdmin)
            if (notificationForAdmin == null || notificationForAdmin is NotificationForAdmin)
                notificationForAdmin = this;

            // Initialize URLs
            AddUrl = "NotificationForAdminAdd";
            ...
            MultiDeleteUrl = "NotificationForAdminDelete";
            MultiUpdateUrl = "NotificationForAdminUpdate";
        ...
        // Page name
        public string PageName => "NotificationForAdminList";
        ...
        // Constructor
        public NotificationForAdminListBase(Controller? controller = null): this() { // DN
            if (controller != null)
                ...
                if (Response != null && !Response.HasStarted) {
                    ...
                    if (IsModal) { // Show as modal
                        ...
                        if (pageName != ListUrl) { // Not List page
                            ...
                            result.Add("view", pageName == "NotificationForAdminView" ? "1" : "0"); // If View page, no primary button
                        } else { // List page
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (IsModal)
                FormName = "fNotificationForAdmingrid";
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                notificationForAdminList?.PageRender();
            }
            return PageResult();
        }
        #pragma warning restore 219
        ...
        protected void SetupSortOrder() {
            ...
            if (Command != "json") {
                string defaultSort = CreatedDateTime.Expression + " DESC"; // Set up default sort
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalView && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-view"" title=""{viewcaption}"" data-table=""NotificationForAdmin"" data-caption=""{viewcaption}"" data-ew-action=""modal"" data-action=""view"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(ViewUrl)) + "\" data-btn=\"null\">" + Language.Phrase("ViewLink") + "</a>");
                else
            ...
            if (listOption != null && !IsExport() && CurrentAction == "") {
                ...
                foreach (var (key, act) in ListActions.Items) {
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        string link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fNotificationForAdminlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + " " + caption + "</button></li>";
                        if (!Empty(link)) {
                            links.Add(link);
                            if (Empty(body)) // Setup first button
                                body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fNotificationForAdminlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + caption + "</button>";
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""NotificationForAdmin"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fNotificationForAdminsrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fNotificationForAdminsrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fNotificationForAdminlist\"" + act.ToDataAttrs() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_NotificationForAdmin");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(notificationForAdminList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(notificationForAdminList.RowCount) + "_NotificationForAdmin");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(notificationForAdminList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && notificationForAdminList.RowType == RowType.Add || IsEdit && notificationForAdminList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        protected Dictionary<string, object> NewRow() {
            ...
            row.Add("Body", Body.DefaultValue ?? DbNullValue); // DN
        ...
        public async Task RenderRow()
        {
            ...
            // CrewSeafarerIDWithName
            CrewSeafarerIDWithName.CellCssStyle = "white-space: nowrap;";

            // Subject
            Subject.CellCssStyle = "white-space: nowrap;";

            // Body
            Body.CellCssStyle = "white-space: nowrap;";
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fNotificationForAdminsrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
            ...
            if (ModalSearch && !IsMobile())
                item.Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-table=\"NotificationForAdmin\" data-caption=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-ew-action=\"modal\" data-url=\"" + AppPath("NotificationForAdminSearch") + "\" data-btn=\"SearchBtn\">" + Language.Phrase("AdvancedSearch", false) + "</a>";
            else
                item.Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-caption=\"" + Language.Phrase("AdvancedSearch", true) + "\" href=\"" + AppPath("NotificationForAdminSearch") + "\">" + Language.Phrase("AdvancedSearch", false) + "</a>";
            item.Visible = true;
