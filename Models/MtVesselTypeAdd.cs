...
public partial class PCM {
    ...
    /// mtVesselTypeAdd
    /// </summary>
    public static MtVesselTypeAdd mtVesselTypeAdd
    {
        get => HttpData.Get<MtVesselTypeAdd>("mtVesselTypeAdd")!;
        set => HttpData["mtVesselTypeAdd"] = value;
    }

    /// <summary>
    /// Page class for MTVesselType
    /// </summary>
    public class MtVesselTypeAdd : MtVesselTypeAddBase
    {
        // Constructor
        public MtVesselTypeAdd(Controller controller) : base(controller)
        {
        }

        // Constructor
        public MtVesselTypeAdd() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class MtVesselTypeAddBase : MtVesselType
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "mtVesselTypeAdd";
        ...
        // Page name
        public string PageName => "MtVesselTypeAdd";
        ...
        // Constructor
        public MtVesselTypeAddBase(Controller? controller = null): this() { // DN
            if (controller != null)
            ...
            switch (CurrentAction) {
                case "copy": // Copy an existing record
                    if (rsold == null) { // Record not loaded
                        ...
                        return Terminate("MtVesselTypeList"); // No matching record, return to List page // DN
                    }
                    break;
                case "insert": // Add new record // DN
                    ...
                    if (res) { // Add successful
                        ...
                        else if (GetPageName(returnUrl) == "MtVesselTypeView")
        ...
        protected async Task<JsonBoolResult> AddRow(Dictionary<string, object>? rsold = null) { // DN
            ...
            try {
                // Abbreviation
                Abbreviation.SetDbValue(rsnew, Abbreviation.CurrentValue);
            ...
            if (!Empty(Abbreviation.CurrentValue)) { // Check field with unique index
                var filter = "(Abbreviation = '" + AdjustSql(Abbreviation.CurrentValue, DbId) + "')";
                using var rschk = await LoadReader(filter);
                if (rschk?.HasRows ?? false) { // DN
                    FailureMessage = Language.Phrase("DupIndex").Replace("%f", Abbreviation.Caption).Replace("%v", ConvertToString(Abbreviation.CurrentValue));
                    return JsonBoolResult.FalseResult;
                }
            }
