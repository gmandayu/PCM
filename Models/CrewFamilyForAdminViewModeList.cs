...
public partial class PCM {
    /// <summary>
    /// crewFamilyForAdminViewModeList
    /// </summary>
    public static CrewFamilyForAdminViewModeList crewFamilyForAdminViewModeList
    {
        get => HttpData.Get<CrewFamilyForAdminViewModeList>("crewFamilyForAdminViewModeList")!;
        set => HttpData["crewFamilyForAdminViewModeList"] = value;
    }

    /// <summary>
    /// Page class for CrewFamilyForAdminViewMode
    /// </summary>
    public class CrewFamilyForAdminViewModeList : CrewFamilyForAdminViewModeListBase
    {
        // Constructor
        public CrewFamilyForAdminViewModeList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public CrewFamilyForAdminViewModeList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class CrewFamilyForAdminViewModeListBase : CrewFamilyForAdminViewMode
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "crewFamilyForAdminViewModeList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fCrewFamilyForAdminViewModelist";
        ...
        // Constructor
        public CrewFamilyForAdminViewModeListBase()
        {
            ...
            // Initialize URLs
            AddUrl = "CrewFamilyForAdminViewModeAdd";
            ...
            MultiDeleteUrl = "CrewFamilyForAdminViewModeDelete";
            MultiUpdateUrl = "CrewFamilyForAdminViewModeUpdate";
        ...
        // Page name
        public string PageName => "CrewFamilyForAdminViewModeList";
        ...
        // Constructor
        public CrewFamilyForAdminViewModeListBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (IsModal)
                FormName = "fCrewFamilyForAdminViewModegrid";
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                crewFamilyForAdminViewModeList?.PageRender();
            }
            return PageResult();
        }
        #pragma warning restore 219
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalView && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-view"" title=""{viewcaption}"" data-table=""CrewFamilyForAdminViewMode"" data-caption=""{viewcaption}"" data-ew-action=""modal"" data-action=""view"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(ViewUrl)) + "\" data-btn=\"null\">" + Language.Phrase("ViewLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""CrewFamilyForAdminViewMode"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
            ...
            if (listOption != null && !IsExport() && CurrentAction == "") {
                ...
                foreach (var (key, act) in ListActions.Items) {
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        string link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fCrewFamilyForAdminViewModelist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + " " + caption + "</button></li>";
                        if (!Empty(link)) {
                            links.Add(link);
                            if (Empty(body)) // Setup first button
                                body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fCrewFamilyForAdminViewModelist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + caption + "</button>";
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""CrewFamilyForAdminViewMode"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fCrewFamilyForAdminViewModesrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fCrewFamilyForAdminViewModesrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fCrewFamilyForAdminViewModelist\"" + act.ToDataAttrs() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_CrewFamilyForAdminViewMode");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(crewFamilyForAdminViewModeList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(crewFamilyForAdminViewModeList.RowCount) + "_CrewFamilyForAdminViewMode");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(crewFamilyForAdminViewModeList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && crewFamilyForAdminViewModeList.RowType == RowType.Add || IsEdit && crewFamilyForAdminViewModeList.RowType == RowType.Edit) // Inline-Add/Edit row
