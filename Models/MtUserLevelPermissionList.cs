...
public partial class PCM {
    /// <summary>
    /// mtUserLevelPermissionList
    /// </summary>
    public static MtUserLevelPermissionList mtUserLevelPermissionList
    {
        get => HttpData.Get<MtUserLevelPermissionList>("mtUserLevelPermissionList")!;
        set => HttpData["mtUserLevelPermissionList"] = value;
    }

    /// <summary>
    /// Page class for MTUserLevelPermission
    /// </summary>
    public class MtUserLevelPermissionList : MtUserLevelPermissionListBase
    {
        // Constructor
        public MtUserLevelPermissionList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public MtUserLevelPermissionList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class MtUserLevelPermissionListBase : MtUserLevelPermission
    {
        // Page ID
        ...
        // Table name
        public string TableName { get; set; } = "MTUserLevelPermission";

        // Page object name
        public string PageObjName = "mtUserLevelPermissionList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fMTUserLevelPermissionlist";
        ...
        // Constructor
        public MtUserLevelPermissionListBase()
        {
            // CSS class name as context
            TableVar = "MTUserLevelPermission";
            ...
            // Table object (mtUserLevelPermission)
            if (mtUserLevelPermission == null || mtUserLevelPermission is MtUserLevelPermission)
                mtUserLevelPermission = this;

            // Initialize URLs
            AddUrl = "MtUserLevelPermissionAdd";
            ...
            MultiDeleteUrl = "MtUserLevelPermissionDelete";
            MultiUpdateUrl = "MtUserLevelPermissionUpdate";
        ...
        // Page name
        public string PageName => "MtUserLevelPermissionList";
        ...
        // Constructor
        public MtUserLevelPermissionListBase(Controller? controller = null): this() { // DN
            if (controller != null)
                ...
                if (Response != null && !Response.HasStarted) {
                    ...
                    if (IsModal) { // Show as modal
                        ...
                        if (pageName != ListUrl) { // Not List page
                            ...
                            result.Add("view", pageName == "MtUserLevelPermissionView" ? "1" : "0"); // If View page, no primary button
                        } else { // List page
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (IsModal)
                FormName = "fMTUserLevelPermissiongrid";
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                mtUserLevelPermissionList?.PageRender();
            }
            return PageResult();
        }
        #pragma warning restore 219
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalView && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-view"" title=""{viewcaption}"" data-table=""MTUserLevelPermission"" data-caption=""{viewcaption}"" data-ew-action=""modal"" data-action=""view"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(ViewUrl)) + "\" data-btn=\"null\">" + Language.Phrase("ViewLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""MTUserLevelPermission"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
            ...
            if (listOption != null && !IsExport() && CurrentAction == "") {
                ...
                foreach (var (key, act) in ListActions.Items) {
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        string link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fMTUserLevelPermissionlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + " " + caption + "</button></li>";
                        if (!Empty(link)) {
                            links.Add(link);
                            if (Empty(body)) // Setup first button
                                body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fMTUserLevelPermissionlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + caption + "</button>";
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""MTUserLevelPermission"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fMTUserLevelPermissionsrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fMTUserLevelPermissionsrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fMTUserLevelPermissionlist\"" + act.ToDataAttrs() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_MTUserLevelPermission");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(mtUserLevelPermissionList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(mtUserLevelPermissionList.RowCount) + "_MTUserLevelPermission");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(mtUserLevelPermissionList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && mtUserLevelPermissionList.RowType == RowType.Add || IsEdit && mtUserLevelPermissionList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fMTUserLevelPermissionsrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
