...
public partial class PCM {
    /// <summary>
    /// mtCrewAppraisalSearch
    /// </summary>
    public static MtCrewAppraisalSearch mtCrewAppraisalSearch
    {
        get => HttpData.Get<MtCrewAppraisalSearch>("mtCrewAppraisalSearch")!;
        set => HttpData["mtCrewAppraisalSearch"] = value;
    }

    /// <summary>
    /// Page class for MTCrewAppraisal
    /// </summary>
    public class MtCrewAppraisalSearch : MtCrewAppraisalSearchBase
    {
        // Constructor
        public MtCrewAppraisalSearch(Controller controller) : base(controller)
        {
        }

        // Constructor
        public MtCrewAppraisalSearch() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class MtCrewAppraisalSearchBase : MtCrewAppraisal
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "mtCrewAppraisalSearch";
        ...
        // Constructor
        public MtCrewAppraisalSearchBase()
        {
            // Initialize
            CurrentPage = this;
        ...
        // Page name
        public string PageName => "MtCrewAppraisalSearch";
        ...
        // Constructor
        public MtCrewAppraisalSearchBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (IsSearch) {
                ...
                if (!Empty(srchStr)) {
                    srchStr = "MtCrewAppraisalList?" + srchStr;
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                mtCrewAppraisalSearch?.PageRender();
            }
            return PageResult();
        }
        ...
        protected string BuildAdvancedSearch() {
            ...
            BuildSearchUrl(ref srchUrl, Questionnaire); // Questionnaire
            BuildSearchUrl(ref srchUrl, Approved, true); // Approved
            BuildSearchUrl(ref srchUrl, AverageRating); // AverageRating
            BuildSearchUrl(ref srchUrl, Rehire, true); // Rehire
            BuildSearchUrl(ref srchUrl, Promote, true); // Promote
            BuildSearchUrl(ref srchUrl, Appraiser); // Appraiser
            BuildSearchUrl(ref srchUrl, DateInput); // DateInput
        ...
        protected void LoadSearchValues() {
            ...
            // Questionnaire
            if (!IsAddOrEdit)
                if (Form.ContainsKey("x_Questionnaire"))
                    Questionnaire.AdvancedSearch.SearchValue = CurrentForm.GetValue("x_Questionnaire");
            if (Form.ContainsKey("z_Questionnaire"))
                Questionnaire.AdvancedSearch.SearchOperator = CurrentForm.GetValue("z_Questionnaire");
            ...
            // AverageRating
            if (!IsAddOrEdit)
                if (Form.ContainsKey("x_AverageRating"))
                    AverageRating.AdvancedSearch.SearchValue = CurrentForm.GetValue("x_AverageRating");
            if (Form.ContainsKey("z_AverageRating"))
                AverageRating.AdvancedSearch.SearchOperator = CurrentForm.GetValue("z_AverageRating");

            // Rehire
            if (!IsAddOrEdit)
                if (Form.ContainsKey("x_Rehire"))
                    Rehire.AdvancedSearch.SearchValue = CurrentForm.GetValue("x_Rehire");
            if (Form.ContainsKey("z_Rehire"))
                Rehire.AdvancedSearch.SearchOperator = CurrentForm.GetValue("z_Rehire");

            // Promote
            if (!IsAddOrEdit)
                if (Form.ContainsKey("x_Promote"))
                    Promote.AdvancedSearch.SearchValue = CurrentForm.GetValue("x_Promote");
            if (Form.ContainsKey("z_Promote"))
                Promote.AdvancedSearch.SearchOperator = CurrentForm.GetValue("z_Promote");

            // Appraiser
            if (!IsAddOrEdit)
                if (Form.ContainsKey("x_Appraiser"))
                    Appraiser.AdvancedSearch.SearchValue = CurrentForm.GetValue("x_Appraiser");
            if (Form.ContainsKey("z_Appraiser"))
                Appraiser.AdvancedSearch.SearchOperator = CurrentForm.GetValue("z_Appraiser");

            // DateInput
            if (!IsAddOrEdit)
                if (Form.ContainsKey("x_DateInput"))
                    DateInput.AdvancedSearch.SearchValue = CurrentForm.GetValue("x_DateInput");
            if (Form.ContainsKey("z_DateInput"))
                DateInput.AdvancedSearch.SearchOperator = CurrentForm.GetValue("z_DateInput");
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                curVal = ConvertToString(MTCrewID.AdvancedSearch.SearchValue);
                if (!Empty(curVal)) {
                    if (MTCrewID.Lookup != null && IsDictionary(MTCrewID.Lookup?.Options) && MTCrewID.Lookup?.Options.Values.Count > 0) { // Load from cache // DN
                        MTCrewID.EditValue = MTCrewID.LookupCacheOption(curVal);
                    } else { // Lookup from database // DN
                        ...
                        if (rswrk?.Count > 0 && MTCrewID.Lookup != null) { // Lookup values found
                            var listwrk = MTCrewID.Lookup?.RenderViewRow(rswrk[0]);
                            MTCrewID.EditValue = MTCrewID.HighlightLookup(ConvertToString(rswrk[0]), MTCrewID.DisplayValue(listwrk));
                        } else {
                            MTCrewID.EditValue = HtmlEncode(FormatNumber(MTCrewID.AdvancedSearch.SearchValue, MTCrewID.FormatPattern));
                        }
                    }
                } else {
                ...
                if (!Questionnaire.Raw)
                    Questionnaire.AdvancedSearch.SearchValue = HtmlDecode(Questionnaire.AdvancedSearch.SearchValue);
                Questionnaire.EditValue = HtmlEncode(Questionnaire.AdvancedSearch.SearchValue);
                ...
                AverageRating.EditValue = AverageRating.AdvancedSearch.SearchValue; // DN
                ...
                if (!Appraiser.Raw)
                    Appraiser.AdvancedSearch.SearchValue = HtmlDecode(Appraiser.AdvancedSearch.SearchValue);
                Appraiser.EditValue = HtmlEncode(Appraiser.AdvancedSearch.SearchValue);
                Appraiser.PlaceHolder = RemoveHtml(Appraiser.Caption);

                // DateInput
                DateInput.SetupEditAttributes();
                DateInput.EditValue = FormatDateTime(UnformatDateTime(DateInput.AdvancedSearch.SearchValue, DateInput.FormatPattern), DateInput.FormatPattern); // DN
        ...
        protected bool ValidateSearch() {
            ...
            if (!Config.ServerValidate)
                return true;
            if (!CheckNumber(ConvertToString(AverageRating.AdvancedSearch.SearchValue))) {
                AverageRating.AddErrorMessage(AverageRating.GetErrorMessage(false));
            }
            if (!CheckDate(ConvertToString(DateInput.AdvancedSearch.SearchValue), DateInput.FormatPattern)) {
        ...
        protected void SetupBreadcrumb() {
            ...
            breadcrumb.Add("list", TableVar, AppPath(AddMasterUrl("MtCrewAppraisalList")), "", TableVar, true);
