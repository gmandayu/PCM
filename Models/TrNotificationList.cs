...
public partial class PCM {
    ...
    /// trNotificationList
    /// </summary>
    public static TrNotificationList trNotificationList
    {
        get => HttpData.Get<TrNotificationList>("trNotificationList")!;
        set => HttpData["trNotificationList"] = value;
    }

    /// <summary>
    /// Page class for TRNotification
    /// </summary>
    public class TrNotificationList : TrNotificationListBase
    {
        // Constructor
        public TrNotificationList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public TrNotificationList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class TrNotificationListBase : TrNotification
    {
        // Page ID
        ...
        // Table name
        public string TableName { get; set; } = "TRNotification";

        // Page object name
        public string PageObjName = "trNotificationList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fTRNotificationlist";
        ...
        public TrNotificationListBase()
        {
            ...
            // Table object (trNotification)
            if (trNotification == null || trNotification is TrNotification)
                trNotification = this;

            // Initialize URLs
            AddUrl = "TrNotificationAdd";
            ...
            MultiDeleteUrl = "TrNotificationDelete";
            MultiUpdateUrl = "TrNotificationUpdate";
        ...
        // Page name
        public string PageName => "TrNotificationList";
        ...
        // Constructor
        public TrNotificationListBase(Controller? controller = null): this() { // DN
            if (controller != null)
                ...
                if (Response != null && !Response.HasStarted) {
                    ...
                    if (IsModal) { // Show as modal
                        ...
                        if (pageName != ListUrl) { // Not List page
                            ...
                            result.Add("view", pageName == "TrNotificationView" ? "1" : "0"); // If View page, no primary button
                        } else { // List page
            ...
            if (listOption != null && !IsExport() && CurrentAction == "") {
                ...
                foreach (var (key, act) in ListActions.Items) {
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        string link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fTRNotificationlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + " " + caption + "</button></li>";
                        if (!Empty(link)) {
                            links.Add(link);
                            if (Empty(body)) // Setup first button
                                body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fTRNotificationlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + caption + "</button>";
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            item.Body = "<div class=\"btn-group\"><div class=\"form-check\"><input type=\"checkbox\" name=\"key\" id=\"key\" class=\"form-check-input\" data-ew-action=\"select-all-keys\" form=\"fTRNotificationlist\"><label class=\"form-check-label\" for=\"key\">" + Language.Phrase("SelectAll") + "</label></div></div>";
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fTRNotificationsrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = false;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fTRNotificationsrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = false;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fTRNotificationlist\"" + act.ToDataAttrs() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_TRNotification");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(trNotificationList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(trNotificationList.RowCount) + "_TRNotification");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(trNotificationList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && trNotificationList.RowType == RowType.Add || IsEdit && trNotificationList.RowType == RowType.Edit) // Inline-Add/Edit row
