...
public partial class PCM {
    ...
    /// mtCountryEdit
    /// </summary>
    public static MtCountryEdit mtCountryEdit
    {
        get => HttpData.Get<MtCountryEdit>("mtCountryEdit")!;
        set => HttpData["mtCountryEdit"] = value;
    }

    /// <summary>
    /// Page class for MTCountry
    /// </summary>
    public class MtCountryEdit : MtCountryEditBase
    {
        // Constructor
        public MtCountryEdit(Controller controller) : base(controller)
        {
        }

        // Constructor
        public MtCountryEdit() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class MtCountryEditBase : MtCountry
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "mtCountryEdit";
        ...
        // Page name
        public string PageName => "MtCountryEdit";
        ...
        // Constructor
        public MtCountryEditBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        switch (CurrentAction) {
                case "show": // Get a record to display
                        if (!loaded) { // Load record based on key
                            ...
                            return Terminate("MtCountryList"); // No matching record, return to list
                        }
                    break;
                case "update": // Update // DN
                    CloseRecordset(); // DN
                    string returnUrl = ReturnUrl;
                    if (GetPageName(returnUrl) == "MtCountryList")
                    ...
                    if (res) {
                        ...
                        if (IsModal && UseAjaxActions) {
                            ...
                            if (GetPageName(returnUrl) != "MtCountryList") {
                                ...
                                returnUrl = "MtCountryList"; // Return list page content
                            }
                        }
        ...
        protected async Task LoadFormValues() {
            ...
            // Check field name 'Nationality' before field var 'x_Nationality'
            val = CurrentForm.HasValue("Nationality") ? CurrentForm.GetValue("Nationality") : CurrentForm.GetValue("x_Nationality");
            if (!Nationality.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("Nationality") && !CurrentForm.HasValue("x_Nationality")) // DN
                    Nationality.Visible = false; // Disable update for API request
                else
                    Nationality.SetFormValue(val);
            }

            // Check field name 'CallingCode' before field var 'x_CallingCode'
            val = CurrentForm.HasValue("CallingCode") ? CurrentForm.GetValue("CallingCode") : CurrentForm.GetValue("x_CallingCode");
            if (!CallingCode.IsDetailKey) {
                if (IsApi() && !CurrentForm.HasValue("CallingCode") && !CurrentForm.HasValue("x_CallingCode")) // DN
                    CallingCode.Visible = false; // Disable update for API request
                else
                    CallingCode.SetFormValue(val);
            }
        ...
        public void RestoreFormValues()
        {
            ...
            Nationality.CurrentValue = Nationality.FormValue;
            CallingCode.CurrentValue = CallingCode.FormValue;
        }
        ...
        public async Task RenderRow()
        {
            ...
            // Nationality
            Nationality.RowCssClass = "row";

            // CallingCode
            CallingCode.RowCssClass = "row";
        ...
        protected async Task<bool> ValidateForm() {
            ...
            if (Nationality.Required) {
                if (!Nationality.IsDetailKey && Empty(Nationality.FormValue)) {
                    Nationality.AddErrorMessage(ConvertToString(Nationality.RequiredErrorMessage).Replace("%s", Nationality.Caption));
                }
            }
            if (CallingCode.Required) {
                if (!CallingCode.IsDetailKey && Empty(CallingCode.FormValue)) {
                    CallingCode.AddErrorMessage(ConvertToString(CallingCode.RequiredErrorMessage).Replace("%s", CallingCode.Caption));
                }
            }
        ...
        protected async Task<JsonBoolResult> EditRow() { // DN
            ...
            // Nationality
            Nationality.SetDbValue(rsnew, Nationality.CurrentValue, Nationality.ReadOnly);

            // CallingCode
            CallingCode.SetDbValue(rsnew, CallingCode.CurrentValue, CallingCode.ReadOnly);
        ...
        protected void SetupBreadcrumb() {
            ...
            breadcrumb.Add("list", TableVar, AppPath(AddMasterUrl("MtCountryList")), "", TableVar, true);
            string pageId = "edit";
