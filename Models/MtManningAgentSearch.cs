...
public partial class PCM {
    /// <summary>
    /// mtManningAgentSearch
    /// </summary>
    public static MtManningAgentSearch mtManningAgentSearch
    {
        get => HttpData.Get<MtManningAgentSearch>("mtManningAgentSearch")!;
        set => HttpData["mtManningAgentSearch"] = value;
    }

    /// <summary>
    /// Page class for MTManningAgent
    /// </summary>
    public class MtManningAgentSearch : MtManningAgentSearchBase
    {
        // Constructor
        public MtManningAgentSearch(Controller controller) : base(controller)
        {
        }

        // Constructor
        public MtManningAgentSearch() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class MtManningAgentSearchBase : MtManningAgent
    {
        // Page ID
        ...
        // Page object name
        public string PageObjName = "mtManningAgentSearch";
        ...
        // Constructor
        public MtManningAgentSearchBase()
        {
            // Initialize
            CurrentPage = this;
        ...
        // Page name
        public string PageName => "MtManningAgentSearch";
        ...
        // Constructor
        public MtManningAgentSearchBase(Controller? controller = null): this() { // DN
            if (controller != null)
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (IsSearch) {
                ...
                if (!Empty(srchStr)) {
                    srchStr = "MtManningAgentList?" + srchStr;
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                mtManningAgentSearch?.PageRender();
            }
            return PageResult();
        }
        ...
        protected string BuildAdvancedSearch() {
            string srchUrl = "";
            BuildSearchUrl(ref srchUrl, Code); // Code
            BuildSearchUrl(ref srchUrl, _Name); // Name
            BuildSearchUrl(ref srchUrl, Active, true); // Active
        ...
        protected void LoadSearchValues() {
            // Code
            if (!IsAddOrEdit)
                if (Form.ContainsKey("x_Code"))
                    Code.AdvancedSearch.SearchValue = CurrentForm.GetValue("x_Code");
            if (Form.ContainsKey("z_Code"))
                Code.AdvancedSearch.SearchOperator = CurrentForm.GetValue("z_Code");
            ...
            // Active
            if (!IsAddOrEdit)
                if (Form.ContainsKey("x_Active"))
                    Active.AdvancedSearch.SearchValue = CurrentForm.GetValue("x_Active");
            if (Form.ContainsKey("z_Active"))
                Active.AdvancedSearch.SearchOperator = CurrentForm.GetValue("z_Active");
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                if (!Code.Raw)
                    Code.AdvancedSearch.SearchValue = HtmlDecode(Code.AdvancedSearch.SearchValue);
                Code.EditValue = HtmlEncode(Code.AdvancedSearch.SearchValue);
                ...
                curVal = ConvertToString(CreatedByUserID.AdvancedSearch.SearchValue);
                if (!Empty(curVal)) {
                    if (CreatedByUserID.Lookup != null && IsDictionary(CreatedByUserID.Lookup?.Options) && CreatedByUserID.Lookup?.Options.Values.Count > 0) { // Load from cache // DN
                        CreatedByUserID.EditValue = CreatedByUserID.LookupCacheOption(curVal);
                    } else { // Lookup from database // DN
                        ...
                        if (rswrk?.Count > 0 && CreatedByUserID.Lookup != null) { // Lookup values found
                            var listwrk = CreatedByUserID.Lookup?.RenderViewRow(rswrk[0]);
                            CreatedByUserID.EditValue = CreatedByUserID.HighlightLookup(ConvertToString(rswrk[0]), CreatedByUserID.DisplayValue(listwrk));
                        } else {
                            CreatedByUserID.EditValue = HtmlEncode(CreatedByUserID.AdvancedSearch.SearchValue);
                        }
                    }
                } else {
                    CreatedByUserID.EditValue = DbNullValue;
                }
                ...
                curVal = ConvertToString(LastUpdatedByUserID.AdvancedSearch.SearchValue);
                if (!Empty(curVal)) {
                    if (LastUpdatedByUserID.Lookup != null && IsDictionary(LastUpdatedByUserID.Lookup?.Options) && LastUpdatedByUserID.Lookup?.Options.Values.Count > 0) { // Load from cache // DN
                        LastUpdatedByUserID.EditValue = LastUpdatedByUserID.LookupCacheOption(curVal);
                    } else { // Lookup from database // DN
                        ...
                        if (rswrk?.Count > 0 && LastUpdatedByUserID.Lookup != null) { // Lookup values found
                            var listwrk = LastUpdatedByUserID.Lookup?.RenderViewRow(rswrk[0]);
                            LastUpdatedByUserID.EditValue = LastUpdatedByUserID.HighlightLookup(ConvertToString(rswrk[0]), LastUpdatedByUserID.DisplayValue(listwrk));
                        } else {
                            LastUpdatedByUserID.EditValue = HtmlEncode(LastUpdatedByUserID.AdvancedSearch.SearchValue);
                        }
                    }
                } else {
                    LastUpdatedByUserID.EditValue = DbNullValue;
                }
        ...
        protected void SetupBreadcrumb() {
            ...
            breadcrumb.Add("list", TableVar, AppPath(AddMasterUrl("MtManningAgentList")), "", TableVar, true);
