...
public partial class PCM {
    ...
    public class UpdateTrackingListBase : UpdateTracking
    {
        ...
        protected bool RestoreFilterList() {
            ...
            // Field ColumnName
            if (filter?.TryGetValue("x_ColumnName", out sv) ?? false) {
                ColumnName.AdvancedSearch.SearchValue = sv;
                ColumnName.AdvancedSearch.SearchOperator = filter["z_ColumnName"];
                ColumnName.AdvancedSearch.SearchCondition = filter["v_ColumnName"];
                ColumnName.AdvancedSearch.SearchValue2 = filter["y_ColumnName"];
                ColumnName.AdvancedSearch.SearchOperator2 = filter["w_ColumnName"];
                ColumnName.AdvancedSearch.Save();
            }

            // Field ChangeType
            if (filter?.TryGetValue("x_ChangeType", out sv) ?? false) {
                ChangeType.AdvancedSearch.SearchValue = sv;
                ChangeType.AdvancedSearch.SearchOperator = filter["z_ChangeType"];
                ChangeType.AdvancedSearch.SearchCondition = filter["v_ChangeType"];
                ChangeType.AdvancedSearch.SearchValue2 = filter["y_ChangeType"];
