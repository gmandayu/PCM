...
public partial class PCM {
    ...
    /// mtClothesSizeList
    /// </summary>
    public static MtClothesSizeList mtClothesSizeList
    {
        get => HttpData.Get<MtClothesSizeList>("mtClothesSizeList")!;
        set => HttpData["mtClothesSizeList"] = value;
    }

    /// <summary>
    /// Page class for MTClothesSize
    /// </summary>
    public class MtClothesSizeList : MtClothesSizeListBase
    {
        // Constructor
        public MtClothesSizeList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public MtClothesSizeList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class MtClothesSizeListBase : MtClothesSize
    {
        // Page ID
        ...
        // Table name
        public string TableName { get; set; } = "MTClothesSize";

        // Page object name
        public string PageObjName = "mtClothesSizeList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fMTClothesSizelist";
        ...
        // Constructor
        public MtClothesSizeListBase()
        {
            // CSS class name as context
            TableVar = "MTClothesSize";
            ...
            // Table object (mtClothesSize)
            if (mtClothesSize == null || mtClothesSize is MtClothesSize)
                mtClothesSize = this;

            // Initialize URLs
            AddUrl = "MtClothesSizeAdd";
            ...
            MultiDeleteUrl = "MtClothesSizeDelete";
            MultiUpdateUrl = "MtClothesSizeUpdate";
        ...
        // Page name
        public string PageName => "MtClothesSizeList";
        ...
        // Constructor
        public MtClothesSizeListBase(Controller? controller = null): this() { // DN
            if (controller != null)
                ...
                if (Response != null && !Response.HasStarted) {
                    ...
                    if (IsModal) { // Show as modal
                        ...
                        if (pageName != ListUrl) { // Not List page
                            ...
                            result.Add("view", pageName == "MtClothesSizeView" ? "1" : "0"); // If View page, no primary button
                        } else { // List page
        ...
        public async Task<string> GetFilterList()
        {
            ...
            filters.Merge(JObject.Parse(CoverallSizeDescription.AdvancedSearch.ToJson())); // Field CoverallSizeDescription
            filters.Merge(JObject.Parse(ShirtSizeDescription.AdvancedSearch.ToJson())); // Field ShirtSizeDescription
            filters.Merge(JObject.Parse(TrouserSizeDescription.AdvancedSearch.ToJson())); // Field TrouserSizeDescription
        ...
        protected bool RestoreFilterList() {
            ...
            // Field CoverallSizeDescription
            if (filter?.TryGetValue("x_CoverallSizeDescription", out sv) ?? false) {
                CoverallSizeDescription.AdvancedSearch.SearchValue = sv;
                CoverallSizeDescription.AdvancedSearch.SearchOperator = filter["z_CoverallSizeDescription"];
                CoverallSizeDescription.AdvancedSearch.SearchCondition = filter["v_CoverallSizeDescription"];
                CoverallSizeDescription.AdvancedSearch.SearchValue2 = filter["y_CoverallSizeDescription"];
                CoverallSizeDescription.AdvancedSearch.SearchOperator2 = filter["w_CoverallSizeDescription"];
                CoverallSizeDescription.AdvancedSearch.Save();
            }

            // Field ShirtSizeDescription
            if (filter?.TryGetValue("x_ShirtSizeDescription", out sv) ?? false) {
                ShirtSizeDescription.AdvancedSearch.SearchValue = sv;
                ShirtSizeDescription.AdvancedSearch.SearchOperator = filter["z_ShirtSizeDescription"];
                ShirtSizeDescription.AdvancedSearch.SearchCondition = filter["v_ShirtSizeDescription"];
                ShirtSizeDescription.AdvancedSearch.SearchValue2 = filter["y_ShirtSizeDescription"];
                ShirtSizeDescription.AdvancedSearch.SearchOperator2 = filter["w_ShirtSizeDescription"];
                ShirtSizeDescription.AdvancedSearch.Save();
            }

            // Field TrouserSizeDescription
            if (filter?.TryGetValue("x_TrouserSizeDescription", out sv) ?? false) {
                TrouserSizeDescription.AdvancedSearch.SearchValue = sv;
                TrouserSizeDescription.AdvancedSearch.SearchOperator = filter["z_TrouserSizeDescription"];
                TrouserSizeDescription.AdvancedSearch.SearchCondition = filter["v_TrouserSizeDescription"];
                TrouserSizeDescription.AdvancedSearch.SearchValue2 = filter["y_TrouserSizeDescription"];
                TrouserSizeDescription.AdvancedSearch.SearchOperator2 = filter["w_TrouserSizeDescription"];
                TrouserSizeDescription.AdvancedSearch.Save();
            }
        ...
        public string AdvancedSearchWhere(bool def = false) {
            ...
            BuildSearchSql(ref where, CoverallSizeDescription, def, true); // CoverallSizeDescription
            BuildSearchSql(ref where, ShirtSizeDescription, def, true); // ShirtSizeDescription
            BuildSearchSql(ref where, TrouserSizeDescription, def, true); // TrouserSizeDescription
            ...
            if (!def && Command == "search") {
                ...
                CoverallSizeDescription.AdvancedSearch.Save(); // CoverallSizeDescription
                ShirtSizeDescription.AdvancedSearch.Save(); // ShirtSizeDescription
                TrouserSizeDescription.AdvancedSearch.Save(); // TrouserSizeDescription
        ...
        public void ShowFilterList()
        {
            ...
            // Field CoverallSizeDescription
            filter = QueryBuilderWhere("CoverallSizeDescription");
            if (Empty(filter))
                BuildSearchSql(ref filter, CoverallSizeDescription, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + CoverallSizeDescription.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field ShirtSizeDescription
            filter = QueryBuilderWhere("ShirtSizeDescription");
            if (Empty(filter))
                BuildSearchSql(ref filter, ShirtSizeDescription, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + ShirtSizeDescription.Caption + "</span>" + captionSuffix + filter + "</div>";

            // Field TrouserSizeDescription
            filter = QueryBuilderWhere("TrouserSizeDescription");
            if (Empty(filter))
                BuildSearchSql(ref filter, TrouserSizeDescription, false, true);
            if (!Empty(filter))
                filterList += "<div><span class=\"" + captionClass + "\">" + TrouserSizeDescription.Caption + "</span>" + captionSuffix + filter + "</div>";
        ...
        protected bool CheckSearchParms() {
            ...
            if (_Name.AdvancedSearch.IssetSession)
                return true;
            if (CoverallSizeDescription.AdvancedSearch.IssetSession)
                return true;
            if (ShirtSizeDescription.AdvancedSearch.IssetSession)
                return true;
            if (TrouserSizeDescription.AdvancedSearch.IssetSession)
                return true;
        ...
        protected void ResetAdvancedSearchParms() {
            ...
            CoverallSizeDescription.AdvancedSearch.UnsetSession();
            ShirtSizeDescription.AdvancedSearch.UnsetSession();
            TrouserSizeDescription.AdvancedSearch.UnsetSession();
        ...
        protected void RestoreSearchParms() {
            ...
            CoverallSizeDescription.AdvancedSearch.Load();
            ShirtSizeDescription.AdvancedSearch.Load();
            TrouserSizeDescription.AdvancedSearch.Load();
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(_Name, ctrl); // Name
                UpdateSort(CoverallSizeDescription, ctrl); // CoverallSizeDescription
                UpdateSort(ShirtSizeDescription, ctrl); // ShirtSizeDescription
                UpdateSort(TrouserSizeDescription, ctrl); // TrouserSizeDescription
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalView && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-view"" title=""{viewcaption}"" data-table=""MTClothesSize"" data-caption=""{viewcaption}"" data-ew-action=""modal"" data-action=""view"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(ViewUrl)) + "\" data-btn=\"null\">" + Language.Phrase("ViewLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""MTClothesSize"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
            ...
            if (listOption != null && !IsExport() && CurrentAction == "") {
                ...
                foreach (var (key, act) in ListActions.Items) {
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        string link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fMTClothesSizelist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + " " + caption + "</button></li>";
                        if (!Empty(link)) {
                            links.Add(link);
                            if (Empty(body)) // Setup first button
                                body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fMTClothesSizelist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + caption + "</button>";
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""MTClothesSize"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            if (UseColumnVisibility) {
                ...
                CreateColumnOption(option.Add("CoverallSizeDescription")); // DN
                CreateColumnOption(option.Add("ShirtSizeDescription")); // DN
                CreateColumnOption(option.Add("TrouserSizeDescription")); // DN
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fMTClothesSizesrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fMTClothesSizesrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fMTClothesSizelist\"" + act.ToDataAttrs() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_MTClothesSize");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(mtClothesSizeList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(mtClothesSizeList.RowCount) + "_MTClothesSize");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(mtClothesSizeList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && mtClothesSizeList.RowType == RowType.Add || IsEdit && mtClothesSizeList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        protected void LoadSearchValues() {
            ...
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_CoverallSizeDescription[]"))
                    CoverallSizeDescription.AdvancedSearch.SearchValue = Get("x_CoverallSizeDescription[]");
                else
                    CoverallSizeDescription.AdvancedSearch.SearchValue = Get("CoverallSizeDescription"); // Default Value // DN
            if (!Empty(CoverallSizeDescription.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_CoverallSizeDescription"))
                CoverallSizeDescription.AdvancedSearch.SearchOperator = Get("z_CoverallSizeDescription");

            // ShirtSizeDescription
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_ShirtSizeDescription[]"))
                    ShirtSizeDescription.AdvancedSearch.SearchValue = Get("x_ShirtSizeDescription[]");
                else
                    ShirtSizeDescription.AdvancedSearch.SearchValue = Get("ShirtSizeDescription"); // Default Value // DN
            if (!Empty(ShirtSizeDescription.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_ShirtSizeDescription"))
                ShirtSizeDescription.AdvancedSearch.SearchOperator = Get("z_ShirtSizeDescription");

            // TrouserSizeDescription
            if (!IsAddOrEdit)
                if (Query.ContainsKey("x_TrouserSizeDescription[]"))
                    TrouserSizeDescription.AdvancedSearch.SearchValue = Get("x_TrouserSizeDescription[]");
                else
                    TrouserSizeDescription.AdvancedSearch.SearchValue = Get("TrouserSizeDescription"); // Default Value // DN
            if (!Empty(TrouserSizeDescription.AdvancedSearch.SearchValue) && Command == "")
                Command = "search";
            if (Query.ContainsKey("z_TrouserSizeDescription"))
                TrouserSizeDescription.AdvancedSearch.SearchOperator = Get("z_TrouserSizeDescription");
        ...
        public async Task RenderRow()
        {
            ...
            // CoverallSizeDescription
            CoverallSizeDescription.CellCssStyle = "white-space: nowrap;";

            // ShirtSizeDescription
            ShirtSizeDescription.CellCssStyle = "white-space: nowrap;";
            ...
            if (RowType == RowType.View) {
                ...
                // CoverallSizeDescription
                CoverallSizeDescription.ViewValue = ConvertToString(CoverallSizeDescription.CurrentValue); // DN
                CoverallSizeDescription.ViewCustomAttributes = "";

                // ShirtSizeDescription
                ShirtSizeDescription.ViewValue = ConvertToString(ShirtSizeDescription.CurrentValue); // DN
                ShirtSizeDescription.ViewCustomAttributes = "";
                ...
                // CoverallSizeDescription
                CoverallSizeDescription.HrefValue = "";
                CoverallSizeDescription.TooltipValue = "";

                // ShirtSizeDescription
                ShirtSizeDescription.HrefValue = "";
                ShirtSizeDescription.TooltipValue = "";
                ...
                // CoverallSizeDescription
                if (CoverallSizeDescription.UseFilter && !Empty(CoverallSizeDescription.AdvancedSearch.SearchValue)) {
                    CoverallSizeDescription.EditValue = ConvertToString(CoverallSizeDescription.AdvancedSearch.SearchValue).Split(Config.MultipleOptionSeparator).ToList();
                }

                // ShirtSizeDescription
                if (ShirtSizeDescription.UseFilter && !Empty(ShirtSizeDescription.AdvancedSearch.SearchValue)) {
                    ShirtSizeDescription.EditValue = ConvertToString(ShirtSizeDescription.AdvancedSearch.SearchValue).Split(Config.MultipleOptionSeparator).ToList();
                }

                // TrouserSizeDescription
                if (TrouserSizeDescription.UseFilter && !Empty(TrouserSizeDescription.AdvancedSearch.SearchValue)) {
                    TrouserSizeDescription.EditValue = ConvertToString(TrouserSizeDescription.AdvancedSearch.SearchValue).Split(Config.MultipleOptionSeparator).ToList();
                }
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fMTClothesSizesrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
            ...
            if (ModalSearch && !IsMobile())
                item.Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-table=\"MTClothesSize\" data-caption=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-ew-action=\"modal\" data-url=\"" + AppPath("MtClothesSizeSearch") + "\" data-btn=\"SearchBtn\">" + Language.Phrase("AdvancedSearch", false) + "</a>";
            else
                item.Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" + Language.Phrase("AdvancedSearch", true) + "\" data-caption=\"" + Language.Phrase("AdvancedSearch", true) + "\" href=\"" + AppPath("MtClothesSizeSearch") + "\">" + Language.Phrase("AdvancedSearch", false) + "</a>";
            item.Visible = true;
