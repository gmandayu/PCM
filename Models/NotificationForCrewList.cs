...
public partial class PCM {
    /// <summary>
    /// notificationForCrewList
    /// </summary>
    public static NotificationForCrewList notificationForCrewList
    {
        get => HttpData.Get<NotificationForCrewList>("notificationForCrewList")!;
        set => HttpData["notificationForCrewList"] = value;
    }

    /// <summary>
    /// Page class for NotificationForCrew
    /// </summary>
    public class NotificationForCrewList : NotificationForCrewListBase
    {
        // Constructor
        public NotificationForCrewList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public NotificationForCrewList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class NotificationForCrewListBase : NotificationForCrew
    {
        // Page ID
        ...
        // Table name
        public string TableName { get; set; } = "NotificationForCrew";

        // Page object name
        public string PageObjName = "notificationForCrewList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fNotificationForCrewlist";
        ...
        // Constructor
        public NotificationForCrewListBase()
        {
            ...
            // Table object (notificationForCrew)
            if (notificationForCrew == null || notificationForCrew is NotificationForCrew)
                notificationForCrew = this;

            // Initialize URLs
            AddUrl = "NotificationForCrewAdd";
            ...
            MultiDeleteUrl = "NotificationForCrewDelete";
            MultiUpdateUrl = "NotificationForCrewUpdate";
        ...
        // Page name
        public string PageName => "NotificationForCrewList";
        ...
        public void SetVisibility()
        {
            ...
            MarkAsRead.SetVisibility();
        }

        // Constructor
        public NotificationForCrewListBase(Controller? controller = null): this() { // DN
            if (controller != null)
                ...
                if (Response != null && !Response.HasStarted) {
                    ...
                    if (IsModal) { // Show as modal
                        ...
                        if (pageName != ListUrl) { // Not List page
                            ...
                            result.Add("view", pageName == "NotificationForCrewView" ? "1" : "0"); // If View page, no primary button
                        } else { // List page
        ...
        public string MultiColumnGridClass = "row-cols-md-1";
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            // Set up lookup cache
            await SetupLookupOptions(MarkAsRead);

            // Update form name to avoid conflict
            if (IsModal)
                FormName = "fNotificationForCrewgrid";
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                notificationForCrewList?.PageRender();
            }
            return PageResult();
        }
        #pragma warning restore 219
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(Body, ctrl); // Body
                UpdateSort(CreatedDateTime, ctrl); // CreatedDateTime
                UpdateSort(MarkAsRead, ctrl); // MarkAsRead
            ...
            if (listOption != null && !IsExport() && CurrentAction == "") {
                ...
                foreach (var (key, act) in ListActions.Items) {
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        string link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fNotificationForCrewlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + " " + caption + "</button></li>";
                        if (!Empty(link)) {
                            links.Add(link);
                            if (Empty(body)) // Setup first button
                                body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fNotificationForCrewlist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + caption + "</button>";
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            item.Body = "<div class=\"btn-group\"><div class=\"form-check\"><input type=\"checkbox\" name=\"key\" id=\"key\" class=\"form-check-input\" data-ew-action=\"select-all-keys\" form=\"fNotificationForCrewlist\"><label class=\"form-check-label\" for=\"key\">" + Language.Phrase("SelectAll") + "</label></div></div>";
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fNotificationForCrewsrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = false;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fNotificationForCrewsrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = false;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fNotificationForCrewlist\"" + act.ToDataAttrs() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_NotificationForCrew");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(notificationForCrewList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(notificationForCrewList.RowCount) + "_NotificationForCrew");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(notificationForCrewList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && notificationForCrewList.RowType == RowType.Add || IsEdit && notificationForCrewList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        public async Task LoadRowValues(Dictionary<string, object>? row)
        {
            ...
            MarkAsRead.SetDbValue((ConvertToBool(row["MarkAsRead"]) ? "1" : "0"));
        }
        ...
        protected Dictionary<string, object> NewRow() {
            ...
            row.Add("MarkAsRead", MarkAsRead.DefaultValue ?? DbNullValue); // DN
            return row;
        }
