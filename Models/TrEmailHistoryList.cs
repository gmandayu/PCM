...
public partial class PCM {
    ...
    /// trEmailHistoryList
    /// </summary>
    public static TrEmailHistoryList trEmailHistoryList
    {
        get => HttpData.Get<TrEmailHistoryList>("trEmailHistoryList")!;
        set => HttpData["trEmailHistoryList"] = value;
    }

    /// <summary>
    /// Page class for TREmailHistory
    /// </summary>
    public class TrEmailHistoryList : TrEmailHistoryListBase
    {
        // Constructor
        public TrEmailHistoryList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public TrEmailHistoryList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class TrEmailHistoryListBase : TrEmailHistory
    {
        // Page ID
        ...
        // Table name
        public string TableName { get; set; } = "TREmailHistory";

        // Page object name
        public string PageObjName = "trEmailHistoryList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fTREmailHistorylist";
        ...
        // Constructor
        public TrEmailHistoryListBase()
        {
            // CSS class name as context
            TableVar = "TREmailHistory";
            ...
            // Table object (trEmailHistory)
            if (trEmailHistory == null || trEmailHistory is TrEmailHistory)
                trEmailHistory = this;

            // Initialize URLs
            AddUrl = "TrEmailHistoryAdd";
            ...
            MultiDeleteUrl = "TrEmailHistoryDelete";
            MultiUpdateUrl = "TrEmailHistoryUpdate";
        ...
        // Page name
        public string PageName => "TrEmailHistoryList";
        ...
        // Constructor
        public TrEmailHistoryListBase(Controller? controller = null): this() { // DN
            if (controller != null)
                ...
                if (Response != null && !Response.HasStarted) {
                    ...
                    if (IsModal) { // Show as modal
                        ...
                        if (pageName != ListUrl) { // Not List page
                            ...
                            result.Add("view", pageName == "TrEmailHistoryView" ? "1" : "0"); // If View page, no primary button
                        } else { // List page
        ...
        protected bool RestoreFilterList() {
            ...
            if (filter?.TryGetValue("x_MTCrew_ID", out sv) ?? false) {
                ...
                MTCrew_ID.AdvancedSearch.SearchOperator = filter["z_MTCrew_ID"];
                MTCrew_ID.AdvancedSearch.SearchCondition = filter["v_MTCrew_ID"];
            ...
            if (listOption != null && !IsExport() && CurrentAction == "") {
                ...
                foreach (var (key, act) in ListActions.Items) {
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        string link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fTREmailHistorylist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + " " + caption + "</button></li>";
                        if (!Empty(link)) {
                            links.Add(link);
                            if (Empty(body)) // Setup first button
                                body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fTREmailHistorylist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + caption + "</button>";
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fTREmailHistorysrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fTREmailHistorysrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fTREmailHistorylist\"" + act.ToDataAttrs() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_TREmailHistory");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(trEmailHistoryList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(trEmailHistoryList.RowCount) + "_TREmailHistory");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(trEmailHistoryList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && trEmailHistoryList.RowType == RowType.Add || IsEdit && trEmailHistoryList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fTREmailHistorysrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
