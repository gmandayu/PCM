...
public partial class PCM {
    /// <summary>
    /// mtRecruitmentStatusList
    /// </summary>
    public static MtRecruitmentStatusList mtRecruitmentStatusList
    {
        get => HttpData.Get<MtRecruitmentStatusList>("mtRecruitmentStatusList")!;
        set => HttpData["mtRecruitmentStatusList"] = value;
    }

    /// <summary>
    /// Page class for MTRecruitmentStatus
    /// </summary>
    public class MtRecruitmentStatusList : MtRecruitmentStatusListBase
    {
        // Constructor
        public MtRecruitmentStatusList(Controller controller) : base(controller)
        {
        }

        // Constructor
        public MtRecruitmentStatusList() : base()
        {
        }
    }
    ...
    /// Page base class
    /// </summary>
    public class MtRecruitmentStatusListBase : MtRecruitmentStatus
    {
        // Page ID
        ...
        // Table name
        public string TableName { get; set; } = "MTRecruitmentStatus";

        // Page object name
        public string PageObjName = "mtRecruitmentStatusList";

        // Title
        public string? Title = null; // Title for <title> tag

        // Grid form hidden field names
        public string FormName = "fMTRecruitmentStatuslist";
        ...
        // Constructor
        public MtRecruitmentStatusListBase()
        {
            // CSS class name as context
            TableVar = "MTRecruitmentStatus";
            ...
            // Table object (mtRecruitmentStatus)
            if (mtRecruitmentStatus == null || mtRecruitmentStatus is MtRecruitmentStatus)
                mtRecruitmentStatus = this;

            // Initialize URLs
            AddUrl = "MtRecruitmentStatusAdd";
            ...
            MultiDeleteUrl = "MtRecruitmentStatusDelete";
            MultiUpdateUrl = "MtRecruitmentStatusUpdate";
        ...
        // Page name
        public string PageName => "MtRecruitmentStatusList";
        ...
        // Constructor
        public MtRecruitmentStatusListBase(Controller? controller = null): this() { // DN
            if (controller != null)
                ...
                if (Response != null && !Response.HasStarted) {
                    ...
                    if (IsModal) { // Show as modal
                        ...
                        if (pageName != ListUrl) { // Not List page
                            ...
                            result.Add("view", pageName == "MtRecruitmentStatusView" ? "1" : "0"); // If View page, no primary button
                        } else { // List page
        ...
        public override async Task<IActionResult> Run()
        {
            ...
            if (IsModal)
                FormName = "fMTRecruitmentStatusgrid";
            ...
            if (!IsApi() && !IsTerminated) {
                ...
                // Page Render event
                mtRecruitmentStatusList?.PageRender();
            }
            return PageResult();
        }
        #pragma warning restore 219
        ...
        protected void SetupSortOrder() {
            ...
            if (Get("order", out StringValues sv)) {
                ...
                UpdateSort(Description, ctrl); // Description
        ...
        public async Task RenderListOptions()
        {
            ...
            if (isVisible) {
                if (ModalView && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-view"" title=""{viewcaption}"" data-table=""MTRecruitmentStatus"" data-caption=""{viewcaption}"" data-ew-action=""modal"" data-action=""view"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(ViewUrl)) + "\" data-btn=\"null\">" + Language.Phrase("ViewLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalEdit && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-edit"" title=""{editcaption}"" data-table=""MTRecruitmentStatus"" data-caption=""{editcaption}"" data-ew-action=""modal"" data-action=""edit"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(EditUrl)) + "\" data-btn=\"SaveBtn\">" + Language.Phrase("EditLink") + "</a>");
                else
            ...
            if (isVisible) {
                if (ModalAdd && !IsMobile())
                    listOption?.SetBody($@"<a class=""ew-row-link ew-copy"" title=""{copycaption}"" data-table=""MTRecruitmentStatus"" data-caption=""{copycaption}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(CopyUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("CopyLink") + "</a>");
                else
            ...
            if (listOption != null && !IsExport() && CurrentAction == "") {
                ...
                foreach (var (key, act) in ListActions.Items) {
                    if (act.Select == Config.ActionSingle && act.Allowed) {
                        ...
                        string link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fMTRecruitmentStatuslist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + " " + caption + "</button></li>";
                        if (!Empty(link)) {
                            links.Add(link);
                            if (Empty(body)) // Setup first button
                                body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlTitle(caption) + "\" data-caption=\"" + HtmlTitle(caption) + "\" data-ew-action=\"submit\" form=\"fMTRecruitmentStatuslist\" data-key=\"" + KeyToJson(true) + "\"" + act.ToDataAttrs() + ">" + icon + caption + "</button>";
                        }
                    }
                }
        ...
        protected void SetupOtherOptions() {
            ...
            if (ModalAdd && !IsMobile())
                item.Body = $@"<a class=""ew-add-edit ew-add"" title=""{addTitle}"" data-table=""MTRecruitmentStatus"" data-caption=""{addTitle}"" data-ew-action=""modal"" data-action=""add"" data-ajax=""" + (UseAjaxActions ? "true" : "false") + "\" data-url=\"" + HtmlEncode(AppPath(AddUrl)) + "\" data-btn=\"AddBtn\">" + Language.Phrase("AddLink") + "</a>";
            else
            ...
            item.Body = "<a class=\"ew-save-filter\" data-form=\"fMTRecruitmentStatussrch\" data-ew-action=\"none\">" + Language.Phrase("SaveCurrentFilter") + "</a>";
            item.Visible = true;
            item = FilterOptions.Add("deletefilter");
            item.Body = "<a class=\"ew-delete-filter\" data-form=\"fMTRecruitmentStatussrch\" data-ew-action=\"none\">" + Language.Phrase("DeleteFilter") + "</a>";
            item.Visible = true;
        ...
        public void RenderOtherOptions()
        {
            ...
            var options = OtherOptions;
                ...
                foreach (var (key, act) in ListActions.Items.Where(kvp => kvp.Value.Select == Config.ActionMultiple)) {
                    ...
                    item.Body = "<<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" + HtmlEncode(caption) + "\" data-caption=\"" + HtmlEncode(caption) + "\" data-ew-action=\"submit\" form=\"fMTRecruitmentStatuslist\"" + act.ToDataAttrs() + ">" + icon + "</button>";
        ...
        public async Task SetupRow()
        {
            if (IsGridAdd || IsGridEdit) {
                if (SameString(RowIndex, "$rowindex$")) { // Render template row first
                    ...
                    RowAttrs.Add("id", "r0_MTRecruitmentStatus");
            ...
            RowAttrs.Add("data-rowindex", ConvertToString(mtRecruitmentStatusList.RowCount));
            RowAttrs.Add("data-key", GetKey(true));
            RowAttrs.Add("id", "r" + ConvertToString(mtRecruitmentStatusList.RowCount) + "_MTRecruitmentStatus");
            RowAttrs.Add("data-rowtype", ConvertToString((int)RowType));
            RowAttrs.AppendClass(mtRecruitmentStatusList.RowCount % 2 != 1 ? "ew-table-alt-row" : "");
            if (IsAdd && mtRecruitmentStatusList.RowType == RowType.Add || IsEdit && mtRecruitmentStatusList.RowType == RowType.Edit) // Inline-Add/Edit row
        ...
        public async Task RenderRow()
        {
            ...
            if (RowType == RowType.View) {
                ...
                Code.ViewValue = FormatNumber(Code.ViewValue, Code.FormatPattern);
        ...
        protected void SetupSearchOptions() {
            ...
            item.Body = "<a class=\"btn btn-default ew-search-toggle" + searchToggleClass + "\" role=\"button\" title=\"" + Language.Phrase("SearchPanel") + "\" data-caption=\"" + Language.Phrase("SearchPanel") + "\" data-ew-action=\"search-toggle\" data-form=\"fMTRecruitmentStatussrch\" aria-pressed=\"" + (searchToggleClass == " active" ? "true" : "false") + "\">" + Language.Phrase("SearchLink") + "</a>";
            item.Visible = true;
